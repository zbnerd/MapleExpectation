...
public enum CommonErrorCode implements ErrorCode {
  ...
  SERVICE_UNAVAILABLE(
      "S007", "서비스가 일시적으로 사용할 수 없습니다. 잠시 후 다시 시도해주세요.", HttpStatus.SERVICE_UNAVAILABLE),
  REDIS_SCRIPT_EXECUTION_FAILED(
      "S008", "Redis 스크립트 실행 실패 (스크립트: %s)", HttpStatus.INTERNAL_SERVER_ERROR),
  DATABASE_NAMED_LOCK_FAILED(
      "S009",
      "DB named lock 처리 실패: %s (lockKey=%s, waitTime=%s)",
  ...
  MYSQL_FALLBACK_FAILED(
      "S012", "MySQL 장애 시 Fallback 실패 (ocid: %s)", HttpStatus.SERVICE_UNAVAILABLE),
  COMPENSATION_SYNC_FAILED(
      "S013", "Compensation Log 동기화 실패 (entryId: %s)", HttpStatus.INTERNAL_SERVER_ERROR),
