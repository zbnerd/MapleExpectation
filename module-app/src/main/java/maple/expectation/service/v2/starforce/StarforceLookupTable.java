 ...
 *   <li>🔴 Red (SRE): 서버 시작 시 초기화 (ApplicationRunner)
 *   <li>🟢 Green (Performance): 인메모리 캐시로 O(1) 조회
 * </ul>
 *
 * <h3>메이플스토리 스타포스 규칙</h3>
 *
 * <ul>
 *   <li>0~10성: 파괴 없음, 100% 성공 또는 30% 성공
 *   <li>11~14성: 파괴 확률 존재
 *   <li>15~25성: 파괴/하락 확률 존재, 세이프가드 옵션
 * </ul>
 *
...
public interface StarforceLookupTable {
   ...
   * @return 기대 파괴 횟수
   */
  BigDecimal getExpectedDestroyCount(
      int currentStar,
      int targetStar,
      boolean useStarCatch,
      boolean useSundayMaple,
   ...
   * 초기화 (서버 시작 시 호출)
   *
   * <p>Pre-compute all starforce expected values for faster lookup.
   */
  void initialize();
