...
public class FlameScoreCalculator {
  ...
  public List<Map<Integer, Double>> buildOptionPmfs(
    ...
    Map<Integer, Double> stageProbs =
        FlameStageProbability.getStageProbs(category.isBossDrop(), flameType);
    FlameOptionType[] optionPool =
        category.isWeapon() ? FlameOptionType.WEAPON_OPTIONS : FlameOptionType.ARMOR_OPTIONS;

    List<Map<Integer, Double>> pmfs = new ArrayList<>();
    for (FlameOptionType option : optionPool) {
      Map<Integer, Double> pmf =
          buildSingleOptionPmf(
  ...
  private Integer calculateArmorScore(
      FlameOptionType option, int level, int stage, JobWeights weights) {
  ...
  private Integer calculateCompositeScore(
      FlameOptionType option, int level, int stage, JobWeights weights) {
    Integer value = FlameStatTable.getArmorValue(option, level, stage);
    if (value == null) {
      return null;
