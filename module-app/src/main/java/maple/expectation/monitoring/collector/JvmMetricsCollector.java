 ...
 *
 * <ul>
 *   <li>Heap Memory (used, max, committed)
 *   <li>GC 통계 (count, time)
 *   <li>Thread Count (live, peak, daemon)
 *   <li>Class Loading
 * </ul>
 *
...
public class JvmMetricsCollector implements MetricsCollectorStrategy {
  ...
  private void collectGcMetrics(Map<String, Object> metrics) {
    // GC Pause Time
    var gcPauseTime = meterRegistry.find("jvm.gc.pause").timer();

    if (gcPauseTime != null) {
      metrics.put("gc_pause_count", gcPauseTime.count());
      metrics.put(
          "gc_pause_total_ms",
          formatDouble(gcPauseTime.totalTime(java.util.concurrent.TimeUnit.MILLISECONDS)));
    }

    // GC Memory Allocated
    var gcAlloc = meterRegistry.find("jvm.gc.memory.allocated").counter();
