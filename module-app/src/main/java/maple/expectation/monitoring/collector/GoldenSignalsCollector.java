 ...
 *
 * <ul>
 *   <li><b>Latency</b>: 요청 처리 시간 (p50, p95, p99)
 *   <li><b>Traffic</b>: 초당 요청 수 (RPS)
 *   <li><b>Errors</b>: 에러율 (5xx / total)
 *   <li><b>Saturation</b>: 자원 포화도 (버퍼, 커넥션 풀)
 * </ul>
 *
 * <h4>CLAUDE.md 준수사항</h4>
 *
 * <ul>
 *   <li>Section 4 (SOLID): Strategy 패턴 구현
 *   <li>Section 12 (LogicExecutor): 예외 없는 조회 로직
 * </ul>
 *
...
public class GoldenSignalsCollector implements MetricsCollectorStrategy {
  ...
  private void collectLatencyMetrics(Map<String, Object> metrics) {
    ...
    if (httpTimer != null) {
      ...
      metrics.put(
          "latency_p95_ms", formatDouble(httpTimer.percentile(0.95, TimeUnit.MILLISECONDS)));
      metrics.put(
          "latency_p99_ms", formatDouble(httpTimer.percentile(0.99, TimeUnit.MILLISECONDS)));
  ...
  private void collectTrafficMetrics(Map<String, Object> metrics) {
    ...
    // Nexon API 호출 통계
    Timer nexonTimer = meterRegistry.find("nexon.api.performance").timer();
  ...
  private void collectErrorMetrics(Map<String, Object> metrics) {
    ...
    Counter errorCounter =
        meterRegistry.find("http.server.requests").tag("status", "5xx").counter();
  ...
  private void collectSaturationMetrics(Map<String, Object> metrics) {
    // HikariCP 커넥션 풀 포화도
    var activeConnections = meterRegistry.find("hikaricp.connections.active").gauge();
    var maxConnections = meterRegistry.find("hikaricp.connections.max").gauge();
    ...
    // 버퍼 대기 항목 수
    var bufferPending = meterRegistry.find("buffer.pending").gauge();
