 ...
 *
 * <ul>
 *   <li>🔴 Red (SRE): 비동기 처리 (CompletableFuture)
 *   <li>🟣 Purple (Auditor): BigDecimal 기반 응답
 *   <li>🟢 Green (Performance): InnoDB Buffer Pool 오염 방지
 * </ul>
 *
 * <h3>V3와의 차이</h3>
 *
 * <ul>
 *   <li>비용 상세 분류: 블랙큐브, 레드큐브, 에디셔널, 스타포스
 *   <li>프리셋별 기대값: 프리셋 1, 2, 3 개별 조회 가능
 *   <li>정밀 계산: BigDecimal 기반 (long truncation 방지)
 *   <li>캐시 전략: DB 저장으로 Buffer Pool 오염 방지
 * </ul>
 *
 * <h3>API Endpoints</h3>
 *
 * <ul>
 *   <li>GET /api/v4/characters/{userIgn}/expectation - 전체 기대값 조회
 *   <li>GET /api/v4/characters/{userIgn}/expectation/{presetNo} - 특정 프리셋 조회
 * </ul>
 */
@Slf4j
@RestController
...
public class GameCharacterControllerV4 {
   ...
   *
   * <ul>
   *   <li>L1 캐시 히트 → 스레드풀 우회, 동기 반환
   *   <li>L1 캐시 미스 → 기존 비동기 경로 사용
   *   <li>force=true → Fast Path 스킵, 강제 재계산
   * </ul>
   *
   * <h4>응답 형식</h4>
   *
   * <ul>
   *   <li>Accept-Encoding: gzip → GZIP 바이트 직접 반환 (서버 CPU 절감)
   *   <li>그 외 → JSON 응답 (기존 방식)
   * </ul>
   *
   * <h4>쿼리 파라미터</h4>
   *
   * <ul>
   *   <li>force=true: 캐시 무시하고 강제 재계산
   *   <li>force=false (기본): 캐시 응답 사용
   * </ul>
   *
   * <h4>성능 이점 (#264)</h4>
   *
   * <ul>
   *   <li>L1 히트: 0.1ms (스레드풀 경합 없음, RPS 3-5x 향상)
   *   <li>네트워크: 200KB → 15KB (93% 감소)
   * </ul>
   *
  ...
  public CompletableFuture<ResponseEntity<?>> getExpectation(
    ...
    log.debug(
        "[V4] Calculating expectation for: {} (force={}, gzip={})",
        maskIgn(userIgn),
        force,
