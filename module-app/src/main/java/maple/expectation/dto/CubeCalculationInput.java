 ...
 *
 * <ul>
 *   <li><b>기존 모드</b>: options 리스트로 정확한 옵션 조합 지정
 *   <li><b>DP 모드</b>: targetStatType + minTotal로 "21% 이상" 같은 누적 확률 계산
 * </ul>
 *
 * <h3>#240 V4 확장</h3>
 *
 * <ul>
 *   <li>에디셔널 잠재능력 (additionalGrade, additionalOptions)
 *   <li>스타포스 정보 (starforce, starforceScrollFlag)
 *   <li>아이콘 URL (itemIcon)
 *   <li>장비 세부 분류 (itemEquipmentPart)
 * </ul>
 */
@Data
@Builder
...
public class CubeCalculationInput {
  ...
  @Builder.Default private List<String> options = new ArrayList<>(); // 옵션 3줄 리스트 (기존 방식)
   ...
   * 아이템 아이콘 URL
   *
   * <p>예: "https://open.api.nexon.com/static/maplestory/..."
   */
  ...
  /** 에디셔널 잠재능력 옵션 3줄 */
  @Builder.Default private List<String> additionalOptions = new ArrayList<>();
  ...
  /** Tail Clamp 활성화 여부 true: 상태공간 O(target) 보장 (기본값) false: 전체 상태공간 계산 (디버그/검증용) */
  @Builder.Default private boolean enableTailClamp = true;
   ...
   *
   * <p>P0: UNKNOWN 타입 거부 추가 (Fail-Fast)
   *
   ...
   *
   * <p>잠재능력이 없는 장비(특수스킬반지 등)도 화면에 표시하기 위해 part(슬롯)만 있으면 true를 반환합니다.
   *
