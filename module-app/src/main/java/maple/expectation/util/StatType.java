...
public enum StatType {
  ...
  /** -- GETTER -- 개별 스탯 타입 여부 (STR, DEX, INT, LUK) ALLSTAT은 개별 스탯이 아님 (복합) */
  ...
  /**
   * 문자열을 분석해서 어떤 스탯인지 찾아냅니다. (기존 방식 - 하위호환) 예: "STR +12%" -> StatType.STR 예: "스킬 재사용 대기시간 -2초" ->
  ...
  /**
   * 문자열을 분석해서 단위 포함 스탯 타입을 찾아냅니다. (DP 엔진용) 예: "STR +12%" -> StatType.STR_PERCENT 예: "올스탯 +9%" ->
   * StatType.ALLSTAT_PERCENT 예: "STR +30" -> StatType.STR (플랫)
   *
   * @param option 옵션 문자열
  ...
  /**
   * 문자열에서 모든 매칭되는 스탯 타입을 찾아냅니다. (복합 옵션 대응) 예: "STR/DEX +6%" -> [STR_PERCENT, DEX_PERCENT] 예: "올스탯
   * +9%" -> [ALLSTAT_PERCENT]
   *
   * <p>P0: 키워드 오탐 방지 - longest-first 매칭 적용
   *
   * <p>예: "보스 몬스터 공격 시 데미지" → BOSS_DAMAGE만 (DAMAGE 오탐 방지)
   *
   * <p>하위호환: 매칭 실패 시 UNKNOWN 반환 (기존 동작 유지)
   *
   * @param option 옵션 문자열
   ...
   *
   * <p>비-핵심 경로용 (프록 옵션 등 무시 가능한 경우)
   *
   * <p>P0: 키워드 오탐 방지 - longest-first 매칭 적용
   *
   * <p>#240 V4: 레벨당 스탯 최우선 감지
   *
   * @param option 옵션 문자열
   ...
   * 레벨당 스탯 감지 (#240 V4)
   *
   * <p>"캐릭터 기준 9레벨 당" 패턴을 최우선으로 감지하여 "STR +21" 같은 깡스탯으로 오인식되는 것을 방지합니다.
   *
   * @param option 옵션 문자열
  ...
  private static boolean isLevelBasedStat(StatType type) {
    return type == LEVEL_STR || type == LEVEL_DEX || type == LEVEL_INT || type == LEVEL_LUK;
  }

  /**
   * Primary stat 계열 키워드 포함 여부 (Drift 감지용)
   *
   * <p>STR, DEX, INT, LUK, 올스탯 계열이 포함되어 있는지 휴리스틱 판별
   *
   * @param option 옵션 문자열
   ...
   * 옵션 카테고리 Enum (#240 V4)
   *
   * <p>복합 옵션 감지용: 서로 다른 카테고리의 옵션이 2개 이상이면 복합 옵션
   *
   * <ul>
   *   <li>STAT: STR, DEX, INT, LUK, 올스탯 (주 스탯)
   *   <li>BOSS_IED: 보스 데미지, 방어율 무시
   *   <li>ATK_MAG: 공격력, 마력
   *   <li>CRIT_DMG: 크리티컬 데미지
   *   <li>COOLDOWN: 스킬 재사용 대기시간 감소
   *   <li>OTHER: 기타 (메소, 아이템 드롭 등)
   * </ul>
   */
  ...
  public OptionCategory getCategory() {
    return switch (this) {
      ...
      case ATTACK_POWER, MAGIC_POWER, ATTACK_POWER_PERCENT, MAGIC_POWER_PERCENT ->
          OptionCategory.ATK_MAG;
