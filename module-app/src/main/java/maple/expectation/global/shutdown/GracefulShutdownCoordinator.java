...
public class GracefulShutdownCoordinator implements SmartLifecycle {
   ...
   *
   * <ul>
   *   <li>start(): 이 인스턴스의 Coordinator가 활성화되었음을 표시
   *   <li>stop(): 이 인스턴스의 4단계 순차 종료를 실행 후 false로 전환
   *   <li>isRunning(): Spring이 이 인스턴스의 lifecycle 상태를 확인
   * </ul>
   *
   * <p>각 인스턴스는 독립적으로 SIGTERM을 수신하고 자신의 종료 절차를 실행합니다. DB 동기화 단계에서는 {@link LockStrategy} 분산 락으로 중복
   * 실행을 방지합니다.
   *
  ...
  public GracefulShutdownCoordinator(
    ...
    this.shutdownTimer =
        Timer.builder("shutdown.coordinator.duration")
            .description("Graceful Shutdown 총 소요 시간")
            .register(meterRegistry);
    this.shutdownSuccessCounter =
        Counter.builder("shutdown.coordinator.result")
            .tag("status", "success")
            .description("Shutdown 성공 횟수")
            .register(meterRegistry);
    this.shutdownFailureCounter =
        Counter.builder("shutdown.coordinator.result")
            .tag("status", "failure")
   ...
   *
   * <p>try-catch 금지 -> LogicExecutor.executeWithFinally() 사용
   */
  @Override
  public void stop() {
    ...
    return executor.execute(
        () -> {
          ...
          boolean allCompleted =
              persistenceTracker.awaitAllCompletion(properties.getEquipmentAwaitTimeout());
          ...
          if (!allCompleted) {
            ...
            return new ShutdownData(
                LocalDateTime.now(), properties.getInstanceId(), Map.of(), pendingOcids);
          }
  ...
  private ShutdownData flushLikeBuffer(ShutdownData backupData) {
    return executor.executeOrCatch(
        () -> {
          ...
          log.info(
              "[2/4] 로컬 좋아요 버퍼 Flush 완료: Redis {}건, 파일 백업 {}건",
              result.redisSuccessCount(),
              result.fileBackupCount());
          return backupData;
        },
        (e) -> {
