...
public class SingleFlightExecutor<T> {
  ...
  public SingleFlightExecutor(
      int followerTimeoutSeconds, Executor executor, Function<String, T> timeoutFallback) {
   ...
   *
   * <ol>
   *   <li>키에 대한 inFlight 엔트리 확인
   *   <li>없으면 Leader로 등록 후 계산 시작
   *   <li>있으면 Follower로 Leader 결과 대기
   * </ol>
   *
   ...
   *
   * <p><b>CLAUDE.md Section 12 준수:</b> CompletableFuture.handle()로 동기 예외 처리
   */
  private CompletableFuture<T> executeAsLeader(
      String key, InFlightEntry<T> entry, Supplier<CompletableFuture<T>> asyncSupplier) {
  ...
  private void cleanupLeaderEntry(
      CompletableFuture<T> promise, String key, InFlightEntry<T> entry) {

    if (!promise.isDone()) {
      promise.completeExceptionally(new IllegalStateException("Leader aborted before completion"));
    }
