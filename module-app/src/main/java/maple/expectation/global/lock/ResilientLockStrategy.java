 ...
 *
 * <ul>
 *   <li>Redis: 고성능, 저지연 (< 1ms), Watchdog 자동 갱신
 *   <li>MySQL Fallback: Redis 장애 시에도 가용성 보장
 *   <li>Circuit Breaker: Redis 장애 감지 → 자동 전환
 * </ul>
 *
 * <p>LogicExecutor.executeWithFallback()을 사용하여 단일 파이프라인에서 try-catch/throws 없이 예외 필터링을 구현합니다.
 *
 * <p><b>예외 필터링 정책</b>:
 *
 * <ul>
 *   <li>비즈니스 예외 (ClientBaseException): Fallback 없이 즉시 전파
 *   <li>인프라 예외 (Redis/CircuitBreaker): MySQL Fallback 허용
 *   <li>Unknown 예외 (NPE 등): 즉시 전파 (버그 조기 발견)
 *   <li>Checked Throwable: 정책 위반이므로 fail-fast (IllegalStateException)
 * </ul>
 *
 * <h3>MySQL Fallback 조건부 로딩</h3>
 *
 * <p>MySqlNamedLockStrategy는 @ConditionalOnBean(name = "lockJdbcTemplate")이므로, lockJdbcTemplate이 없는
 * 환경(test 프로필만 활성화)에서는 Redis-only 모드로 동작합니다. MySQL fallback이 불가능한 경우, 명확한 예외 메시지와 함께 실패합니다.
 *
...
public class ResilientLockStrategy extends AbstractLockStrategy {
  ...
  @Nullable private final LockStrategy mysqlLockStrategy;
   ...
   *
   * <ul>
   *   <li>Biz(ClientBaseException): fallback 금지, 즉시 전파
   *   <li>Infra(Redis/CB/DistributedLockException): MySQL fallback
   *   <li>Unknown: 즉시 전파(버그 조기 발견)
   * </ul>
   *
   * <p><b>제약</b>:
   *
   ...
   *
   * <ul>
   *   <li>Before: executeWithLock(() → true) 사용 → 락 획득 후 즉시 해제됨
   *   <li>After: tryLockImmediately() 사용 → 락 획득만, 해제는 unlockInternal()에서
   * </ul>
   *
   * <p><b>제약 사항</b>: waitTime 파라미터 무시됨 (tryLockImmediately는 즉시 시도)
   *
   * @see <a
   *     href="https://github.com/redisson/redisson/wiki/8.-Distributed-locks-and-synchronizers">Redisson
   *     Lock Best Practice</a>
   */
  @Override
   ...
   *
   * <p>MySQL Named Lock은 세션 기반이므로 tryLockImmediately() 지원 불가. Redis 실패 시 DistributedLockException으로
   * 명확한 실패 전달.
   */
  private boolean handleTryLockFallback(Throwable t, String key, long leaseTime) {
    ...
    if (isInfrastructureException(cause)) {
      ...
      throw new DistributedLockException(
          "Tiered Lock 획득 실패: Redis 불가 + MySQL 세션 기반으로 fallback 불가 [key=" + key + "]", cause);
    }
  ...
  private <T> T handleFallback(
      Throwable t, String key, String op, ThrowingSupplier<T> mysqlFallback) {
    Throwable cause = ExceptionUtils.unwrapAsyncException(t);

    // InterruptedException은 Lock 도메인 예외로 정규화
    if (cause instanceof InterruptedException ie) {
      Thread.currentThread().interrupt();
      throw new DistributedLockException("락 획득/실행 중 인터럽트 [op=" + op + ", key=" + key + "]", ie);
    }
    ...
    if (isInfrastructureException(cause)) {
      ...
      if (mysqlLockStrategy == null) {
        log.error(
            ...
            cause.getMessage());
        throw new DistributedLockException(
            "락 획득/실행 실패: Redis 불가 + MySQL Fallback 비활성화 [op=" + op + ", key=" + key + "]", cause);
      }
  ...
  private void unlockMySqlIfAvailable(String key) {
    Optional.ofNullable(mysqlLockStrategy).ifPresent(strategy -> strategy.unlock(key));
  }
   ...
   *
   * <p>Coffman Condition #4 (Circular Wait)를 방지하기 위해 키를 알파벳순으로 정렬 후 순차적으로 락을 획득합니다.
   *
   * <p><b>Fallback 정책</b>:
   *
   * <ul>
   *   <li>Redis 우선 시도
   *   <li>Redis 실패 시 MySQL Fallback
   *   <li>비즈니스 예외는 Fallback 없이 즉시 전파
   * </ul>
   *
            ...
            circuitBreaker.executeCheckedSupplier(
                () ->
                    redisLockStrategy.executeWithOrderedLocks(
                        keys, totalTimeout, timeUnit, leaseTime, task)),
  ...
  private <T> T handleOrderedLockFallback(
      Throwable t, String keys, ThrowingSupplier<T> mysqlFallback) {
    Throwable cause = ExceptionUtils.unwrapAsyncException(t);

    // InterruptedException 처리
    if (cause instanceof InterruptedException ie) {
      Thread.currentThread().interrupt();
      throw new DistributedLockException("다중 락 획득 중 인터럽트 [keys=" + keys + "]", ie);
    }
    ...
    if (isInfrastructureException(cause)) {
      ...
      if (mysqlLockStrategy == null) {
        log.error(
            ...
            cause.getMessage());
        throw new DistributedLockException(
            "다중 락 획득 실패: Redis 불가 + MySQL Fallback 비활성화 [keys=" + keys + "]", cause);
      }
