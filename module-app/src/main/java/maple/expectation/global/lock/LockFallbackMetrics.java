 ...
 * <h3>목적</h3>
 *
 * <p>Redis 장애 시 MySQL Fallback 동작을 모니터링하여 시스템 회복탄력성을 가시화합니다.
 *
 * <h3>메트릭 정의</h3>
 *
 * <ul>
 *   <li>lock.redis.failure.total: Redis 락 실패 횟수
 *   <li>lock.mysql.fallback.total: MySQL Fallback 활성화 횟수
 *   <li>lock.mysql.fallback.latency: Fallback 지연 시간
 *   <li>lock.redis.unavailable.total: MySQL Fallback 불가능한 경우
 * </ul>
 *
...
public class LockFallbackMetrics {
  ...
  public void init() {
    redisFailureCounter =
        registry.counter("lock.redis.failure.total", Tags.of("layer", "tiered_lock"));

    mysqlFallbackCounter =
        registry.counter("lock.mysql.fallback.total", Tags.of("layer", "tiered_lock"));

    mysqlUnavailableCounter =
        registry.counter("lock.redis.unavailable.total", Tags.of("layer", "tiered_lock"));

    fallbackLatencyTimer =
        registry.timer("lock.mysql.fallback.latency", Tags.of("layer", "tiered_lock"));
  ...
  public void recordMysqlFallback(String lockKey, String circuitBreakerState) {
    ...
    // 상태별 메트릭
    registry
        .counter("lock.mysql.fallback.detail", Tags.of("cb_state", circuitBreakerState))
        .increment();

    log.warn(
        "[LockFallback] MySQL fallback activated - key={}, state={}", lockKey, circuitBreakerState);
  }
   ...
   *
   * <p>Redis-only 모드에서 Redis 실패 시 락 획득 불가
   *
   * @param lockKey 락 키
