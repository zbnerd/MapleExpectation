 ...
 *
 * <p>성공/실패 여부와 관계없이 반드시 실행됩니다 (finally 블록).
 *
 ...
 * <h3>⚠️ 중복 등록 금지</h3>
 *
 * <p>동일한 finalizer를 FinallyPolicy와 executeWithFinallyUnchecked에 중복 등록하면 <b>2회 실행</b>되어 예기치 않은
 * 동작(double-unlock 등)이 발생할 수 있습니다.
 *
 * <h3>권장 패턴</h3>
 *
 * <ul>
 *   <li><b>분산 락 해제</b>: {@code executeWithFinallyUnchecked} 사용 (호출별 1회 보장)
 *   <li><b>전역 로깅/메트릭</b>: {@code FinallyPolicy} 사용 (Pipeline Bean 등록)
 * </ul>
 *
 * <h3>사용 예시</h3>
 *
...
public class FinallyPolicy implements ExecutionPolicy {
   ...
   *
   * <p>cleanup 실패가 외부로 전파되어 관측 가능합니다 (try/finally 의미론).
   *
   * <ul>
   *   <li>task 성공 + cleanup 실패 → cleanup 예외가 외부로 전파됨
   *   <li>task 실패 + cleanup 실패 → cleanup 예외는 suppressed로 보존
   * </ul>
   *
   ...
   *       <ul>
   *         <li>용도: 락 해제, 커넥션 반납 등 "성공을 가장하면 안 되는" 정리 작업
   *         <li>효과: try/finally와 동일한 의미론 보장
   *       </ul>
   *   <li><b>SWALLOW</b>: cleanup 실패가 흡수되어 본 흐름 영향 없음
   *       <ul>
   *         <li>용도: 전역 메트릭/로깅 등 부가적 after hook
   *         <li>⚠️ 주의: cleanup 실패가 "성공을 가장"할 수 있으므로, 반드시 WARN 로그 등으로 가시화 필요
   *       </ul>
   * </ul>
   *
   * @param cleanupTask 반드시 실행할 정리 작업
  ...
  /** 테스트에서 policy가 보유한 action을 확인/호출하기 위한 접근자. - 외부 API로 노출되어도 의미적으로 무해한 값이며, 디버깅에도 유용하다. */
