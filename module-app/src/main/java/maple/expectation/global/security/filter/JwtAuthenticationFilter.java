 ...
 *
 * <ol>
 *   <li>Authorization 헤더에서 Bearer 토큰 추출
 *   <li>JWT 파싱 및 유효성 검증
 *   <li>Redis 세션 조회 및 fingerprint 이중 검증
 *   <li>SecurityContext에 인증 정보 저장
 ...
 *
 * <ul>
 *   <li>JWT와 Redis 세션의 fingerprint 이중 검증 (Session Hijacking 방지)
 *   <li>세션이 없으면 JWT가 유효해도 인증 실패
 * </ul>
 *
 * <p>CRITICAL (Spring Security 6.x Best Practice):
 *
 * <ul>
 *   <li>@Component 사용 금지 → CGLIB 프록시 생성 시 OncePerRequestFilter의 logger NPE 발생
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
   ...
   *
   * <p>CLAUDE.md 섹션 4 준수: Optional 체이닝으로 선언적 처리
   *
   * <p>Context7 Best Practice: SecurityContextHolder.createEmptyContext() 사용
   */
   ...
   *
   * <p>중요: SecurityContextHolder.createEmptyContext()로 새 컨텍스트 생성 - 기존 컨텍스트 재사용 시 동시성 문제 가능 - Spring
   * Security 6.x 공식 권장 패턴
   */
