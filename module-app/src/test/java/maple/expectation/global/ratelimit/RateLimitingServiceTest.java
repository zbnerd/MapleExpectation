...
class RateLimitingServiceTest {
  ...
  @Mock private IpBasedRateLimiter ipRateLimiter;

  @Mock private UserBasedRateLimiter userRateLimiter;
  ...
  void checkRateLimit_AnonymousUser_UsesIpStrategy() {
    ...
    given(ipRateLimiter.tryConsume(eq("192.168.1.100"))).willReturn(ConsumeResult.allowed(99));
  ...
  void checkRateLimit_UserStrategyDisabled_FallsBackToIp() {
    ...
    given(ipRateLimiter.isEnabled()).willReturn(true);
    given(ipRateLimiter.tryConsume(eq("192.168.1.1"))).willReturn(ConsumeResult.allowed(99));
  ...
  void checkRateLimit_WhenExceeded_ReturnsDenied() {
    ...
    given(ipRateLimiter.isEnabled()).willReturn(true);
    given(ipRateLimiter.tryConsume(eq("192.168.1.1"))).willReturn(ConsumeResult.denied(0, 45));
