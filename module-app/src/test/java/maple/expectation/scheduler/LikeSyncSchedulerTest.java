 ...
 *
 * <p>스케줄러 메서드를 직접 호출하여 서비스 호출 여부를 검증합니다.
 *
 * <h4>테스트 범위</h4>
 *
 * <ul>
 *   <li>localFlush: L1 → L2 동기화
 *   <li>globalSyncCount: L2 → L3 (count) 동기화
 *   <li>globalSyncRelation: L2 → L3 (relation) 동기화
 *   <li>Redis/In-Memory 모드 분기
 * </ul>
 */
@Tag("unit")
class LikeSyncSchedulerTest {
  ...
  class GlobalSyncCountTest {
    ...
    void whenInMemoryMode_shouldUseLockStrategy() throws Throwable {
      // given
      given(likeBufferStrategy.getType()).willReturn(LikeBufferStrategy.StrategyType.IN_MEMORY);
      given(
      ...
      // then
      verify(lockStrategy)
          .executeWithLock(eq("like-db-sync-lock"), eq(0L), eq(30L), any(ThrowingSupplier.class));
    }
    ...
    void whenRedisMode_shouldUsePartitionedFlush() throws Throwable {
      // given
      given(likeBufferStrategy.getType()).willReturn(LikeBufferStrategy.StrategyType.REDIS);
      ...
      verify(lockStrategy, never())
          .executeWithLock(anyString(), anyLong(), anyLong(), any(ThrowingSupplier.class));
    }
  }
  ...
  class GlobalSyncRelationTest {
    ...
    void shouldUseLockStrategy() throws Throwable {
      ...
      verify(lockStrategy)
          .executeWithLock(
              eq("like-relation-sync-lock"), eq(0L), eq(30L), any(ThrowingSupplier.class));
    }
