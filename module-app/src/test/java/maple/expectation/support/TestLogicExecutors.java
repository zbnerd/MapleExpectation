 ...
 *
 * <p>Eliminates ~800 lines of duplicate mock boilerplate across 35+ test files by providing
 * a simple pass-through implementation that directly executes tasks without mocking behavior.
 *
 * <h3>Usage Example</h3>
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>The passThrough() mock uses lenient() to prevent unnecessary strictness in tests
 * and delegates all method calls to actual task execution, making it ideal for testing
 * components that depend on LogicExecutor without interfering with task logic itself.
 */
public final class TestLogicExecutors {
     ...
     *
     * <p>Each method is mocked with lenient() and delegates to actual task execution,
     * making it ideal for testing components that depend on LogicExecutor without
     * interfering with task logic itself.
     *
    ...
    public static LogicExecutor passThrough() {
        ...
        Mockito.lenient().when(mock.execute(
        ...
        Mockito.lenient().when(mock.executeOrDefault(
                ArgumentMatchers.<ThrowingSupplier<Object>>any(),
        ...
        Mockito.lenient().doAnswer(invocation -> {
            ...
            task.run();
            return null;
        }).when(mock).executeVoid(
                ArgumentMatchers.<ThrowingRunnable>any(),
                ArgumentMatchers.<TaskContext>any()
        );

        // Pattern 1: Execute with finally block
        Mockito.lenient().when(mock.executeWithFinally(
                ArgumentMatchers.<ThrowingSupplier<Object>>any(),
        ...
        Mockito.lenient().when(mock.executeWithTranslation(
                ArgumentMatchers.<ThrowingSupplier<Object>>any(),
        ...
        Mockito.lenient().when(mock.executeWithFallback(
                ArgumentMatchers.<ThrowingSupplier<Object>>any(),
        ...
        Mockito.lenient().when(mock.executeOrCatch(
                ArgumentMatchers.<ThrowingSupplier<Object>>any(),
                ArgumentMatchers.<java.util.function.Function<Throwable, Object>>any(),
                ArgumentMatchers.<TaskContext>any()
        )).thenAnswer(invocation -> {
