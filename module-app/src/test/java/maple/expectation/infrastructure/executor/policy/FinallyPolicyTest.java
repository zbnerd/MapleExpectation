 ...
 *
 * <p>Codex 리뷰 기반 PROPAGATE failureMode 검증:
 *
 * <ul>
 *   <li>cleanup 실패가 관측되어야 함 (숨겨지면 안 됨)
 *   <li>task 성공 + cleanup 실패 → cleanup 예외가 외부로 전파
...
class FinallyPolicyTest {
  ...
  class SwallowModeTest {
    ...
    void swallow_taskSuccess_cleanupFails_returnsNormally() throws Throwable {
      ...
      assertThat(cleanupCount.get()).as("cleanup이 실행되어야 함 (예외가 발생해도)").isEqualTo(1);
    }
      ...
      assertThatThrownBy(
          ...
          .satisfies(
              thrown -> {
                ...
                assertThat(suppressed).as("SWALLOW 모드에서는 cleanup 예외가 suppressed에 없어야 함").isEmpty();
              });
    }
  ...
  class PipelinePropagateTest {
    ...
    @Test
    @DisplayName(
        "task RuntimeException + cleanup RuntimeException → task 예외가 primary, cleanup은 suppressed")
    void taskFails_cleanupFails_taskIsPrimary_cleanupIsSuppressed() {
      ...
      assertThatThrownBy(
          ...
          .satisfies(
              thrown -> {
                Throwable[] suppressed = thrown.getSuppressed();
                assertThat(suppressed).as("cleanup 예외가 suppressed로 보존되어야 함").hasSize(1);
    ...
    void cleanupSuccess_returnsNormally() throws Throwable {
      ...
      // then
      assertThat(result).isEqualTo("success");
      assertThat(cleanupCount.get()).as("cleanup이 정확히 1회 실행되어야 함").isEqualTo(1);
    }
    ...
    void taskFails_cleanupStillRuns() {
      ...
      assertThat(cleanupCount.get()).as("task 실패 시에도 cleanup이 정확히 1회 실행되어야 함").isEqualTo(1);
    }
