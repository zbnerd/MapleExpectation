 ...
 *
 * <p>Spring의 {@code @Order} 애노테이션은 낮은 값이 높은 우선순위를 가집니다. 즉, 100이 200보다 먼저 실행됩니다.
 *
 * <h3>실행 순서 규칙</h3>
 *
 * <ul>
 *   <li><b>BEFORE</b>: ORDER 값이 작을수록 먼저 실행 (0 → N)
 *   <li><b>AFTER</b>: entered 역순(LIFO, N → 0)
 * </ul>
 *
 * <h3>권장 순서</h3>
 *
...
public final class PolicyOrder {
   ...
   * 분산 락 정책
   *
   * <p>ORDER를 크게 두어 BEFORE에서 늦게 획득하고, AFTER에서 LIFO 특성상 빠르게 해제하여 락 점유 시간을 최소화합니다.
   */
   ...
   * 자원 정리 정책
   *
   * <p>ORDER가 큰 정책은 BEFORE에서 늦게 실행되고(entered의 뒤쪽), AFTER에서는 LIFO 특성상 먼저 실행됩니다.
   *
   * <p>즉, FINALLY를 크게 두면 AFTER 초기에 cleanup이 수행되어 리소스 해제(락/ThreadLocal 정리 등)를 빠르게 할 수 있습니다.
   */
