...
public abstract class AbstractBucket4jRateLimiter implements RateLimiter {
  ...
  public ConsumeResult tryConsume(String key) {
    ...
    TaskContext context =
        TaskContext.of("RateLimit", "Consume", getStrategyName() + ":" + maskKey(key));

    return executor.executeOrDefault(() -> doTryConsume(fullKey), handleFailure(key), context);
  }
  ...
  private ConsumeResult doTryConsume(String fullKey) {
    ...
    ConsumptionProbe probe =
        proxyManager.builder().build(fullKey, configSupplier).tryConsumeAndReturnRemaining(1);
  ...
  private ConsumeResult handleFailure(String key) {
    ...
    // Fail-Close 모드 (보안 우선)
    log.warn(
        "[RateLimit-FailClose] Redis failure, denying request: strategy={}, key={}",
        getStrategyName(),
        maskKey(key));
  ...
  private void recordMetrics(boolean consumed) {
    ...
    meterRegistry
        .counter("ratelimit.consume", "strategy", getStrategyName(), "result", result)
        .increment();
  }
