 ...
 *
 * <p>Redis Buffer 연산을 위한 Lua Script SHA 해시를 캐싱하고, NOSCRIPT 에러 발생 시 자동 재로드합니다.
 *
 * <h3>Thread Safety</h3>
 *
 ...
 *
 * <ul>
 *   <li>Green (Performance): SHA 캐싱으로 네트워크 오버헤드 최소화
 *   <li>Red (SRE): NOSCRIPT 자동 복구로 Redis 재시작 대응
 *   <li>Purple (Auditor): AtomicReference로 동시성 안전성 보장
 * </ul>
 */
@Slf4j
@Component
...
public class BufferLuaScriptProvider {
    ...
    boolean loaded =
        executor.executeOrDefault(
            () -> {
              ...
              getExpiredInflightShaRef.set(
                  script.scriptLoad(BufferLuaScripts.GET_EXPIRED_INFLIGHT));
  ...
  public String getPublishSha() {
    return publishShaRef.updateAndGet(
        current -> current != null ? current : reloadScript(BufferLuaScripts.PUBLISH, "Publish"));
  }

  public String getConsumeSha() {
    return consumeShaRef.updateAndGet(
        current -> current != null ? current : reloadScript(BufferLuaScripts.CONSUME, "Consume"));
  }

  public String getAckSha() {
    return ackShaRef.updateAndGet(
        current -> current != null ? current : reloadScript(BufferLuaScripts.ACK, "Ack"));
  }

  public String getNackToRetrySha() {
    return nackToRetryShaRef.updateAndGet(
        current ->
            current != null
                ? current
                : reloadScript(BufferLuaScripts.NACK_TO_RETRY, "NackToRetry"));
  }

  public String getNackToDlqSha() {
    return nackToDlqShaRef.updateAndGet(
        current ->
            current != null ? current : reloadScript(BufferLuaScripts.NACK_TO_DLQ, "NackToDlq"));
  }

  public String getRedriveSha() {
    return redriveShaRef.updateAndGet(
        current -> current != null ? current : reloadScript(BufferLuaScripts.REDRIVE, "Redrive"));
  }

  public String getProcessRetryQueueSha() {
    return processRetryQueueShaRef.updateAndGet(
        current ->
            current != null
                ? current
                : reloadScript(BufferLuaScripts.PROCESS_RETRY_QUEUE, "ProcessRetryQueue"));
  }

  public String getGetExpiredInflightSha() {
    return getExpiredInflightShaRef.updateAndGet(
        current ->
            current != null
                ? current
                : reloadScript(BufferLuaScripts.GET_EXPIRED_INFLIGHT, "GetExpiredInflight"));
  }

  public String getGetQueueCountsSha() {
    return getQueueCountsShaRef.updateAndGet(
        current ->
            current != null
                ? current
                : reloadScript(BufferLuaScripts.GET_QUEUE_COUNTS, "GetQueueCounts"));
  }
