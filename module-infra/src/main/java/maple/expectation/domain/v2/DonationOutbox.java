...
public class DonationOutbox {
  ...
  private static String computeContentHash(String reqId, String type, String payload) {
    ...
    byte[] hash =
        digest.digest((reqId + "|" + type + "|" + payload).getBytes(StandardCharsets.UTF_8));
  ...
  public void markFailed(String error) {
    ...
    long backoffSeconds = Math.min((long) Math.pow(2, retryCount) * 30, MAX_BACKOFF.toSeconds());
