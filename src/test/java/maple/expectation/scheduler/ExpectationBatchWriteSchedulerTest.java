 ...
 * <p>스케줄러 flush 메서드를 직접 호출하여 동작을 검증합니다.</p>
 *
 * <h4>테스트 범위</h4>
 * <ul>
 *   <li>flush: 배치 DB 동기화</li>
 *   <li>Shutdown 중 스킵</li>
 *   <li>분산 락 사용</li>
 *   <li>빈 버퍼 스킵</li>
 * </ul>
...
class ExpectationBatchWriteSchedulerTest {
    ...
    void setUp() {
        ...
        scheduler = new ExpectationBatchWriteScheduler(
    ...
    class FlushTest {
        ...
        void whenBufferHasData_shouldFlushWithLock() throws Throwable {
            ...
            given(buffer.drain(100)).willReturn(List.of(
                    new ExpectationWriteTask(1L, 1, BigDecimal.valueOf(1000),
                            BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO, LocalDateTime.now())
            ));
            ...
            verify(lockStrategy).executeWithLock(
                    eq("expectation-batch-sync-lock"),
                    eq(0L),
                    eq(10L),
        ...
        void whenFlush_shouldUpsertEachTask() throws Throwable {
            ...
            given(buffer.drain(100)).willReturn(List.of(
                    new ExpectationWriteTask(1L, 1, BigDecimal.valueOf(1000),
                            ...
                            BigDecimal.valueOf(300), BigDecimal.valueOf(400), LocalDateTime.now()),
                    new ExpectationWriteTask(2L, 2, BigDecimal.valueOf(2000),
                            BigDecimal.valueOf(500), BigDecimal.valueOf(600),
                            BigDecimal.valueOf(700), BigDecimal.valueOf(200), LocalDateTime.now())
            ));
            given(buffer.getPendingCount()).willReturn(0);
            given(lockStrategy.executeWithLock(anyString(), anyLong(), anyLong(), any(ThrowingSupplier.class)))
                    .willAnswer(invocation -> {
            ...
            verify(repository, times(2)).upsertExpectationSummary(
                    anyLong(), anyInt(), any(BigDecimal.class),
