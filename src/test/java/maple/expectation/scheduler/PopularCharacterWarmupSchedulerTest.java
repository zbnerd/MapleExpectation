 ...
 * <p>웜업 스케줄러 메서드를 직접 호출하여 동작을 검증합니다.</p>
 *
 * <h4>테스트 범위</h4>
 * <ul>
 *   <li>dailyWarmup: 매일 새벽 5시 웜업</li>
 *   <li>initialWarmup: 서버 시작 후 30초 웜업</li>
 *   <li>분산 락 사용</li>
 *   <li>인기 캐릭터 조회 및 캐시 프리로딩</li>
 * </ul>
 */
@Tag("unit")
class PopularCharacterWarmupSchedulerTest {
    ...
    void setUp() {
        ...
        scheduler = new PopularCharacterWarmupScheduler(
    ...
    class InitialWarmupTest {
        ...
        void shouldExecuteInitialWarmup() throws Throwable {
            ...
            verify(lockStrategy).executeWithLock(
                    eq("popular-warmup-lock"),
                    anyLong(), anyLong(), any(ThrowingSupplier.class)
            );
        }
    }
    ...
    class WarmupLogicTest {
        ...
        void whenCharacterFails_shouldContinueWithNext() throws Throwable {
            ...
            given(expectationService.calculateExpectation(eq("Success2"), eq(false)))
                    .willReturn(null);
            given(expectationService.calculateExpectation(eq("Success3"), eq(false)))
                    .willReturn(null);
