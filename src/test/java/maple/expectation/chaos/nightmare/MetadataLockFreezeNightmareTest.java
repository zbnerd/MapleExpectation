 ...
 * <ul>
 *   <li>🔴 Red (SRE): 장애 주입 - DDL 실행으로 Metadata Lock 대기열 생성</li>
 *   <li>🔵 Blue (Architect): 흐름 검증 - Lock 대기 체인 분석</li>
 *   <li>🟢 Green (Performance): 메트릭 검증 - 블로킹 쿼리 수, 대기 시간</li>
 *   <li>🟣 Purple (Auditor): 데이터 무결성 - DDL 완료 후 스키마 일관성</li>
 *   <li>🟡 Yellow (QA Master): 테스트 전략 - MDL 대기열 발생 시 P0 Issue 생성</li>
 * </ul>
 *
 * <h4>예상 결과: FAIL (취약점 노출)</h4>
 * <p>Production에서 DDL 실행 시 전체 쿼리가 블로킹되는 현상 재현.
 * pt-online-schema-change 또는 gh-ost 같은 Online DDL 도구 필요.</p>
 *
 * <h4>관련 CS 원리</h4>
 * <ul>
 *   <li>Metadata Lock (MDL): MySQL 5.5.3+에서 스키마 일관성 보장</li>
 *   <li>Lock Starvation: 대기 중인 DDL이 후속 쿼리를 모두 블로킹</li>
 *   <li>Online DDL: 무중단 스키마 변경 기법</li>
 *   <li>Convoy Effect: 느린 작업이 빠른 작업들을 대기시키는 현상</li>
 * </ul>
 *
 * @see <a href="https://dev.mysql.com/doc/refman/8.0/en/metadata-locking.html">MySQL Metadata Locking</a>
 */
@Slf4j
@Tag("nightmare")
@SpringBootTest
...
class MetadataLockFreezeNightmareTest extends AbstractContainerBaseTest {
    ...
    void setUp() throws Exception {
        try (Connection conn = dataSource.getConnection()) {
            ...
            // 초기 데이터 삽입
            String insertData = "INSERT INTO %s (value) VALUES ('test-data-%d')".formatted(TEST_TABLE, System.currentTimeMillis());
     ...
     * <p><b>시나리오</b>:
     * <ol>
     *   <li>Thread A: 장시간 실행되는 SELECT 쿼리 (트랜잭션 유지)</li>
     *   <li>Thread B: ALTER TABLE 실행 (MDL exclusive lock 대기)</li>
     *   <li>Thread C-N: 일반 SELECT 쿼리들 (ALTER 뒤에서 대기)</li>
     *   <li>결과: 모든 쿼리가 Thread A 완료까지 블로킹</li>
     * </ol>
     *
     * <p><b>성공 기준</b>: 블로킹된 쿼리 수 ≤ 5</p>
     * <p><b>실패 조건</b>: 블로킹된 쿼리 수 > 5 → MDL Freeze 발생</p>
     */
    @Test
    ...
    void shouldNotBlockQueries_whenDdlExecuted() throws Exception {
        ...
        Future<?> longQuery = executor.submit(() -> {
            try (Connection conn = dataSource.getConnection()) {
        ...
        Future<?> ddlQuery = executor.submit(() -> {
            try {
        ...
        log.info("├────────────────────────────────────────────────────────────┤");
        log.info("│ Normal Queries: {}                                          │", completedCount.get());
     ...
     *
     * <p>performance_schema.metadata_locks 테이블로 락 대기 상태 확인</p>
     */
    @Test
    ...
    void shouldMaintainIntegrity_afterDdlTimeout() throws Exception {
        ...
        log.info("│ Data Intact: {}                                            │", initialCount == finalCount ? "YES ✅" : "NO ❌");
        log.info("└────────────────────────────────────────────────────────────┘");
