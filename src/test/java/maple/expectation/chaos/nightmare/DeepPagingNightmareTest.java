 ...
 * <ul>
 *   <li>🔴 Red (SRE): 장애 주입 - OFFSET 1,000,000으로 극한 페이징 유발</li>
 *   <li>🔵 Blue (Architect): 흐름 검증 - Cursor-based Pagination 패턴 제안</li>
 *   <li>🟢 Green (Performance): 메트릭 검증 - 쿼리 실행 시간, Full Scan 여부</li>
 *   <li>🟣 Purple (Auditor): 데이터 무결성 - 페이징 결과 일관성</li>
 *   <li>🟡 Yellow (QA Master): 테스트 전략 - 응답 시간 > 5초 시 P0 Issue</li>
 * </ul>
 *
 * <h4>예상 결과: FAIL (성능 문제)</h4>
 * <p>OFFSET이 클수록 MySQL은 그만큼의 행을 스캔한 후 버림.
 * OFFSET 1,000,000 + LIMIT 10은 1,000,010행을 읽어야 함.</p>
 *
 * <h4>관련 CS 원리</h4>
 * <ul>
 *   <li>Offset Pagination: LIMIT + OFFSET의 O(n) 성능</li>
 *   <li>Cursor-based Pagination: WHERE id > last_id의 O(1) 성능</li>
 *   <li>Keyset Pagination: 인덱스 활용 빠른 페이징</li>
 *   <li>Full Table Scan: OFFSET이 인덱스를 무력화</li>
 *   <li>Database Buffer Pool: 불필요한 페이지 로드</li>
 * </ul>
 *
...
class DeepPagingNightmareTest extends AbstractContainerBaseTest {
     ...
     * <p><b>시나리오</b>:
     * <ol>
     *   <li>OFFSET 0 (첫 페이지) 쿼리 실행 시간 측정</li>
     *   <li>OFFSET 9990 (마지막 페이지) 쿼리 실행 시간 측정</li>
     *   <li>성능 차이 분석</li>
     * </ol>
     *
     * <p><b>성공 기준</b>: 마지막 페이지도 100ms 이내</p>
     * <p><b>실패 조건</b>: 성능 저하가 심각함 (10배 이상)</p>
     */
    @Test
    ...
    void shouldMeasureDeepOffsetPerformanceDegradation() throws Exception {
        ...
        log.info("├────────────────────────────────────────────────────────────┤");
        log.info("│ Degradation Ratio: {}x                                     │", String.format("%.2f", degradationRatio));
        log.info("├────────────────────────────────────────────────────────────┤");
     ...
     *
     * <p>WHERE id > last_id 방식으로 페이징 성능 개선</p>
     */
    @Test
    ...
    void shouldCompareOffsetVsCursorPagination() throws Exception {
        ...
        log.info("├────────────────────────────────────────────────────────────┤");
        log.info("│ Performance Improvement: {}%                               │", String.format("%.1f", improvement));
        log.info("├────────────────────────────────────────────────────────────┤");
    ...
    void shouldAnalyzeQueryPlanWithExplain() throws Exception {
        ...
        try (Connection conn = dataSource.getConnection()) {
            // OFFSET 방식 EXPLAIN
            String offsetQuery = "EXPLAIN SELECT * FROM " + TEST_TABLE + " ORDER BY id LIMIT 10 OFFSET 9990";
            log.info("┌────────────────────────────────────────────────────────────┐");
            ...
            try (PreparedStatement ps = conn.prepareStatement(offsetQuery);
                ...
                while (rs.next()) {
                    ...
                    log.info("│ possible_keys: {}                                        │", rs.getString("possible_keys"));
                    log.info("│ key: {}                                                  │", rs.getString("key"));
                    log.info("│ rows: {}                                                 │", rs.getString("rows"));
                    log.info("│ Extra: {}                                                │", rs.getString("Extra"));
                }
            }

            // CURSOR 방식 EXPLAIN
            String cursorQuery = "EXPLAIN SELECT * FROM " + TEST_TABLE + " WHERE id > 9990 ORDER BY id LIMIT 10";
            log.info("├────────────────────────────────────────────────────────────┤");
