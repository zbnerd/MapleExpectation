 ...
 *   <li>🔵 Blue (Architect): 흐름 검증 - AbortPolicy vs CallerRunsPolicy 동작</li>
 *   <li>🟢 Green (Performance): 메트릭 검증 - 작업 제출 시간, 블로킹 여부</li>
 *   <li>🟣 Purple (Auditor): 데이터 무결성 - 작업 거부 시 Future 완료 보장</li>
 *   <li>🟡 Yellow (QA Master): 테스트 전략 - 실제 운영 Executor 설정 검증</li>
 * </ul>
 *
 * <h4>테스트 목적</h4>
 * <p>실제 운영 환경의 {@code expectationComputeExecutor}와 {@code alertTaskExecutor}가
 * Thread Pool 포화 시 CallerRunsPolicy로 인한 메인 스레드 블로킹 없이
 * 적절히 작업을 거부(AbortPolicy)하는지 검증한다.</p>
 *
 * <h4>운영 Executor 설정 (ExecutorConfig.java)</h4>
 * <ul>
 *   <li><b>expectationComputeExecutor</b>: core=4, max=8, queue=200, EXPECTATION_ABORT_POLICY</li>
 *   <li><b>alertTaskExecutor</b>: core=2, max=4, queue=200, LOGGING_ABORT_POLICY</li>
 * </ul>
 *
 * <h4>예상 결과: PASS</h4>
 * <p>운영 환경은 AbortPolicy 기반 정책을 사용하므로 메인 스레드 블로킹이 발생하지 않음.
 * 큐 포화 시 RejectedExecutionException 발생하여 빠른 실패 보장.</p>
 *
 * <h4>관련 CS 원리</h4>
 * <ul>
 ...
 *       <ul>
 *         <li>CallerRunsPolicy: 호출자 스레드에서 실행 (블로킹) - <b>사용 금지</b></li>
 *         <li>AbortPolicy: RejectedExecutionException 발생 (빠른 실패)</li>
 *       </ul>
 *   </li>
...
class ThreadPoolExhaustionNightmareTest extends IntegrationTestSupport {
     ...
     */
    @Autowired
    @Qualifier("expectationComputeExecutor")
    private Executor expectationComputeExecutor;
     ...
     * <p><b>시나리오</b>:
     * <ol>
     *   <li>Pool Size(4~8) + Queue Size(200) = 최대 208개 동시 처리</li>
     *   <li>250개 작업 제출 (용량 초과)</li>
     *   <li>큐 포화 시 EXPECTATION_ABORT_POLICY 발동 → RejectedExecutionException</li>
     *   <li>메인 스레드 블로킹 없이 빠르게 거부</li>
     * </ol>
     *
     * <p><b>성공 기준</b>: 작업 제출 시간 < 500ms (비블로킹)</p>
     * <p><b>검증 포인트</b>: CallerRunsPolicy와 달리 메인 스레드가 블로킹되지 않음</p>
     */
    @Test
    ...
    void shouldNotBlockMainThread_withExpectationComputeExecutor() throws Exception {
        ...
        log.info("[Red] Pool Config: core={}, max={}, queue={}", corePoolSize, maxPoolSize, queueCapacity);
        ...
        for (int i = 0; i < taskCount; i++) {
            ...
            try {
                executor.execute(() -> {
                    ...
                    if (currentThread.equals(mainThreadName) || !currentThread.startsWith("expectation-")) {
                        callerRunsCount.incrementAndGet();
                        log.warn("[Red] Task {}: CallerRunsPolicy detected! (Thread: {})",
            ...
            if (submitTime > 100) {
                log.warn("[Red] Task {}: Submit blocked for {}ms! (CallerRunsPolicy 의심)", taskId, submitTime);
            }
        }
        ...
        log.info("├────────────────────────────────────────────────────────────┤");
        log.info("│ Pool Config: core={}, max={}, queue={}                     │",
     ...
     *
     * <p>Alert 전용 Executor도 AbortPolicy 기반으로 메인 스레드 블로킹을 방지하는지 검증</p>
     */
    @Test
    ...
    void shouldNotBlockMainThread_withAlertTaskExecutor() throws Exception {
        ...
        log.info("[Blue] Pool Config: core={}, max={}, queue={}", corePoolSize, maxPoolSize, queueCapacity);
            ...
            try {
                executor.execute(() -> {
                    ...
                    if (currentThread.equals(mainThreadName) || !currentThread.startsWith("alert-")) {
                        callerRunsCount.incrementAndGet();
                        log.warn("[Blue] Task {}: CallerRunsPolicy detected! (Thread: {})",
                                taskId, currentThread);
                    }
        ...
        // 검증: 비블로킹 확인
        assertThat(totalSubmitTime)
                .as("[Nightmare] 전체 제출이 빠르게 완료되어야 함")
                .isLessThan(5000L);
    }
     ...
     *
     * <p>실제 운영 Executor의 Thread Pool 메트릭을 실시간으로 모니터링하여
     * 포화 상태 진입 및 회복 과정을 검증</p>
     */
    @Test
    ...
    void shouldAnalyzeThreadPoolMetrics_withActualExecutor() throws Exception {
        ...
        assertThat(maxActiveCount.get())
                .as("Pool이 실제로 스케일링되어야 함")
                .isGreaterThanOrEqualTo(corePoolSize);
    }

    /**
     * 🟣 Purple's Test 4: RejectedExecutionException 발생 시 Future 완료 보장
     *
     * <p>운영 환경의 AbortPolicy가 CompletableFuture의 exceptionally 완료를 보장하는지 검증.
     * DiscardPolicy와 달리 Future가 영원히 pending되지 않음을 확인.</p>
     */
    @Test
    ...
    void shouldCompleteFutureExceptionally_whenRejected() throws Exception {
        ...
        log.info("[Purple] Submitting {} tasks to pool (capacity: {})",
                taskCount, maxPoolSize + queueCapacity);
        ...
        log.info("│ Completed Normally: {}                                     │", completedNormally.get());
        log.info("│ Completed Exceptionally: {}                                │", completedExceptionally.get());
     ...
     *
     * <p>테스트 전용 Executor를 생성하여 두 정책의 차이를 명확히 비교.
     * 이 테스트는 운영 환경이 아닌 교육 목적으로만 사용.</p>
     */
    @Test
    ...
    void shouldDemonstratePolicyDifference_forEducationalPurpose() throws Exception {
        ...
        log.info("│   - Tasks in Main Thread: {}                               │", callerRunsInMain.get());
