 ...
 *
 * <ul>
 *   <li>Red (SRE): 장애 주입 - 외부 API 10초 지연으로 커넥션 점유 유발
 *   <li>Blue (Architect): 흐름 검증 - 트랜잭션 경계 분리 패턴 적용
 *   <li>Green (Performance): 메트릭 검증 - HikariCP Pool 사용률, Connection Wait Time
 *   <li>Purple (Auditor): 데이터 무결성 - 트랜잭션 롤백 시 일관성 유지
 *   <li>Yellow (QA Master): 테스트 전략 - Pool 고갈 방지 검증
 * </ul>
 *
 * <h4>예상 결과: PASS (Issue #226 수정 후)</h4>
 *
 * <p>GameCharacterService.createNewCharacter()가 트랜잭션 경계 분리 패턴을 적용하여 API 호출은 트랜잭션 밖에서, DB 저장은 트랜잭션
 * 안에서 수행합니다. 커넥션 점유 시간: 28초 → ~100ms
 *
 * <h4>해결된 취약점</h4>
 *
 * <p>GameCharacterService.java, OcidResolver.java - 트랜잭션 경계 분리
 *
 * <h4>관련 CS 원리</h4>
 *
 * <ul>
 *   <li>Transaction Scope Minimization: 트랜잭션 경계를 DB 작업에만 한정
 *   <li>Connection Pool Efficiency: 외부 I/O를 트랜잭션 밖에서 처리
 *   <li>Separation of Concerns: API 호출과 DB 작업 분리
 *   <li>Little's Law: L = λW (짧은 점유 시간 = 적은 대기 커넥션)
 * </ul>
 *
...
class ConnectionVampireNightmareTest extends AbstractContainerBaseTest {
  ...
  @MockBean private NexonApiClient nexonApiClient;
   ...
   *
   * <ol>
   *   <li>외부 API에 5초 지연 주입 (Mock)
   *   <li>동시에 20개 요청 발생 (Pool 크기의 2배)
   *   <li>트랜잭션 경계 분리로 API 호출은 트랜잭션 밖에서 수행
   *   <li>Pool 고갈 없이 모든 요청 성공 예상
   * </ol>
   *
   * <p><b>예상 성공</b>: 트랜잭션 경계 분리로 Connection Pool 고갈 방지
   *
   * <p><b>Issue #226 수정 사항</b>:
   *
   * <ul>
   *   <li>Before: @Transactional 범위 내 .join() 호출 → 최대 28초 DB Connection 점유
   *   <li>After: API 호출은 트랜잭션 밖, DB 작업만 트랜잭션 안 → ~100ms 점유
   * </ul>
   */
  @Test
  ...
  void shouldNotExhaustConnectionPool_whenExternalApiDelayed_afterFix() throws Exception {
    ...
    // 메트릭 계산
    long avgAcquireTime =
        connectionAcquireTimes.stream().mapToLong(Long::longValue).sum()
            / Math.max(connectionAcquireTimes.size(), 1);
    long maxAcquireTime =
        connectionAcquireTimes.stream().mapToLong(Long::longValue).max().orElse(0);
   ...
   *
   * <p><b>시나리오</b>: API 지연 시간만큼 Connection이 점유되는지 검증
   */
  @Test
  ...
  void shouldHoldConnectionDuringExternalCall() throws Exception {
    ...
    when(nexonApiClient.getOcidByCharacterName(anyString()))
        .thenAnswer(
            invocation -> {
              Thread.sleep(apiDelayMs);
              return CompletableFuture.completedFuture(new CharacterOcidResponse("test-ocid"));
            });
   ...
   *
   * <p><b>시나리오</b>: 동시 요청 시 Active Connections 수 측정
   */
  @Test
  ...
  void shouldRecoverAfterPoolExhaustion() throws Exception {
    ...
    try (Connection conn = dataSource.getConnection()) {
      ...
      // Then: 빠른 복구 (5초 이내)
      assertThat(recoveryTime).as("Pool 복구 후 커넥션 획득이 빨라야 함").isLessThan(5000);
    }
  }

  private void printConnectionVampireResults(
      int concurrentRequests, boolean completed, long avgAcquireTime, long maxAcquireTime) {
    log.info("==========================================================");
