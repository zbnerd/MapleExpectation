 ...
 * <ul>
 *   <li>🔴 Red (SRE): 장애 주입 - 비동기 호출로 MDC 컨텍스트 전파 검증</li>
 *   <li>🔵 Blue (Architect): 흐름 검증 - TaskDecorator 전파 경로 분석</li>
 *   <li>🟢 Green (Performance): 메트릭 검증 - 컨텍스트 전파 성공률 측정</li>
 *   <li>🟣 Purple (Auditor): 데이터 무결성 - 감사 로그 무결성 검증</li>
 *   <li>🟡 Yellow (QA Master): 테스트 전략 - 실제 Executor로 전파 검증</li>
 * </ul>
 *
 * <h4>예상 결과: PASS (TaskDecorator 적용됨)</h4>
 * <p>{@code ExecutorConfig.contextPropagatingDecorator()}가 MDC를 비동기 스레드로 전파합니다.</p>
 *
 * <h4>테스트 목적</h4>
 * <p>프로젝트의 실제 Executor({@code alertTaskExecutor}, {@code expectationComputeExecutor})가
 * {@link MDCFilter#REQUEST_ID_KEY}를 올바르게 전파하는지 검증합니다.</p>
 *
 * <h4>관련 CS 원리</h4>
 * <ul>
 *   <li>ThreadLocal: 스레드별 격리된 변수 저장소</li>
 *   <li>MDC (Mapped Diagnostic Context): 로깅 컨텍스트 전파</li>
 *   <li>TaskDecorator: Spring의 비동기 작업 데코레이터 패턴</li>
 *   <li>Context Propagation: 분산 추적의 핵심 메커니즘</li>
 * </ul>
 *
...
class AsyncContextLossNightmareTest extends IntegrationTestSupport {
     ...
     * <p><b>시나리오</b>:
     * <ol>
     *   <li>메인 스레드에 MDC 설정 (requestId - MDCFilter와 동일 키)</li>
     *   <li>실제 alertTaskExecutor로 비동기 작업 제출</li>
     *   <li>비동기 스레드에서 MDC 값 확인</li>
     *   <li>결과: TaskDecorator가 MDC를 전파하여 값 유지</li>
     * </ol>
     *
     * <p><b>성공 기준</b>: 비동기 스레드에서 MDC 컨텍스트 유지</p>
     */
    @Test
    ...
    void shouldPropagateMdcContext_withAlertTaskExecutor() throws Exception {
        ...
        log.info("│ Received requestId: {}                                     ",
                asyncRequestId.get() != null ? asyncRequestId.get() : "NULL ❌");
        log.info("│ Context Propagation: {}                                    ",
     ...
     *
     * <p>계산 전용 Executor에서도 MDC가 올바르게 전파되는지 검증</p>
     */
    @Test
     ...
     *
     * <p>실제 Executor로 여러 번 비동기 호출하여 전파 성공률 100% 확인</p>
     */
    @Test
    ...
    void shouldMaintain100PercentPropagationRate_over100Calls() throws Exception {
            ...
            CompletableFuture.runAsync(() -> {
                try {
                    ...
                    if (expectedId.equals(asyncRequestId)) {
                        ...
                        log.warn("[Green] Propagation failed - expected: {}, actual: {}",
                                expectedId, asyncRequestId);
                    }
                } finally {
                    latch.countDown();
                }
        ...
        if (successRate == 100.0) {
            ...
            log.info("│ ⚠️ {} % propagation failure detected                       ", String.format("%.1f", 100 - successRate));
        }
        log.info("└────────────────────────────────────────────────────────────┘");

        assertThat(successRate)
                .as("[Nightmare] MDC propagation rate should be 100%")
                .isEqualTo(100.0);
    }
     ...
     *
     * <p>CompletableFuture 체인 (thenRunAsync → thenRunAsync)에서
     * 각 단계마다 MDC가 올바르게 전파되는지 검증</p>
     *
     * <h4>🔴 알려진 취약점</h4>
     * <p>thenRunAsync() 체인에서 MDC가 손실되면 이 테스트가 FAIL합니다.
     * 실패 시 다음 해결책을 고려해야 합니다:</p>
     * <ul>
     *   <li>모든 작업을 독립적인 runAsync()로 실행</li>
     *   <li>명시적 MDC 전달 (thenApplyAsync에서 Context 객체 사용)</li>
     *   <li>Micrometer Context Propagation 라이브러리 도입</li>
     * </ul>
     */
    @Test
     ...
     *
     * <p>프로젝트 Executor와 달리 TaskDecorator가 없는 일반 ExecutorService는
     * MDC를 전파하지 못한다는 것을 대조군으로 검증</p>
     */
    @Test
    ...
    void shouldLoseMdcContext_withoutTaskDecorator() throws Exception {
        ...
        Future<?> future = plainExecutor.submit(() -> {
            asyncRequestId.set(MDC.get(MDCFilter.REQUEST_ID_KEY));
     ...
     *
     * <p>실제 Executor가 올바른 스레드 풀에서 실행되는지 확인</p>
     */
    @Test
