 ...
 * <ul>
 *   <li>🔴 Red (SRE): 장애 주입 - this.method() 호출로 @Transactional 우회 유발</li>
 *   <li>🔵 Blue (Architect): 흐름 검증 - OcidResolver의 Self-Invocation 경로 분석</li>
 *   <li>🟢 Green (Performance): 메트릭 검증 - 트랜잭션 미적용 시 성능 영향</li>
 *   <li>🟣 Purple (Auditor): 데이터 무결성 - 트랜잭션 없이 DB 작업 시 위험</li>
 *   <li>🟡 Yellow (QA Master): 테스트 전략 - 실제 서비스 코드의 Self-Invocation 검증</li>
 * </ul>
 *
 * <h4>예상 결과: CONDITIONAL (코드 구조에 따라)</h4>
 * <p>OcidResolver.resolve() → createAndGetOcid() → createNewCharacter() 호출 시
 * createNewCharacter()의 @Transactional(propagation = REQUIRES_NEW)가 동작하지 않을 수 있음.</p>
 *
 * <h4>테스트 대상</h4>
 * <ul>
 *   <li>OcidResolver.resolve() - 내부적으로 createNewCharacter() 호출</li>
 *   <li>OcidResolver.createNewCharacter() - @Transactional(propagation = REQUIRES_NEW)</li>
 *   <li>GameCharacterService.createNewCharacter() - @Transactional(propagation = REQUIRES_NEW) + @ObservedTransaction</li>
 * </ul>
 *
 * <h4>관련 CS 원리</h4>
 * <ul>
 *   <li>Proxy Pattern: Spring AOP의 JDK Dynamic Proxy / CGLIB</li>
 *   <li>Self-Invocation: 프록시를 거치지 않는 내부 호출</li>
 *   <li>Transaction Propagation: REQUIRES_NEW가 동작하지 않으면 기존 TX에 참여</li>
 * </ul>
 *
...
class SelfInvocationNightmareTest extends AbstractContainerBaseTest {
     ...
     * <p><b>시나리오</b>:
     * <ol>
     *   <li>OcidResolver.resolve(userIgn) 호출</li>
     *   <li>DB에 없으면 createAndGetOcid() 호출</li>
     *   <li>createAndGetOcid()에서 this.createNewCharacter() 호출</li>
     *   <li>createNewCharacter()의 @Transactional(REQUIRES_NEW) 동작 여부 확인</li>
     * </ol>
     *
     * <p><b>문제 분석</b>: this.createNewCharacter() 호출 시 프록시를 거치지 않으므로
     * @Transactional 어노테이션이 적용되지 않을 수 있음</p>
     */
    @Test
     ...
     *
     * <p>외부에서 직접 createNewCharacter() 호출 vs 내부에서 호출 비교</p>
     */
    @Test
     ...
     *
     * <p>외부에서 직접 호출하면 @Transactional이 정상 적용됨을 검증</p>
     */
    @Test
     ...
     *
     * <p>프로젝트 내 Self-Invocation 가능성이 있는 코드 패턴 분석</p>
     */
    @Test
     ...
     *
     * <p>OcidResolver.resolveCharacter()에서도 동일한 Self-Invocation 패턴 존재</p>
     */
    @Test
