...
import maple.expectation.global.lock.MySqlNamedLockStrategy;
 ...
 * <ul>
 *   <li>🔴 Red (SRE): 장애 주입 - 역순 락 획득으로 순환 대기 유발</li>
 *   <li>🔵 Blue (Architect): 흐름 검증 - Lock Ordering 정책 검증</li>
 *   <li>🟢 Green (Performance): 메트릭 검증 - Deadlock 탐지 시간, 타임아웃</li>
 *   <li>🟣 Purple (Auditor): 데이터 무결성 - Deadlock 해제 후 상태 일관성</li>
 *   <li>🟡 Yellow (QA Master): 테스트 전략 - Deadlock 발생 시 P0 Issue 생성</li>
 * </ul>
 *
 * <h4>예상 결과: FAIL (취약점 노출)</h4>
 * <p>MySqlNamedLockStrategy는 Lock Ordering을 강제하지 않아
 * 역순 락 획득 시 Deadlock 발생 가능.</p>
 *
 * <h4>관련 CS 원리</h4>
 * <ul>
 *   <li>Coffman Conditions (4가지 교착 조건):
 *       <ol>
 *         <li>Mutual Exclusion (상호 배제) - Named Lock은 배타적</li>
 *         <li>Hold and Wait (점유 대기) - 락 보유 중 다른 락 대기</li>
 *         <li>No Preemption (비선점) - GET_LOCK은 강제 해제 불가</li>
 *         <li>Circular Wait (순환 대기) ← 이 테스트의 공격 벡터</li>
 *       </ol>
 *   </li>
 *   <li>Lock Ordering: 자원 획득 순서 고정으로 순환 대기 방지</li>
 *   <li>MySQL GET_LOCK: 세션 기반 분산 락 (연결당 1개만 보유 가능)</li>
 *   <li>Timeout-based Resolution: 대기 시간 초과로 deadlock 해소</li>
 * </ul>
 *
...
class CircularLockDeadlockNightmareTest extends AbstractContainerBaseTest {
     ...
     * <p><b>시나리오</b>:
     * <ol>
     *   <li>Thread 1: LOCK_A 획득 → LOCK_B 획득 시도</li>
     *   <li>Thread 2: LOCK_B 획득 → LOCK_A 획득 시도 (역순)</li>
     *   <li>CyclicBarrier로 동시 시작 보장</li>
     *   <li>Deadlock 또는 Timeout 발생 확인</li>
     * </ol>
     *
     * <p><b>성공 기준</b>: Deadlock/Timeout 0건</p>
     * <p><b>실패 조건</b>: Deadlock/Timeout ≥ 1건 → Lock Ordering 필요</p>
     */
    @Test
    ...
    void shouldNotDeadlock_withReverseLockOrdering() throws Exception {
        ...
        executor.submit(() -> {
            try {
                ...
                try {
                    lockStrategy.executeWithLock(LOCK_B, 5000, 10000, () -> {
                        log.info("[Thread-2] Acquired {}, now trying {}", LOCK_B, LOCK_A);
                        Thread.sleep(100); // Brief delay to increase deadlock probability

                        try {
                            return lockStrategy.executeWithLock(LOCK_A, 5000, 10000, () -> {
     ...
     *
     * <p>양 스레드가 동일한 순서(LOCK_A → LOCK_B)로 락을 획득하면
     * Deadlock이 발생하지 않음을 검증</p>
     */
    @Test
        ...
        for (int i = 0; i < 2; i++) {
            ...
            executor.submit(() -> {
                try {
                    ...
                    try {
                        lockStrategy.executeWithLock(LOCK_A, 10000, 10000, () -> {
                            log.info("[Thread-{}] Acquired {}", threadId, LOCK_A);

                            try {
                                return lockStrategy.executeWithLock(LOCK_B, 10000, 10000, () -> {
    ...
    void shouldMeasureDeadlockProbability_over10Iterations() throws Exception {
        ...
        for (int iter = 0; iter < iterations; iter++) {
            ...
            executor.submit(() -> {
                try {
                    barrier.await(5, TimeUnit.SECONDS);
                    try {
                        lockStrategy.executeWithLock(LOCK_A, 2000, 3000, () -> {
                            Thread.sleep(50);
                            try {
                                return lockStrategy.executeWithLock(LOCK_B, 2000, 3000, () -> {
                                    Thread.sleep(50);
                                    return "ok";
                                });
                            } catch (Throwable t) {
            ...
            executor.submit(() -> {
                try {
                    barrier.await(5, TimeUnit.SECONDS);
                    try {
                        lockStrategy.executeWithLock(LOCK_B, 2000, 3000, () -> {
                            Thread.sleep(50);
                            try {
                                return lockStrategy.executeWithLock(LOCK_A, 2000, 3000, () -> {
                                    Thread.sleep(50);
                                    return "ok";
                                });
                            } catch (Throwable t) {
            ...
            log.info("[Green] Iteration {}: success={}, deadlock={}, timeout={}",
                    iter + 1, successCount.get(), deadlockCount.get(), timeoutCount.get());
        ...
        log.info("│ Timeout Rate: {} %                                        │", String.format("%.1f", timeoutRate));
        log.info("└────────────────────────────────────────────────────────────┘");
    ...
    private void handleLockException(Exception e, String threadName,
        ...
        } else if (msg.contains("timeout") || msg.contains("획득 실패") || msg.contains("could not acquire")) {
