 ...
 * <ul>
 *   <li>🔴 Red (SRE): 복합 장애 주입 - N19 + Redis/DB/Process 장애</li>
 *   <li>🔵 Blue (Architect): 회복 흐름 검증 - Fallback, Rollback, Idempotent</li>
 *   <li>🟢 Green (Performance): 복구 속도 메트릭 - Recovery time, throughput degradation</li>
 *   <li>🟣 Purple (Auditor): 데이터 무결성 - 유실 0건, 중복 0건 검증</li>
 *   <li>🟡 Yellow (QA Master): 테스트 전략 - 3가지 복합 시나리오 검증</li>
 * </ul>
 *
 * <h4>테스트 목적</h4>
 * <p>N19 Outbox Replay 테스트는 단일 장애(API)만 검증했습니다. 이 테스트는
 * <b>복합 장애(Compound Failures)</b> 시나리오로 시스템의 회복 탄력성을 검증합니다.</p>
 *
 * <h4>복합 장애 시나리오</h4>
 * <ol>
 *   <li><b>CF-1: N19 + Redis Timeout</b> - Replay 중 Redis 장애 → Cache fallback</li>
 *   <li><b>CF-2: N19 + DB Failover</b> - Replay 중 DB restart → Transaction rollback</li>
 *   <li><b>CF-3: N19 + Process Kill</b> - Replay 중 Process kill → Orphaned record recovery</li>
 * </ol>
 *
 * <h4>예상 결과: CONDITIONAL PASS</h4>
 * <p>모든 시나리오에서 데이터 유실 0건, 100% 복구, DLQ < 0.1% 달성.</p>
 *
...
class NexonApiOutboxMultiFailureNightmareTest extends IntegrationTestSupport {
     ...
     *   <li>10K Outbox 적재</li>
     *   <li>Replay 시작</li>
     *   <li>Replay 중 Redis timeout 발생</li>
     *   <li>Cache fallback → DB 직접 조회</li>
     *   <li>Replay 계속 → 100% 완료</li>
     * </ol>
     *
     * <p><b>성공 기준</b>: 데이터 유실 0건, 100% 완료, Cache fallback 작동</p>
     */
    @Test
    ...
    void shouldRecoverAfterRedisTimeout() throws Exception {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            if (i % 5 == 0) {
                log.info("[CF-1] Progress: {}/{} completed ({:.1f}%)",
                        completed, totalEntries, progress);
            }
        }
        ...
        Awaitility.await()
                ...
                .untilAsserted(() -> {
                    ...
                    log.info("[CF-1] Progress: {}/{} completed, {} DLQ",
        ...
        assertThat(totalAccounted)
                .as("[CF-1] 데이터 유실은 0건이어야 함")
                .isEqualTo(totalEntries);

        assertThat(finalDlq)
                .as("[CF-1] DLQ는 0.1% 미만이어야 함")
     ...
     *   <li>10K Outbox 적재</li>
     *   <li>Replay 시작</li>
     *   <li>Replay 중 DB Connection timeout 발생</li>
     *   <li>Transaction rollback → Connection pool 재연결</li>
     *   <li>Idempotent replay → 100% 완료</li>
     * </ol>
     *
     * <p><b>성공 기준</b>: 데이터 유실 0건, 중복 0건, 100% 완료</p>
     */
    @Test
    ...
    void shouldRecoverAfterDbFailover() throws Exception {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            if (i % 5 == 0) {
                double progress = (completed * 100.0) / totalEntries;
                log.info("[CF-2] Progress: {}/{} completed ({:.1f}%)",
                        completed, totalEntries, progress);
            }
        ...
        Awaitility.await()
                ...
                .untilAsserted(() -> {
                    ...
                    log.info("[CF-2] Progress: {}/{} completed, {} DLQ",
                            completed, totalEntries, deadLetter);
        ...
        if (isZeroDataLoss && isDlqAcceptable && isHighCompletionRate && noDuplicates) {
            ...
            log.info("│ DLQ OK: {}                                                ", isDlqAcceptable ? "✅" : "❌");
            log.info("│ High Completion: {}                                       ", isHighCompletionRate ? "✅" : "❌");
            log.info("│ No Duplicates: {}                                         ", noDuplicates ? "✅" : "❌");
        }

        log.info("└────────────────────────────────────────────────────────────┘");

        assertThat(totalAccounted)
                .as("[CF-2] 데이터 유실은 0건이어야 함")
                .isEqualTo(totalEntries);

        assertThat(finalDlq)
                .as("[CF-2] DLQ는 0.1% 미만이어야 함")
                .isLessThan((long) (totalEntries * 0.001));
    }
     ...
     * <ol>
     *   <li>10K Outbox 적재</li>
     *   <li>Replay 시작 (50% 진행)</li>
     *   <li>Process 강제 종료 (시뮬레이션)</li>
     *   <li>Process 재시작 → Orphaned record 복구</li>
     *   <li>Idempotent replay → 100% 완료</li>
     * </ol>
     *
     * <p><b>성공 기준</b>: 데이터 유실 0건, 중복 0건, Orphaned record 복구</p>
     *
     * <p><b>참고</b>: 실제 Process Kill은 테스트 환경에서 어렵기 때문에
     * Orphaned record 상태 복구 메커니즘을 검증합니다.</p>
     */
    @Test
    ...
    void shouldRecoverAfterProcessKill() throws Exception {
        ...
        Awaitility.await()
                ...
                .untilAsserted(() -> {
                    ...
                    log.info("[CF-3] Progress: {}/{} completed, {} DLQ",
                            completed, totalEntries, deadLetter);
        ...
        if (isZeroDataLoss && isDlqAcceptable && isHighCompletionRate && noOrphanedRecords && noDuplicates) {
            ...
            log.info("│ DLQ OK: {}                                                ", isDlqAcceptable ? "✅" : "❌");
            log.info("│ High Completion: {}                                       ", isHighCompletionRate ? "✅" : "❌");
            log.info("│ No Orphaned: {}                                           ", noOrphanedRecords ? "✅" : "❌");
            log.info("│ No Duplicates: {}                                         ", noDuplicates ? "✅" : "❌");
        }
        ...
        assertThat(totalAccounted)
                .as("[CF-3] 데이터 유실은 0건이어야 함")
                .isEqualTo(totalEntries);

        assertThat(finalDlq)
                .as("[CF-3] DLQ는 0.1% 미만이어야 함")
                .isLessThan((long) (totalEntries * 0.001));

        assertThat(finalProcessing)
                .as("[CF-3] Orphaned record는 없어야 함")
                .isEqualTo(0);
    }
