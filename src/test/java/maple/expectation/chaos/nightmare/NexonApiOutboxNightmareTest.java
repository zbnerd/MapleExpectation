 ...
 * <ul>
 *   <li>🔴 Red (SRE): 장애 주입 - 6시간 Nexon API 503 장애 시뮬레이션</li>
 *   <li>🔵 Blue (Architect): 흐름 검증 - Outbox 적재 → Processor Replay → 완료</li>
 *   <li>🟢 Green (Performance): 메트릭 검증 - 100K건 처리 속도, 처리량(rows/sec)</li>
 *   <li>🟣 Purple (Auditor): 데이터 무결성 - 유실 0건, DLQ < 0.1% 검증</li>
 *   <li>🟡 Yellow (QA Master): 테스트 전략 - 가상 시간으로 6시간 압축 시뮬레이션</li>
 * </ul>
 *
 * <h4>테스트 목적</h4>
 * <p>Nexon API가 6시간 장애 발생 시, Outbox Pattern을 통해 데이터 유실 없이
 * 장애 복구 후 자동으로 재처리되는지 검증한다.</p>
 *
 * <h4>N19 시나리오</h4>
 * <ol>
 *   <li><b>Setup Phase</b>: 100K Outbox 항목 생성</li>
 *   <li><b>Outage Phase</b>: NexonApiClient가 503 반환 (6시간 시뮬레이션)</li>
 *   <li><b>Recovery Phase</b>: API 복구, Processor가 자동 재시도</li>
 *   <li><b>Validation Phase</b>: 100% 완료, DLQ < 0.1%, 데이터 유실 0건</li>
 * </ol>
 *
 * <h4>가상 시간 전략</h4>
 * <p>실제 6시간 대신 1분 = 1시간 가속 시뮬레이션으로 테스트 속도 최적화.</p>
 *
 * <h4>예상 결과: CONDITIONAL PASS</h4>
 * <p>OutboxProcessor가 정상 작동하면 100K 건 전체 완료. DLQ는 없거나 0.1% 미만.</p>
 *
 * <h4>관련 CS 원리</h4>
 * <ul>
 *   <li>Transactional Outbox Pattern: 분산 트랜잭션 대안</li>
 *   <li>Event Sourcing: 이벤트 재생 가능</li>
...
class NexonApiOutboxNightmareTest extends IntegrationTestSupport {
    ...
    @Autowired
    private NexonApiOutboxRepository outboxRepository;

    @Autowired
    private NexonApiOutboxMetrics outboxMetrics;

    @MockitoBean(name = "nexonApiClient")
    private NexonApiClient nexonApiClient;

    @MockitoBean
    private NexonApiRetryClient nexonApiRetryClient;

    @MockitoBean
    private NexonApiOutboxProcessor outboxProcessor;
     ...
     * <p><b>시나리오</b>:
     * <ol>
     *   <li>NexonApiClient를 503 에러로 Mock 설정</li>
     *   <li>100K Outbox 항목 생성</li>
     *   <li>모든 항목이 PENDING 상태로 적재되는지 확인</li>
     *   <li>Processor가 재시도해도 모두 실패 (503)</li>
     * </ol>
     *
     * <p><b>성공 기준</b>: 100K 건 전체 Outbox 적재, PENDING 상태 유지</p>
     */
    @Test
    ...
    void shouldAccumulateOutboxEntriesDuring6HourOutage() {
        ...
        for (int i = 0; i < totalEntries; i++) {
            ...
            NexonApiOutbox outbox = NexonApiOutbox.create(
                    requestId,
        ...
        // Then: Outbox 적재 확인
        long pendingCount = outboxRepository.countByStatusIn(List.of(OutboxStatus.PENDING, OutboxStatus.FAILED));
        ...
        log.info("│ Throughput: {} entries/sec                                ",
        ...
        assertThat(pendingCount)
                .as("[Nightmare] 100K 건이 Outbox에 적재되어야 함")
     ...
     * <p><b>시나리오</b>:
     * <ol>
     *   <li>10K Outbox 항목 생성</li>
     *   <li>NexonApiClient 503 장애 상태 유지</li>
     *   <li>Processor가 시도하다 실패 (모두 FAILED/RETRY 상태)</li>
     *   <li>NexonApiClient 복구 (200 OK)</li>
     *   <li>Processor 자동 재시도 → COMPLETED</li>
     * </ol>
     *
     * <p><b>성공 기준</b>: 복구 후 100% COMPLETED</p>
     */
    @Test
    ...
    void shouldReplayAllOutboxEntriesAfterApiRecovery() throws Exception {
        ...
        // 실패 상태 확인
        long failedCount = outboxRepository.countByStatusIn(List.of(OutboxStatus.FAILED, OutboxStatus.PENDING));
        ...
        Awaitility.await()
                ...
                .untilAsserted(() -> {
                    ...
                    log.info("[Blue] Progress: {}/{} completed, {} DLQ",
                            completed, totalEntries, deadLetter);
     ...
     *
     * <p>처리량이 SLA를 충족하는지 검증 (목표: > 1000 rows/sec)</p>
     */
    @Test
    ...
    void shouldAchieveTargetReplayThroughput() throws Exception {
        ...
        assertThat(throughput)
                .as("[Nightmare] 처리량은 100 rows/sec 이상이어야 함 (완화된 목표)")
                .isGreaterThan(100);
    }
     ...
     *
     * <p>재처리 과정에서 데이터 유실이 없고, DLQ 비율이 0.1% 미만인지 검증</p>
     */
    @Test
    ...
    void shouldMaintainDataIntegrity_withZeroDataLoss() throws Exception {
        ...
        Awaitility.await()
                ...
                .untilAsserted(() -> {
                    ...
                    log.info("[Purple] Status - COMPLETED: {}, DLQ: {}, PENDING: {}, FAILED: {}",
                            completed, deadLetter, pending, failed);
        ...
        log.info("│ DLQ Rate: {}% (Target: < 0.1%)                           ", String.format("%.4f", dlqRate));
        ...
        assertThat(totalAccounted)
                .as("[Nightmare] 데이터 유실은 0건이어야 함 (모든 항목 계상)")
                .isEqualTo(totalEntries);
     ...
     *
     * <p>Setup → Outage → Recovery → Validation 전체 흐름 검증</p>
     */
    @Test
    ...
    void shouldSurvive6HourOutage_withCompleteRecovery() throws Exception {
        ...
        long initialPending = outboxRepository.countByStatusIn(List.of(OutboxStatus.PENDING, OutboxStatus.FAILED));
        ...
        while (System.currentTimeMillis() - outageStart < outageDuration) {
            ...
            log.info("[Yellow] Outage in progress... {} sec elapsed",
        ...
        log.info("[Yellow] Phase 2 Complete - Failed: {}, Pending: {}", failedDuringOutage, stillPending);
        ...
        for (int i = 0; i < 30; i++) {
            ...
            if (i % 10 == 0) {
                long completed = outboxRepository.countByStatusIn(List.of(OutboxStatus.COMPLETED));
                log.info("[Yellow] Recovery progress: {} / {} completed ({:.1f}%)",
        ...
        log.info("│ Total Accounted: {} / {}                                   ", totalAccounted, totalEntries);
        log.info("├────────────────────────────────────────────────────────────┤");
        ...
        if (isZeroDataLoss && isDlqAcceptable && isHighCompletionRate) {
            ...
            log.info("│ Zero Loss: {}                                             ", isZeroDataLoss ? "✅" : "❌");
            log.info("│ DLQ OK: {}                                                ", isDlqAcceptable ? "✅" : "❌");
            log.info("│ High Completion: {}                                       ", isHighCompletionRate ? "✅" : "❌");
        }

        log.info("└────────────────────────────────────────────────────────────┘");

        assertThat(totalAccounted)
                .as("[Nightmare] E2E: 데이터 유실 0건")
                .isEqualTo(totalEntries);
    ...
    private void mockApiServiceUnavailable() {
        Mockito.when(nexonApiClient.getOcidByCharacterName(anyString()))
                .thenReturn(CompletableFuture.failedFuture(new RuntimeException("503 Service Unavailable")));
        ...
        Mockito.when(nexonApiRetryClient.processOutboxEntry(Mockito.any()))
                .thenReturn(false);
