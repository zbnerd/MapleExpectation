 ...
 * <ul>
 *   <li>🔴 Red (SRE): 장애 주입 - ThreadPool 큐 포화로 CallerRunsPolicy 발동</li>
 *   <li>🔵 Blue (Architect): 흐름 검증 - HTTP 요청 스레드가 백그라운드 작업 수행</li>
 *   <li>🟢 Green (Performance): 메트릭 검증 - 응답 시간 증가, 타임아웃 발생</li>
 *   <li>🟣 Purple (Auditor): 데이터 무결성 - 작업 완료 여부 확인</li>
 *   <li>🟡 Yellow (QA Master): 테스트 전략 - 실제 운영 Executor 설정 검증</li>
 * </ul>
 *
 * <h4>테스트 목적</h4>
 * <p>실제 운영 환경의 {@code expectationComputeExecutor}와 {@code alertTaskExecutor}가
 * CallerRunsPolicy를 사용하지 않고 AbortPolicy 기반 정책을 사용하는지 검증한다.</p>
 *
 * <h4>운영 Executor 설정 (ExecutorConfig.java)</h4>
 * <ul>
 *   <li><b>expectationComputeExecutor</b>: EXPECTATION_ABORT_POLICY (Issue #168)</li>
 *   <li><b>alertTaskExecutor</b>: LOGGING_ABORT_POLICY</li>
 * </ul>
 *
 * <h4>예상 결과: PASS</h4>
 * <p>운영 환경은 CallerRunsPolicy를 사용하지 않으므로 HTTP 스레드 블로킹이 발생하지 않음.</p>
 *
 * <h4>관련 CS 원리</h4>
 * <ul>
 *   <li>CallerRunsPolicy: 큐가 가득 차면 제출자 스레드에서 직접 실행</li>
 *   <li>Thread Pool Exhaustion: 모든 스레드가 작업 중일 때 발생</li>
 *   <li>Backpressure Leak: 비동기→동기 전환으로 backpressure 전파</li>
 *   <li>Cascading Timeout: 한 컴포넌트 지연이 전체 요청 타임아웃 유발</li>
 * </ul>
 *
...
class CallerRunsPolicyNightmareTest extends IntegrationTestSupport {
     ...
     * <p><b>시나리오</b>:
     * <ol>
     *   <li>실제 운영 Executor의 설정 확인</li>
     *   <li>큐 포화 상황 유도</li>
     *   <li>CallerRunsPolicy 발동 여부 검증</li>
     * </ol>
     *
     * <p><b>성공 기준</b>: CallerRunsPolicy 발동 0회</p>
     * <p><b>실패 조건</b>: 호출자 스레드에서 작업 실행됨</p>
     */
    @Test
     ...
     *
     * <p>CallerRunsPolicy vs AbortPolicy 동작 차이를 명확히 비교</p>
     */
    @Test
    ...
    void shouldSimulateHttpTimeout_CallerRunsPolicyVsAbortPolicy() throws Exception {
        ...
        log.info("│   - Response Time: {}ms                                    │", callerRunsResponseTime.get());
        log.info("│   - Blocked HTTP thread: {}                                │",
                callerRunsResponseTime.get() > CALLER_THREAD_THRESHOLD_MS ? "YES ❌" : "NO ✅");
        log.info("├────────────────────────────────────────────────────────────┤");
        log.info("│ AbortPolicy:                                               │");
        log.info("│   - Response Time: {}ms                                    │", abortResponseTime.get());
        log.info("│   - Fast Fail: {}                                          │",
        ...
        // AbortPolicy는 빠른 실패
        assertThat(abortResponseTime.get())
                .as("AbortPolicy provides fast fail")
                .isLessThan(100);
    }
     ...
     *
     * <p>테스트 전용 Executor로 CallerRunsPolicy가 얼마나 위험한지 시연</p>
     */
    @Test
    ...
    void shouldDemonstrateCascadingTimeoutRisk_withCallerRunsPolicy() throws Exception {
        ThreadPoolExecutor dangerousExecutor = new ThreadPoolExecutor(
        ...
        log.info("│ Executor Thread Executions: {}                             │", executorThreadCount.get());
        log.info("│ Caller Thread Executions: {}                               │", callerThreadCount.get());
        log.info("│ Max Block Time: {}ms                                       │", maxBlockTime.get());
        log.info("│ Total Submit Time: {}ms                                    │", totalTime);
        log.info("├────────────────────────────────────────────────────────────┤");

        if (callerThreadCount.get() > 0) {
            log.info("│ ❌ CallerRunsPolicy blocked caller thread {} times         │", callerThreadCount.get());
            log.info("│                                                            │");
            log.info("│ Impact in Production:                                      │");
            log.info("│   - HTTP request takes {}ms instead of ~0ms                │", maxBlockTime.get());
