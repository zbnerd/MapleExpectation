 ...
 * <ul>
 *   <li>🔴 Red (SRE): 장애 주입 - Toxiproxy로 네트워크 지연(latency) 주입</li>
 *   <li>🟣 Purple (Auditor): 데이터 무결성 검증 - 타임아웃 후 데이터 정합성</li>
 *   <li>🔵 Blue (Architect): 흐름 검증 - Fail-Fast 원칙 및 타임아웃 전파</li>
 *   <li>🟢 Green (Performance): 메트릭 검증 - 응답 시간 분포, 타임아웃 비율</li>
 *   <li>🟡 Yellow (QA Master): 테스트 전략 - 경계값 테스트 (타임아웃 임계치)</li>
 * </ul>
 *
 * <h4>검증 포인트</h4>
 * <ol>
 *   <li>느린 응답이 전체 시스템을 블로킹하지 않는지 (Fail-Fast)</li>
 *   <li>타임아웃이 정확히 작동하는지</li>
 *   <li>느린 응답 후 복구가 정상적인지</li>
 *   <li>분산 락이 느린 네트워크에서도 안전하게 동작하는지</li>
 * </ol>
 *
 * <h4>CS 원리</h4>
 * <ul>
 *   <li>Slow Loris Attack: 연결을 느리게 유지하여 리소스 고갈</li>
 *   <li>Fail-Fast Principle: 빠른 실패로 리소스 해제</li>
 *   <li>Timeout Propagation: 타임아웃의 전파와 전체 예산 관리</li>
 *   <li>Back-pressure: 느린 하위 시스템에 대한 압력 제어</li>
 * </ul>
 *
...
@Tag("chaos")
@SpringBootTest
@DisplayName("Scenario 06: Slow Loris - 느린 응답 및 타임아웃 검증")
class SlowLorisChaosTest extends AbstractContainerBaseTest {
    ...
    private static final int LATENCY_MS = 3000; // 3초 지연
    ...
    /**
     * 🔴 Red's Test 1: 네트워크 지연 주입 시 타임아웃 동작 검증
     *
     * <p><b>시나리오</b>:
     * <ol>
     *   <li>Toxiproxy로 3초 지연 주입</li>
     *   <li>Redis 작업 시도 (타임아웃 2초 설정)</li>
     *   <li>예상: 타임아웃 발생 (Fail-Fast)</li>
     * </ol>
     */
    @Test
    @DisplayName("네트워크 지연 시 Fail-Fast 타임아웃 동작")
    void shouldTimeout_whenNetworkLatencyInjected() throws Exception {
        // Given: Toxiproxy로 3초 지연 주입
        redisProxy.toxics()
        ...
        assertThat(elapsedMs)
                .as("타임아웃이 설정 시간(2초) 근처에서 발생해야 함")
                .isBetween(1500L, 2500L);
    }

    /**
     * 🟢 Green's Test 2: 느린 응답 후 정상 복구 확인
     *
     * <p><b>시나리오</b>:
     * <ol>
     *   <li>지연 주입 후 일부 요청 타임아웃</li>
     *   <li>지연 제거</li>
     *   <li>정상 응답 확인</li>
     * </ol>
     */
    @Test
    @DisplayName("지연 제거 후 정상 응답 복구")
    void shouldRecover_afterLatencyRemoved() throws Exception {
        ...
        assertThat(fastElapsed)
                .as("복구 후 응답 시간은 100ms 미만이어야 함")
                .isLessThan(100L);
    }

    /**
     * 🔵 Blue's Test 3: 분산 락의 느린 네트워크 내구성
     *
     * <p><b>시나리오</b>: 지연이 있어도 분산 락이 안전하게 동작하는지 검증
     */
    @Test
    @DisplayName("분산 락은 느린 네트워크에서도 안전하게 동작")
    void shouldMaintainLockSafety_underSlowNetwork() throws Exception {
        ...
        // 500ms 지연 주입 (락 작업에 영향을 주지만 완전 실패하지는 않음)
        redisProxy.toxics()
        ...
        // 최소 1개 이상의 스레드가 락을 획득해야 함
        assertThat(successCount.get())
                .as("느린 네트워크에서도 최소 1개 스레드는 락 획득 성공")
                .isGreaterThanOrEqualTo(1);
    }

    /**
     * 🟣 Purple's Test 4: 점진적 지연 증가 시 시스템 반응
     *
     * <p><b>시나리오</b>: 지연이 점진적으로 증가할 때 시스템이 적절히 반응하는지
     */
    @Test
    @DisplayName("점진적 지연 증가 시 시스템 반응 분석")
    void shouldAnalyze_gradualLatencyIncrease() throws Exception {
        ...
        System.out.println("┌────────────────────────────────────────────────────────────┐");
        System.out.println("│               Latency vs Response Time Analysis            │");
        System.out.println("├────────────────────────────────────────────────────────────┤");
        ...
        for (int i = 0; i < latencies.length; i++) {
            ...
            try {
                ...
                // 타임아웃 발생 시 현재 시간까지를 응답 시간으로 기록
            }
            responseTimes[i] = (System.nanoTime() - start) / 1_000_000;

            System.out.printf("│ Injected Latency: %4dms │ Response Time: %5dms      │%n",
                    latencies[i], responseTimes[i]);
        }

        System.out.println("└────────────────────────────────────────────────────────────┘");

        // 응답 시간이 주입된 지연과 비례해서 증가해야 함
        for (int i = 0; i < latencies.length; i++) {
            assertThat(responseTimes[i])
                    .as("응답 시간은 주입된 지연(%dms)보다 길어야 함", latencies[i])
                    .isGreaterThanOrEqualTo(latencies[i]);
        }
    }

    /**
     * 🟡 Yellow's Test 5: 타임아웃 경계값 테스트
     *
     * <p><b>시나리오</b>: 타임아웃 임계치 근처에서의 동작 검증
     */
    @Test
    @DisplayName("타임아웃 경계값에서 Fail-Fast 동작")
    void shouldFailFast_atTimeoutBoundary() throws Exception {
        // 정확히 타임아웃 경계(1초)에 가까운 지연 테스트
        int boundaryLatency = 1100; // 1.1초
        int timeout = 1000; // 1초 타임아웃
        ...
        // 타임아웃이 발생해야 하고, 대기 시간이 타임아웃 설정 근처여야 함
        assertThat(timedOut).as("지연이 타임아웃보다 길면 타임아웃 발생").isTrue();
        assertThat(elapsed.toMillis())
                .as("타임아웃은 설정 시간(1초) 근처에서 발생")
