 ...
 *
 * <ul>
 *   <li>🔴 Red (SRE): 장애 주입 - 시스템 시간 조작 시뮬레이션
 *   <li>🟣 Purple (Auditor): 데이터 무결성 검증 - TTL/만료 시간 정합성
 *   <li>🔵 Blue (Architect): 흐름 검증 - 시간 기반 로직 안전성
 *   <li>🟢 Green (Performance): 메트릭 검증 - 시간 드리프트 영향
 *   <li>🟡 Yellow (QA Master): 테스트 전략 - 경계값 테스트
 * </ul>
 *
 * <h4>검증 포인트</h4>
 *
 * <ol>
 *   <li>TTL 계산 시 Clock Drift에 영향받지 않는지
 *   <li>분산 락 만료 시간의 안전성
 *   <li>이벤트 순서(Ordering)의 일관성
 *   <li>Monotonic Clock 사용 여부
 * </ol>
 *
 * <h4>CS 원리</h4>
 *
 * <ul>
 *   <li>Wall Clock vs Monotonic Clock
 *   <li>NTP (Network Time Protocol) 동기화
 *   <li>Lamport Timestamp / Vector Clock
 * </ul>
 *
 * @see java.time.Clock
 * @see System#nanoTime()
...
class ClockDriftChaosTest extends AbstractContainerBaseTest {
  ...
  void shouldUsServerTime_forRedisTTL() throws Exception {
    ...
    assertThat(remainingTtl).as("5초 후 TTL은 약 5초가 남아야 함").isBetween(4L, 6L);
  }
              ...
              try {
                ...
                if (acquired) {
                  try {
                    ...
                    System.out.printf(
                        "[Purple] Thread %d acquired lock at %s%n", threadId, Instant.now());
  ...
  void shouldAlwaysIncrease_monotonicClock() {
    ...
    assertThat(monotonicViolations).as("nanoTime은 절대 감소하지 않아야 함").isZero();
  }
  ...
  void shouldExpireAccurately_withShortTTL() throws Exception {
    ...
    assertThat(expiredValue).as("1초 TTL 후 키가 만료되어야 함").isNull();
  }
}
