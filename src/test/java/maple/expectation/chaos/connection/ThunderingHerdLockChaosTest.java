 ...
 * <ul>
 *   <li>🔴 Red (SRE): 장애 주입 - 대량의 동시 락 요청</li>
 *   <li>🟢 Green (Performance): 메트릭 검증 - 락 대기 시간, 처리량</li>
 *   <li>🟣 Purple (Auditor): 데이터 검증 - 락으로 보호되는 데이터 무결성</li>
 *   <li>🟡 Yellow (QA Master): 테스트 전략 - 동시성 경계 조건</li>
 * </ul>
 *
 * <h4>검증 포인트</h4>
 * <ol>
 *   <li>다수의 스레드가 동시에 락 요청 시 처리</li>
 *   <li>락 해제 시 대기 중인 스레드 하나만 획득</li>
 *   <li>Thundering Herd로 인한 성능 저하 측정</li>
 *   <li>공정성(Fairness) 보장</li>
 * </ol>
 *
...
class ThunderingHerdLockChaosTest extends AbstractContainerBaseTest {
    ...
    void shouldHandleThunderingHerd_withManyThreads() throws Exception {
        ...
        System.out.printf("│ Avg Wait Time: %dms                                         │%n", avgWaitTime);
        System.out.printf("│ Max Wait Time: %dms                                         │%n", maxWaitTime);
        System.out.printf("│ Total Test Duration: %dms                                   │%n", testDuration);
        System.out.printf("│ Throughput: %.1f locks/sec                                  │%n",
                successCount.get() * 1000.0 / testDuration);
        System.out.println("└────────────────────────────────────────────────────────────┘");

        // 대부분의 요청이 성공해야 함
        assertThat(successCount.get())
                .as("대부분의 락 요청이 성공해야 함")
    ...
    void shouldProcessInOrder_fairLocking() throws Exception {
        ...
        System.out.println("├────────────────────────────────────────────────────────────┤");
        System.out.printf("│ Acquire Order: %s                                          │%n", acquireOrder);
        System.out.println("└────────────────────────────────────────────────────────────┘");
    ...
    void shouldMaintainIntegrity_withLock() throws Exception {
        ...
        System.out.println("├────────────────────────────────────────────────────────────┤");
        System.out.printf("│ Threads: %d, Increments/Thread: %d                          │%n",
                threadCount, incrementsPerThread);
        System.out.printf("│ Expected Count: %d                                          │%n", expectedCount);
        System.out.printf("│ Actual Count: %d                                            │%n", actualCount);
        System.out.printf("│ Integrity: %s                                               │%n",
                actualCount == expectedCount ? "PASS ✅" : "FAIL ❌");
        System.out.println("└────────────────────────────────────────────────────────────┘");

        assertThat(actualCount)
                .as("락으로 보호된 카운터는 정확해야 함")
