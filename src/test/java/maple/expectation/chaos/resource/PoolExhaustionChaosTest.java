 ...
 * <ul>
 *   <li>🔴 Red (SRE): 장애 주입 - 커넥션을 점유하여 풀 고갈</li>
 *   <li>🔵 Blue (Architect): 흐름 검증 - 풀 고갈 시 Fail-Fast 동작</li>
 *   <li>🟢 Green (Performance): 메트릭 검증 - 커넥션 대기 시간, 타임아웃</li>
 *   <li>🟣 Purple (Auditor): 데이터 검증 - 풀 고갈이 데이터 무결성에 영향 없음</li>
 * </ul>
 *
 * <h4>검증 포인트</h4>
 * <ol>
 *   <li>풀 고갈 시 빠른 타임아웃 발생</li>
 *   <li>커넥션 반환 후 빠른 복구</li>
 *   <li>HikariCP의 connectionTimeout 동작</li>
 *   <li>풀 고갈 중 다른 요청에 미치는 영향</li>
 * </ol>
 *
...
class PoolExhaustionChaosTest extends AbstractContainerBaseTest {
        ...
        try {
            ...
            for (int i = 0; i < maxConnections + 5; i++) {
                try {
                    ...
                    exhaustCount++;
                    System.out.printf("│ Connection %d acquired in %dms                              │%n", i + 1, elapsed);
                } catch (Exception e) {
            ...
            System.out.printf("│ Held connections: %d                                        │%n", heldConnections.size());
            System.out.println("└────────────────────────────────────────────────────────────┘");
            ...
            System.out.println("┌────────────────────────────────────────────────────────────┐");
            System.out.printf("│ Extra connection attempt: %s after %dms         │%n",
    ...
    void shouldAnalyze_poolContention() throws Exception {
        ...
        long avgTime = times.length > 0 ?
                java.util.Arrays.stream(times).sum() / times.length : 0;
        long maxTime = times.length > 0 ?
                java.util.Arrays.stream(times).max().orElse(0) : 0;

        System.out.println("┌────────────────────────────────────────────────────────────┐");
        System.out.println("│               Pool Contention Analysis                     │");
        System.out.println("├────────────────────────────────────────────────────────────┤");
        System.out.printf("│ Concurrent Requests: %d                                     │%n", concurrentRequests);
        System.out.printf("│ Success: %d, Timeout: %d                                    │%n",
