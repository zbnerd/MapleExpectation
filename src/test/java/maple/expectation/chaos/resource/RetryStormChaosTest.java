 ...
 * <ul>
 *   <li>🔴 Red (SRE): 장애 주입 - 일시적 장애로 대량 재시도 유발</li>
 *   <li>🔵 Blue (Architect): 흐름 검증 - Exponential Backoff 동작</li>
 *   <li>🟢 Green (Performance): 메트릭 검증 - 재시도 횟수 및 부하 측정</li>
 *   <li>🟡 Yellow (QA Master): 테스트 전략 - Retry Budget 검증</li>
 * </ul>
 *
 * <h4>검증 포인트</h4>
 * <ol>
 *   <li>동시 재시도가 시스템을 압도하지 않는지</li>
 *   <li>Exponential Backoff가 제대로 작동하는지</li>
 *   <li>Retry Budget이 재시도 폭주를 제한하는지</li>
 *   <li>Circuit Breaker가 재시도 폭풍을 차단하는지</li>
 * </ol>
 *
...
@Tag("chaos")
@SpringBootTest
@DisplayName("Scenario 09: Retry Storm - 재시도 폭풍 및 Backoff 검증")
class RetryStormChaosTest extends AbstractContainerBaseTest {
    ...
    /**
     * 🔴 Red's Test 1: 일시적 장애가 재시도 폭풍을 유발하는지 검증
     */
    @Test
    @DisplayName("일시적 장애 시 재시도 횟수 제한 확인")
    ...
    void shouldIncreaseBackoffExponentially() throws Exception {
        ...
        System.out.println("┌────────────────────────────────────────────────────────────┐");
        System.out.println("│               Exponential Backoff Analysis                 │");
        System.out.println("├────────────────────────────────────────────────────────────┤");
        System.out.printf("│ Interval 1→2: %dms (expected: ~100ms + execution)          │%n", retryIntervals[1]);
        System.out.printf("│ Interval 2→3: %dms (expected: ~200ms + execution)          │%n", retryIntervals[2]);
        System.out.println("└────────────────────────────────────────────────────────────┘");

        // 두 번째 간격이 첫 번째보다 길어야 함 (Exponential)
        assertThat(retryIntervals[2])
                .as("두 번째 재시도 간격이 첫 번째보다 길어야 함")
                .isGreaterThan(retryIntervals[1]);
    }

    /**
     * 🟢 Green's Test 3: 장애 복구 후 재시도 성공 확인
     */
    @Test
    @DisplayName("장애 복구 후 재시도 성공")
    void shouldSucceed_afterFailureRecovery() throws Exception {
        ...
        System.out.println("┌────────────────────────────────────────────────────────────┐");
        System.out.printf("│ Recovery Test: %s after %d attempts               │%n",
                success ? "SUCCESS" : "FAILED", attemptCount.get());
        System.out.println("└────────────────────────────────────────────────────────────┘");

        assertThat(success).as("장애 복구 후 재시도 성공").isTrue();
