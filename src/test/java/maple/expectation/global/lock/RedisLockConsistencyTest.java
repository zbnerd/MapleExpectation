...
/**
 * Redis Lock 일관성 검증 테스트 (Issue #310 Phase 1)
 *
 * <h3>목적</h3>
 * <p>Dual-Run 기간 동안 Redis와 MySQL 락 간의 데이터 일관성을 검증합니다.</p>
 *
 * <h3>검증 항목</h3>
 * <ul>
 *   <li>Lock Key Collision: 동일 키로 동시 접근 시 정확성</li>
 *   <li>Counter Accuracy: 원자적 증감 연산 정확성</li>
 *   <li>Lock Ordering: 다중 락 순서 보장</li>
 *   <li>Concurrency: 1000 TPS 부하 하에서의 일관성</li>
 * </ul>
 *
 * <h3>실행 방법</h3>
 * <pre>
...
class RedisLockConsistencyTest {
    ...
    @Autowired(required = false)
    @Qualifier("mySqlNamedLockStrategy")
     ...
     *
     * <p>동일 키로 100개의 스레드가 동시에 락 획득 시도 시
     * 모든 작업이 정확히 순차적으로 실행되는지 검증</p>
     */
    @Test
    @EnabledIfSystemProperty(named = "test.consistency.enabled", matches = "true", disabledReason = "Consistency test requires explicit activation")
    void testLockKeyCollision_NoRaceCondition() throws Exception {
        ...
        for (int i = 0; i < threadCount; i++) {
            Future<Integer> future = executorService.submit(() -> {
                try {
                    return redisLockStrategy.executeWithLock(lockKey, 10, 30, () -> {
    ...
    void testCounterAccuracy_ExactlyOnce() throws Exception {
        ...
        for (int i = 0; i < increments; i++) {
            ...
            executorService.submit(() -> {
                redisLockStrategy.executeWithLock(lockKey, 5, 10, () -> {
     ...
     *
     * <p>다중 락을 알파벳순으로 획득 시 Deadlock 방지 검증</p>
     */
    @Test
    @EnabledIfSystemProperty(named = "test.consistency.enabled", matches = "true")
    ...
    /**
     * Test 4: Dual-Run 결과 비교 (Redis vs MySQL)
     *
     * <p>Redis와 MySQL이 동일한 작업에 대해 동일한 결과를 반환하는지 검증</p>
     */
    @Test
    @EnabledIfSystemProperty(named = "test.consistency.enabled", matches = "true")
    void testDualRunResult_Consistency() throws Exception {
        // Given
        String lockKey = "test:consistency:dual";
        ...
        for (int i = 0; i < operations; i++) {
            int result = redisLockStrategy.executeWithLock(lockKey, 5, 10, () -> {
                return sharedCounter.incrementAndGet();
            });
            redisResults.add(result);
        }

        // When - MySQL 락으로 100번 실행 (if available)
        List<Integer> mysqlResults = new CopyOnWriteArrayList<>();
        if (mysqlLockStrategy != null) {
            for (int i = 0; i < operations; i++) {
                int result = mysqlLockStrategy.executeWithLock(lockKey, 5, 10, () -> {
                    return sharedCounter.incrementAndGet();
                });
        ...
        // MySQL 결과 검증 (if available)
        if (mysqlLockStrategy != null) {
            assertThat(mysqlResults).hasSize(operations);
            // MySQL은 이어서 실행되므로 101~200
            assertThat(mysqlResults.get(0)).isEqualTo(operations + 1);
            assertThat(mysqlResults.get(operations - 1)).isEqualTo(operations * 2);
        }

        // 전체 카운터 검증
        int expectedTotal = mysqlLockStrategy != null ? operations * 2 : operations;
     ...
     *
     * <p>초당 1000건의 요청을 10초간 처리하여 일관성 검증</p>
     */
    @Test
    @EnabledIfSystemProperty(named = "test.consistency.enabled", matches = "true")
    void testHighLoadConsistency_1000TPS() throws Exception {
        ...
        for (int i = 0; i < totalOperations; i++) {
            highLoadExecutor.submit(() -> {
                try {
                    redisLockStrategy.executeWithLock(lockKey, 10, 30, () -> {
                        counter.incrementAndGet();
                        return null;
                    });
                } catch (Exception e) {
     ...
     *
     * <p>Redis Cluster 환경에서 Hash Tag가 동일 슬롯을 보장하는지 검증</p>
     */
    @Test
    @EnabledIfSystemProperty(named = "test.consistency.enabled", matches = "true")
