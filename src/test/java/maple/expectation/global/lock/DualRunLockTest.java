 ...
 *
 * <h3>목적</h3>
 * <p>Redis와 MySQL 두 락 전략을 동시에 실행하여 결과를 비교하고,
 * 데이터 일관성을 검증합니다.</p>
 *
 * <h3>테스트 시나리오</h3>
 * <ul>
 *   <li>Scenario 1: 두 락이 동일한 작업에 대해 동일한 결과 반환</li>
 *   <li>Scenario 2: 락 획득 순서 보장 (Alphabetical ordering)</li>
 *   <li>Scenario 3: Concurrent execution에서의 일관성</li>
 *   <li>Scenario 4: Fallback 동작 검증</li>
 * </ul>
 *
 * <h3>실행 방법</h3>
 * <pre>
...
class DualRunLockTest {
     ...
     */
    @Test
    @EnabledIfSystemProperty(named = "test.dual.run.enabled", matches = "true", disabledReason = "Dual-Run test requires explicit activation")
    void testSingleLockExecution_Consistency() throws Exception {
        ...
        // When - Redis 락으로 실행
        int redisResult = redisLockStrategy.executeWithLock(lockKey, 5, 10, () -> {
        ...
        if (mysqlLockStrategy != null) {
            mysqlResult = mysqlLockStrategy.executeWithLock(lockKey, 5, 10, () -> {
                return counter.incrementAndGet();
            });
        }
     ...
     *
     * <p>Coffman Condition #4 (Circular Wait) 방지 검증</p>
     */
    @Test
     ...
     *
     * <p>다중 스레드에서 동시에 락 획득 시도 시 정확성 검증</p>
     */
    @Test
    @EnabledIfSystemProperty(named = "test.dual.run.enabled", matches = "true")
     ...
     *
     * <p>Redis 실패 시 MySQL로 자동 전환되는지 검증</p>
     */
    @Test
    @EnabledIfSystemProperty(named = "test.dual.run.enabled", matches = "true")
    void testRedisFailure_MySqlFallback() throws Exception {
        ...
        if (mysqlLockStrategy != null) {
            ...
            boolean redisSuccess = redisLockStrategy.executeWithLock(lockKey, 5, 10, () -> {
                return true;
            });
