 ...
 *
 * <p>Retry Storm 방지를 위한 예산 관리 기능 검증
 *
 * <h4>검증 항목</h4>
 *
 * <ul>
 *   <li>예산 허용: 정상적인 재시도 시도 허용
 *   <li>예산 소진: 한도 초과 시 Fail Fast
 *   <li>윈도우 리셋: 시간 경과 후 자동 리셋
 *   <li>메트릭 게시: Micrometer 카운터 정확성
 *   <li>Thread-Safety: 동시 요청 안전성
 * </ul>
 *
...
class RetryBudgetManagerTest {
  ...
  @Mock private RetryBudgetProperties properties;
  ...
  void shouldPublishMetrics_WhenEnabled() {
    ...
    assertThat(meterRegistry.get("retry_budget_attempts_total").counter().count())
  ...
  void shouldPublishRejectionMetrics_WhenBudgetExceeded() {
    ...
    // Then: 거부 메트릭 확인
    assertThat(meterRegistry.get("retry_budget_rejected_total").counter().count())
