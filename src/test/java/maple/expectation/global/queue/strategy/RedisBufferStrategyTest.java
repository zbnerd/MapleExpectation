 ...
 * <ul>
 *   <li>Yellow (QA): Mock 기반 단위 테스트로 빠른 피드백</li>
 *   <li>Purple (Auditor): 핵심 로직 검증 (publish, consume, ack, nack)</li>
 *   <li>Blue (Architect): 전략 패턴 계약 준수 검증</li>
 * </ul>
 *
 * <h3>참고</h3>
 * <p>Redis 실제 연동 통합 테스트는 Testcontainers를 사용하여 별도 IT 클래스에서 수행합니다.</p>
 */
...
class RedisBufferStrategyTest {
    ...
    @Mock
    private BufferLuaScriptProvider scriptProvider;
    ...
    void setUp() {
        ...
        buffer = new RedisBufferStrategy<>(
                redissonClient,
                scriptProvider,
                objectMapper,
                executor,
    ...
    private void setupRedissonMocks() {
        ...
        lenient().when(redissonClient.getDeque(anyString(), any(Codec.class))).thenReturn((RDeque) rDeque);
        lenient().when(rBucket.isExists()).thenReturn(true);

        // ScriptProvider Mock - 직접 함수 실행
        lenient().when(scriptProvider.executeWithNoscriptHandling(
                any(Supplier.class),
                anyString(),
                any(Consumer.class),
                any(Function.class),
    ...
    private void setupExecutorMocks() {
        ...
        lenient().when(executor.executeOrDefault(any(ThrowingSupplier.class), any(), any(TaskContext.class)))
        ...
        lenient().when(executor.executeWithFallback(any(ThrowingSupplier.class), any(Function.class), any(TaskContext.class)))
                .thenAnswer((Answer<Object>) invocation -> {
                    ThrowingSupplier<?> task = invocation.getArgument(0);
    ...
    class PublishTest {
        ...
        void publish_shouldReturnMsgId() {
            ...
            when(rScript.evalSha(any(RScript.Mode.class), anyString(), any(RScript.ReturnType.class), anyList(), any(), any()))
                    .thenReturn(1L);
    ...
    class ConsumeTest {
        ...
        void consume_shouldReturnQueueMessages() {
            ...
            String payloadJson = "{\"payload\":{\"id\":1,\"content\":\"test\"},\"retryCount\":0,\"createdAtMs\":1700000000000}";

            List<List<String>> rawResult = List.of(
                    Arrays.asList(msgId, payloadJson)
            );
    ...
    class AckTest {
        ...
        void ack_shouldSucceed() {
            // Given
            when(rScript.evalSha(any(RScript.Mode.class), anyString(), any(RScript.ReturnType.class), anyList(), anyString()))
                    .thenReturn(1L);
    ...
    class NackTest {
        ...
        void nack_shouldMoveToRetryQueue() {
            ...
            when(rScript.evalSha(any(RScript.Mode.class), anyString(), any(RScript.ReturnType.class), anyList(), any()))
                    .thenReturn(1L);
