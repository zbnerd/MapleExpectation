 ...
 *   <li>Purple (Auditor): ACK/NACK 패턴 검증</li>
 *   <li>Blue (Architect): 기존 API 호환성 검증</li>
 * </ul>
 */
...
class RedisExpectationWriteBackBufferTest {
    ...
    @Mock
    private RedisBufferStrategy<ExpectationWriteTask> redisStrategy;
    ...
    class DrainTest {
        ...
        void drain_shouldReturnTasks() {
            ...
            QueueMessage<ExpectationWriteTask> message = new QueueMessage<>(
                    "msg-1", task, 0, Instant.now()
            );
    ...
    class AckAllTest {
        ...
        void ackAll_shouldAckAllDrainedMessages() {
            // Given
            ExpectationWriteTask task = createTestTask();
            QueueMessage<ExpectationWriteTask> message1 = new QueueMessage<>("msg-1", task, 0, Instant.now());
            QueueMessage<ExpectationWriteTask> message2 = new QueueMessage<>("msg-2", task, 0, Instant.now());
    ...
    class NackAllTest {
        ...
        void nackAll_shouldNackAllDrainedMessages() {
            // Given
            ExpectationWriteTask task = createTestTask();
            QueueMessage<ExpectationWriteTask> message = new QueueMessage<>("msg-1", task, 0, Instant.now());
            when(redisStrategy.consume(10)).thenReturn(List.of(message));
    ...
    private List<PresetExpectation> createTestPresets() {
        ...
        PresetExpectation preset1 = PresetExpectation.builder()
                .presetNo(1)
                .totalExpectedCost(BigDecimal.valueOf(2000))
                .costBreakdown(breakdown)
                .build();

        PresetExpectation preset2 = PresetExpectation.builder()
                .presetNo(2)
