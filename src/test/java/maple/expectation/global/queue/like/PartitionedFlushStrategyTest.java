 ...
 * <ul>
 *   <li>Yellow (QA): 파티션 분산 락 테스트</li>
 *   <li>Red (SRE): 락 획득 실패 시 데이터 복원 검증</li>
 *   <li>Purple (Auditor): 부분 실패 시 보상 트랜잭션 검증</li>
 * </ul>
 */
...
class PartitionedFlushStrategyTest {
    ...
    @Mock
    private RedisLikeBufferStorage bufferStorage;
    ...
    void setUp() {
        ...
        strategy = new PartitionedFlushStrategy(redissonClient, bufferStorage, executor, meterRegistry, syncExecutor);
    }
    ...
    private void setupExecutorMocks() {
        ...
        lenient().when(executor.executeWithFinally(any(ThrowingSupplier.class), any(Runnable.class), any(TaskContext.class)))
                .thenAnswer((Answer<Object>) invocation -> {
                    ThrowingSupplier<?> task = invocation.getArgument(0);
    ...
    class FlushWithPartitionsTest {
        ...
        void flush_shouldReturnEmptyWhenNoData() throws Exception {
            ...
            // When
            PartitionedFlushStrategy.FlushResult result = strategy.flushWithPartitions((ign, delta) -> {});
        ...
        void flush_shouldRestoreFailedEntries() throws Exception {
            ...
            BiConsumer<String, Long> failingProcessor = (ign, delta) -> {
                if ("user2".equals(ign)) {
