 ...
 *   <li>Purple (Auditor): SADD 원자성, 중복 검사 검증</li>
 * </ul>
 */
...
class RedisLikeRelationBufferTest {
    ...
    @Mock
    private RSet<String> relationSet;

    @Mock
    private RSet<String> pendingSet;
    ...
    private void setupRedissonMocks() {
        lenient().when(redissonClient.<String>getSet(RedisKey.LIKE_RELATIONS.getKey())).thenReturn(relationSet);
        lenient().when(redissonClient.<String>getSet(RedisKey.LIKE_RELATIONS_PENDING.getKey())).thenReturn(pendingSet);
    ...
    private void setupExecutorMocks() {
        ...
        lenient().when(executor.executeOrCatch(any(ThrowingSupplier.class), any(Function.class), any(TaskContext.class)))
                .thenAnswer((Answer<Object>) invocation -> {
                    ThrowingSupplier<?> task = invocation.getArgument(0);
