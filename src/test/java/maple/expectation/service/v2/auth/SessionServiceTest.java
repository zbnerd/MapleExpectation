 ...
 * <p>Spring Context 없이 Mockito만으로 세션 관리 서비스를 검증합니다.</p>
 *
 * <h4>테스트 범위</h4>
 * <ul>
 *   <li>세션 생성 (UUID 생성, Redis 저장)</li>
 *   <li>세션 조회 및 TTL 갱신 (Sliding Window)</li>
 *   <li>세션 조회 (TTL 갱신 없음)</li>
 *   <li>세션 삭제 (로그아웃)</li>
 *   <li>세션 존재 여부 확인</li>
 * </ul>
 */
    ...
    class CreateSessionTest {
        ...
        void shouldCreateUserSession() {
            ...
            verify(sessionRepository).save(argThat(s ->
                    s.fingerprint().equals(FINGERPRINT) &&
                    s.role().equals(ROLE_USER)));
        }
