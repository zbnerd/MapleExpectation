 ...
 * <h3>검증 항목</h3>
 * <ul>
 *   <li>Pub/Sub 이벤트 발행 및 수신 정상 동작</li>
 ...
 *   <li>테스트 간 상태 격리 (Redis flushAll)</li>
 * </ul>
 */
@DisplayName("좋아요 실시간 동기화 통합 테스트 (Issue #278)")
@TestPropertySource(properties = {
        "like.realtime.enabled=true",
        "app.instance-id=test-instance-1"
})
class LikeRealtimeSyncIntegrationTest extends IntegrationTestSupport {
    ...
    void shouldPublishAndReceiveEventViaTopic() throws InterruptedException {
        ...
        // 별도 리스너 등록 (테스트용)
        int listenerId = topic.addListener(LikeEvent.class, (channel, event) -> {
