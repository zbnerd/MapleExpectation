...
class PotentialCalculatorTest {
    ...
    void setUp() {
        ...
        when(executor.execute(any(), any(TaskContext.class))).thenAnswer(invocation -> {
            try {
                // 첫 번째 인자인 ThrowingSupplier를 실행
                maple.expectation.global.common.function.ThrowingSupplier<?> supplier = invocation.getArgument(0);
                return supplier.get();
            } catch (Throwable e) {
        ...
        when(statParser.parseNum(anyString())).thenAnswer(inv -> {
