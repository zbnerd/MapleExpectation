 ...
 * <ul>
 *   <li>Base URL: https://api.z.ai/api/paas/v4</li>
 *   <li>Model: glm-4.7 (또는 GLM-4.7)</li>
 *   <li>Protocol: OpenAI Chat Completions 호환</li>
 * </ul>
 *
...
public class ZAiConfiguration {
     ...
     *
     * <p>OpenAI-compatible API를 통해 Z.ai의 GLM-4.7 모델을 사용합니다.</p>
     *
    ...
    @Bean
    @Primary
    @ConditionalOnProperty(name = "langchain4j.glm-4.chat-model.api-key", matchIfMissing = false)
    ...
    /**
     * Fallback: OpenAI GPT-4o-mini (Z.ai 미설정 시)
     *
     * <p>Z.ai API 키가 미설정이거나 실패 시 사용합니다.</p>
     */
    @Bean
    @ConditionalOnProperty(name = "langchain4j.glm-4.chat-model.api-key", matchIfMissing = false)
    public ChatLanguageModel openAiFallbackModel(
            @Value("${langchain4j.open-ai.chat-model.api-key:}") String openAiKey,
            @Value("${langchain4j.open-ai.chat-model.model-name:gpt-4o-mini}") String openAiModel
    ) {
        log.info("[Z.ai] Fallback: OpenAI 모델 설정됨: model={}", openAiModel);

        return OpenAiChatModel.builder()
                .apiKey(openAiKey)
                .modelName(openAiModel)
                .timeout(java.time.Duration.parse("PT30s"))
                .logRequests(false)
                .logResponses(false)
                .build();
    }
