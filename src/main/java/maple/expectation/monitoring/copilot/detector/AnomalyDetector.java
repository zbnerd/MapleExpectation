 ...
 *
 * <ul>
 *   <li>Threshold-based: Check against warn/crit thresholds
 *   <li>Z-score: Statistical anomaly detection using mean/stdDev
 *   <li>Hybrid: Both methods combined
 * </ul>
 *
 * <h3>Usage</h3>
 *
...
public class AnomalyDetector {
  ...
  private Optional<AnomalyEvent> detectThresholdBased(
      SignalDefinition signal, double currentValue, long detectedAtMillis) {
  ...
  private Optional<AnomalyEvent> detectZScoreBased(
    ...
    if (values.size() < config.getMinRequiredPoints()) {
      log.debug(
          "[AnomalyDetector] Insufficient data for Z-score: {}/{} required",
          values.size(),
          config.getMinRequiredPoints());
      return Optional.empty();
    }
    ...
    if (stdDev == 0.0) {
      log.debug(
          "[AnomalyDetector] Zero stdDev - all values are identical for signal: {}",
          signal.panelTitle());
      return Optional.empty();
    }
    ...
    log.debug(
        "[AnomalyDetector] Z-score calculation for {}: mean={:.2f}, stdDev={:.2f}, z={:.2f}",
        signal.panelTitle(),
        mean,
        stdDev,
        zScore);
  ...
  private String buildZScoreReason(
    ...
    return String.format(
        ...
        signal.panelTitle(), currentValue, zScore, mean, stdDev, config.getThreshold());
  }
}
