 ...
 * <p>Prevents alert spam by tracking recent incidents via incidentId.</p>
 *
 * <h4>CLAUDE.md Compliance</h4>
 * <ul>
 *   <li>Section 12: All exceptions via LogicExecutor</li>
 *   <li>Section 12-1: Circuit breaker for AI calls</li>
 * </ul>
 *
...
public class MonitoringPipelineService {
     ...
     *
     * <p>Fixed-rate scheduling ensures continuous monitoring even if
     * previous execution takes longer than interval.</p>
     */
    @Scheduled(fixedRateString = "${app.monitoring.interval-seconds:15000}")
    public void runMonitoringCycle() {
        executor.executeVoid(
                this::executePipeline,
                TaskContext.of("MonitoringPipeline", "RunCycle")
        );
    }
    ...
    private Map<String, List<TimeSeries>> queryMetrics(List<SignalDefinition> signals, long now) {
        ...
        for (SignalDefinition signal : signals) {
            ...
            if (!prometheusSeries.isEmpty()) {
                ...
                List<TimeSeries> modelSeries = prometheusSeries.stream()
                        .map(this::convertToModelTimeSeries)
                        .toList();
    ...
    private TimeSeries convertToModelTimeSeries(PrometheusClient.TimeSeries promSeries) {
        ...
        List<MetricPoint> points = promSeries.values().stream()
                .map(vp -> new MetricPoint(
    ...
    private String generateIncidentId(List<AnomalyEvent> anomalies) {
        ...
        String signature = anomalies.stream()
                .limit(3)
    ...
    private String buildSummary(List<AnomalyEvent> anomalies) {
        long critCount = anomalies.stream()
                .filter(a -> "CRIT".equals(a.severity()))
                .count();
    ...
    private List<EvidenceItem> buildEvidence(
        ...
        Map<String, SignalDefinition> signalMap = signals.stream()
    ...
    private void sendDiscordAlert(IncidentContext context, AiSreService.MitigationPlan plan) {
        ...
        List<DiscordNotifier.AnnotatedSignal> annotatedSignals = context.anomalies().stream()
                .limit(3)
                .map(anomaly -> new DiscordNotifier.AnnotatedSignal(
                        signalMap.get(anomaly.signalId()),
                        anomaly.currentValue()
                ))
                .toList();

        List<String> hypotheses = plan.hypotheses().stream()
                .limit(2)
        ...
        String severity = context.anomalies().stream()
                .anyMatch(a -> "CRIT".equals(a.severity())) ? "CRIT" : "WARN";
    ...
    private AiSreService.MitigationPlan createDefaultMitigationPlan(IncidentContext context) {
        ...
        List<AiSreService.Hypothesis> defaultHypotheses = List.of(
        ...
        List<AiSreService.Action> defaultActions = List.of(
                ...
                new AiSreService.Action(3, "Escalate to on-call engineer", "LOW", "Human intervention required")
        );

        AiSreService.RollbackPlan rollbackPlan = new AiSreService.RollbackPlan(
                "If symptoms worsen",
                List.of("Revert recent changes", "Scale up resources")
        );
