 ...
 * Parses dashboard JSON files and extracts PromQL signal definitions.
 *
...
public class GrafanaJsonIngestor {
    ...
    /**
     * Internal ingestion with checked exceptions.
     ...
     */
    private void extractPanelsRecursively(JsonNode panels, String dashboardUid, String dashboardTitle, List<SignalDefinition> signals) throws Exception {
        ...
        for (JsonNode panel : panels) {
            ...
            for (JsonNode target : targets) {
                ...
                signals.add(SignalDefinition.builder()
    ...
    private SeverityMapping extractThresholds(JsonNode panel) {
        JsonNode steps = panel.path("fieldConfig")
                .path("defaults")
                .path("thresholds")
                .path("steps");
    ...
    private int calculatePriority(String title, String expr) {
        ...
        if (lower.contains("p99") || lower.contains("error") || lower.contains("timeout")
                || lower.contains("deadletter") || lower.contains("dlq")
                || lower.contains("lag") || lower.contains("pending")
                || lower.contains("pool") || lower.contains("gc")
                || lower.contains("oom") || lower.contains("lock")) {
            return HIGH_PRIORITY_SCORE;
        }

        // Medium priority keywords
        if (lower.contains("hit rate") || lower.contains("throughput")
     ...
     */
    private String generateStableId(String dashboardUid, int panelId, String refId, String expr) throws Exception {
