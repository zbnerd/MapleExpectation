 ...
 *   <li>🔴 Red (SRE): InnoDB Buffer Pool 오염 방지 - JSON 대신 계산된 값만 저장</li>
 *   <li>🟢 Green (Performance): 인덱스 최적화 - game_character_id로 빠른 조회</li>
 * </ul>
 *
 * <h3>DB 스키마</h3>
 * <pre>
 ...
 */
@Entity
@Table(name = "equipment_expectation_summary",
        indexes = {
                @Index(name = "idx_game_character_preset", columnList = "game_character_id, preset_no")
        },
        uniqueConstraints = {
                @UniqueConstraint(name = "uk_character_preset", columnNames = {"game_character_id", "preset_no"})
        })
@Getter
...
public class EquipmentExpectationSummary {
     ...
     * 연관된 게임 캐릭터 ID
     *
     * <p>FK는 어플리케이션 레벨에서 관리 (성능상 이유로 물리 FK 미사용)</p>
     */
     ...
     * 총 기대 비용 (모든 강화 합산)
     *
     * <p>DECIMAL(20, 2): 최대 999경 메소까지 표현 가능</p>
     */
