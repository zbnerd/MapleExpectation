 ...
 *
 * <h3>기능</h3>
 * <p>전날 인기 캐릭터 TOP N을 조회하여 캐시를 미리 웜업합니다.
 * 서버 시작 시 또는 매일 새벽에 실행되어 Cold Start 문제를 해결합니다.</p>
 *
 * <h3>실행 시점</h3>
 * <ul>
 *   <li>매일 새벽 5시 (트래픽 낮은 시간)</li>
 *   <li>서버 시작 후 30초 (초기 웜업)</li>
 * </ul>
 *
 * <h3>5-Agent Council 합의</h3>
 * <ul>
 *   <li>Green (Performance): 웜업으로 첫 요청 지연 방지, RPS 향상</li>
 *   <li>Blue (Architect): 분산 락으로 단일 인스턴스만 웜업 실행</li>
 *   <li>Red (SRE): 요청 간 지연으로 Thundering Herd 방지</li>
 *   <li>Yellow (QA): 성공/실패 메트릭으로 모니터링</li>
 * </ul>
 *
 * <h3>설정</h3>
 * <pre>
...
public class PopularCharacterWarmupScheduler {
     ...
     * 서버 시작 후 30초 뒤 초기 웜업
     *
     * <p>서버 재시작 후 Cold Cache 상태를 빠르게 해소합니다.</p>
     */
     ...
     * 웜업 실행 (분산 락 사용)
     *
     * <p>여러 인스턴스 중 하나만 웜업을 실행하도록 분산 락을 사용합니다.</p>
     *
    ...
    private void executeWarmup(String warmupType) {
        ...
        executor.executeOrCatch(() -> {
            ...
            lockStrategy.executeWithLock("popular-warmup-lock", 0, 300, () -> {
                doWarmup(warmupType);
                return null;
            });
            return null;
            ...
            if (e instanceof maple.expectation.global.error.exception.DistributedLockException) {
                ...
                meterRegistry.counter("warmup.execution", "type", warmupType, "status", "error").increment();
            }
            return null;
        }, context);
    }
    ...
    private void doWarmup(String warmupType) {
        ...
        log.info("[Warmup] {} started at {} - warming up top {} characters",
                warmupType, LocalDateTime.now(), topCount);
        ...
        log.info("[Warmup] {} completed - success: {}, fail: {}, total: {}",
                warmupType, successCount.get(), failCount.get(), topCharacters.size());
    }
     ...
     */
    private void warmupCharacter(String userIgn, AtomicInteger successCount, AtomicInteger failCount) {
        executor.executeOrCatch(() -> {
