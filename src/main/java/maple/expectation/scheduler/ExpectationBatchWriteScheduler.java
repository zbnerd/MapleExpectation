 ...
 * <ul>
 *   <li>Blue (Architect): LikeSyncScheduler 패턴 재사용</li>
 *   <li>Red (SRE): 분산 락으로 중복 실행 방지</li>
 *   <li>Green (Performance): 5초 주기 배치 동기화</li>
 * </ul>
 *
 * <h3>동작 방식</h3>
 * <ol>
 *   <li>5초마다 버퍼에서 최대 100개 작업 추출</li>
 *   <li>분산 락 획득 후 배치 upsert</li>
 *   <li>락 획득 실패 시 다음 주기로 연기</li>
 * </ol>
 *
 * <h3>메트릭</h3>
 * <ul>
...
public class ExpectationBatchWriteScheduler {
    ...
    public void flush() {
        ...
        executor.executeOrCatch(() -> {
            ...
            // 분산 락으로 중복 실행 방지
            lockStrategy.executeWithLock(LOCK_NAME, 0, 10, () -> {
                flushBatch();
                return null;
            });
    ...
    private void flushBatch() {
        List<ExpectationWriteTask> batch = buffer.drain(BATCH_SIZE);
        ...
        for (ExpectationWriteTask task : batch) {
            executor.executeOrCatch(() -> {
                ...
                log.error("[ExpectationBatch] Upsert failed for {}: {}",
        ...
        log.debug("[ExpectationBatch] Flushed {} tasks, remaining={}",
                successCount, buffer.getPendingCount());
    }
