 ...
 *
 * <p>BYOK (Bring Your Own Key) 방식으로 사용자의 API Key를 직접 사용합니다.</p>
 *
 * <p>보안 고려사항:
 * <ul>
 *   <li>사용자의 API Key는 로그에 절대 남기지 않음</li>
 *   <li>타임아웃 설정으로 무한 대기 방지</li>
 *   <li>예외 발생 시 안전하게 Optional.empty() 반환 (LogicExecutor 패턴)</li>
 * </ul>
 * </p>
 *
 * <p>CLAUDE.md 섹션 12 준수: try-catch 대신 WebClient.onErrorResume() + LogicExecutor.executeOrDefault() 사용</p>
 *
 * <p>onErrorResume vs onStatus:
 * <ul>
 *   <li>onStatus: 상태 코드만 확인 가능, 응답 본문 접근 불가</li>
 *   <li>onErrorResume: WebClientResponseException으로 상태 코드 + 응답 본문 모두 접근 가능</li>
 *   <li>디버깅을 위해 Nexon API의 실제 에러 메시지 로깅 필요 → onErrorResume 사용</li>
 * </ul>
 * </p>
 */
@Slf4j
...
public class RealNexonAuthClient implements NexonAuthClient {
     ...
     * <p>패턴 설명:
     * <ul>
     *   <li>4xx 에러: Nexon API 에러 메시지 로깅 후 Mono.empty() 반환</li>
     *   <li>5xx 에러: 상위로 전파 (서킷브레이커 동작)</li>
     *   <li>성공: 응답 본문 반환</li>
     * </ul>
     * </p>
     */
    private Optional<CharacterListResponse> doGetCharacterList(String apiKey) {
        ...
        CharacterListResponse response = mapleWebClient.get()
            ...
            .onErrorResume(WebClientResponseException.class, ex -> {
                if (ex.getStatusCode().is4xxClientError()) {
                    ...
                    log.warn("[NexonAuth] API Call Failed. Status: {}, Body: {}",
