 ...
 *
 * <p>Issue #63: Zero-Copy 스트리밍으로 GZIP 중복 압축/해제 제거
 */
@Slf4j
@RestController
...
public class GameCharacterControllerV3 {
   ...
   *
   * <p>Issue #63: 데이터가 이미 GZIP 압축되어 있으므로 그대로 전송합니다.
   *
   * <h4>최적화 효과</h4>
   *
   * <ul>
   *   <li>기존: GZIP byte[] → decompress → String → getBytes → GZIPOutputStream (이중 압축)
   *   <li>변경: GZIP byte[] → 직접 전송 (Zero-Copy)
   *   <li>CPU 사용량 50% 감소, 메모리 할당 최소화
   * </ul>
   */
   ...
   *
   * <p>Spring MVC의 CompletableFuture 반환 지원을 활용하여 톰캣 스레드를 즉시 반환하고, Future 완료 시 응답을 전송합니다.
   *
   * <h4>비동기 흐름</h4>
   *
   * <ol>
   *   <li>톰캣 스레드: 요청 수신 → CompletableFuture 반환 → 즉시 풀 반환
   *   <li>expectation-* 스레드: 실제 계산 수행
   *   <li>Future 완료 시: Spring이 자동으로 응답 전송
   * </ol>
   */
