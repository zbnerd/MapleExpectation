 ...
 *
 * <ol>
 *   <li>Cache Miss → 동기 실행 후 캐시 저장
 *   <li>Cache Hit + PER 당첨 → 비동기 갱신 + Stale 데이터 반환
 *   <li>Cache Hit + PER 미당첨 → 캐시 데이터 반환
 * </ol>
 *
 * <h4>Non-Blocking 보장</h4>
 *
 * <ul>
 *   <li>캐시 Hit 시 항상 즉시 반환 (Stale 허용)
 *   <li>백그라운드 갱신은 전용 Thread Pool에서 Fire & Forget
 *   <li>갱신 실패해도 기존 데이터 유지
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li>읽기 전용 캐시: JavaType 파싱 결과 캐싱 (변경 없음)
 *   <li>인스턴스별 독립: 동일 Method에 대해 동일한 JavaType 생성
 *   <li>P2-GREEN-01: 성능 최적화용, 비즈니스 영향 없음
 * </ul>
 *
 * <h4>5-Agent Council 합의 (P1-4)</h4>
 *
 * <ul>
 *   <li>Blue (Architect): 읽기 전용 캐시로 Scale-out 안전
 *   <li>Green (Performance): 인스턴스별 캐싱으로 JVM 내 최적화
 * </ul>
 *
...
public class ProbabilisticCacheAspect {
  ...
  public Object handleCache(ProceedingJoinPoint joinPoint, ProbabilisticCache probabilisticCache)
      throws Throwable {
  ...
  private Object recomputeAndCache(
      ProceedingJoinPoint joinPoint, String cacheKey, ProbabilisticCache annotation)
      throws Throwable {
    ...
    if (json != null) {
      ...
      log.debug(
          "💾 [PER] 캐시 저장: key={}, delta={}ms, ttl={}s", cacheKey, delta, annotation.ttlSeconds());
    }
  ...
  private String serializeWrapperSafely(CachedWrapper<Object> wrapper, String cacheKey) {
    ...
    return executor.executeOrDefault(() -> objectMapper.writeValueAsString(wrapper), null, context);
  }
  ...
  private CachedWrapper<Object> deserializeWrapperSafely(
      String json, String cacheKey, Method method) {
  ...
  private JavaType buildWrapperType(Method method) {
    ...
    return objectMapper.getTypeFactory().constructParametricType(CachedWrapper.class, valueType);
  }
