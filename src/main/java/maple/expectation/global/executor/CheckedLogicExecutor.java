 ...
 *
 * <p>파일 I/O, 네트워크 통신, 외부 API 호출 등 checked 예외가 발생하는 IO 경계에서
 * **try-catch 없이** 예외를 처리하는 템플릿을 제공합니다.</p>
 *
 * <h3>컴파일 타임 경계 강제</h3>
 * <ul>
 *   <li><b>Biz 레이어</b>: {@link LogicExecutor} + {@code Supplier<T>} → checked-throwing 람다 불가</li>
 *   <li><b>IO 레이어</b>: {@link CheckedLogicExecutor} + {@code CheckedSupplier<T>} → checked-throwing 람다 허용</li>
 * </ul>
 *
...
public interface CheckedLogicExecutor {
     ...
     *
     * <p>호출자가 throws Exception을 선언하고 상위에서 처리하는 경우 사용합니다.</p>
     *
     * <h4>예외 처리</h4>
     * <ul>
     *   <li><b>Error</b>: 즉시 throw</li>
     *   <li><b>RuntimeException</b>: 그대로 통과 (단, cause/suppressed에 InterruptedException이 있으면 플래그 복원)</li>
     *   <li><b>Exception</b>: 그대로 전파 (인터럽트 플래그 복원 포함)</li>
     * </ul>
     *
     * @param <T> 반환 타입
     * @param task 실행할 작업
     ...
     *
     * <p><b>try-catch 없이</b> checked 예외를 도메인 RuntimeException으로 변환합니다.
     * 예외 변환 로직이 템플릿 내부로 중앙화되어 호출 코드가 깔끔해집니다.</p>
     *
     * <h4>예외 처리 우선순위</h4>
     * <ol>
     *   <li><b>Error</b>: 즉시 throw (mapper 미호출)</li>
     *   <li><b>RuntimeException</b>: 그대로 throw (이미 unchecked)</li>
     *   <li><b>Exception</b>: mapper.apply(e) 결과를 throw</li>
     *   <li><b>Throwable (비-Exception)</b>: IllegalStateException</li>
     * </ol>
     *
     * <h4>mapper 계약</h4>
     * <ul>
     *   <li>null 반환 금지 → IllegalStateException</li>
     ...
     *
     * <p><b>분산 락, 파일 핸들, 커넥션 등</b> 반드시 해제해야 하는 자원이 있을 때 사용합니다.
     * finalizer는 task 성공/실패와 무관하게 **정확히 1회** 실행됩니다.</p>
     *
     * <h4>예외 우선순위</h4>
     * <ol>
     *   <li>finalizer에서 Error 발생 → <b>즉시 throw</b> (task 결과/예외 무관)</li>
     *   <li>task Error → 즉시 throw</li>
     *   <li>task RuntimeException → 그대로 throw (finalizer 예외는 suppressed)</li>
     *   <li>task Exception → mapper 변환 후 throw (finalizer 예외는 suppressed)</li>
     *   <li>task 성공 + finalizer 예외 → finalizer 예외가 primary</li>
     * </ol>
     *
     * <h4>mapper 적용 범위</h4>
     * <p><b>primary Exception이 task/finalizer 어디에서 발생했든, Level 1에서는 mapper로 RuntimeException으로 변환됩니다.</b>
     * 예: task 성공 + finalizer가 IOException 던짐 → mapper로 변환되어 RuntimeException throw</p>
     *
     * <h4>suppressed 이관</h4>
     * <p>task Exception을 mapper로 변환할 때, task Exception에 누적된 suppressed 예외들이
     * 새로 생성된 RuntimeException으로 복사됩니다. 이를 통해 cleanup 실패 정보가 유실되지 않습니다.</p>
     *
     * @param <T> 반환 타입
     * @param task 실행할 작업
