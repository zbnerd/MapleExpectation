 ...
 * <ul>
 *   <li><b>Pipeline executeRaw() 호출</b>: 모든 메서드가 ExecutionPipeline을 통해 실행</li>
 *   <li><b>Error 즉시 rethrow</b>: VirtualMachineError 등은 번역 없이 전파</li>
 *   <li><b>SG1 적용</b>: executeWithRecovery는 execute() 재사용 금지, executeRaw() 직접 호출</li>
 *   <li><b>호출부 0 수정</b>: 기존 메서드 시그니처 유지, 내부 구현만 교체</li>
 * </ul>
 *
 * @since 2.4.0
 */
...
public class DefaultLogicExecutor implements LogicExecutor {
    ...
    public <T> T executeWithFinally(ThrowingSupplier<T> task, Runnable finallyBlock, TaskContext context) {
        ...
        if (withFinally == pipeline) {
            log.warn("ExecutionPipeline.withAdditionalPolicies returned same instance. " +
