 ...
 *
 * - before:     [Task:START] {taskName}                              -> DEBUG
 * - onSuccess:  (normal) [Task:SUCCESS] {taskName}, elapsed=...      -> DEBUG
 *               (slow)   [Task:SLOW] {taskName}, elapsed=..., threshold=...ms -> INFO (비활성 시 DEBUG 폴백)
 * - onFailure:  [Task:FAILURE] {taskName}, elapsed=..., errorType=... (stacktrace) -> ERROR
 * - after:      [Task:AFTER] {taskName}, outcome=..., elapsed=...    -> DEBUG
 */
@Slf4j
...
public final class LoggingPolicy implements ExecutionPolicy {
    ...
    /**
     * @param slowMs slow 판정 임계치(ms).
     *               0 이하: SLOW 기능 비활성 (모든 성공 로그가 일반 SUCCESS로 처리)
     *               1 이상: 이 값 이상이면 SLOW 로그로 승격(INFO, 비활성 시 DEBUG 폴백)
     */
    public LoggingPolicy(long slowMs) {
        ...
        this.slowThresholdNanos = slowEnabled
                ? TimeUnit.MILLISECONDS.toNanos(clamped)
                : Long.MAX_VALUE; // 비활성: 절대 도달 불가
    }
