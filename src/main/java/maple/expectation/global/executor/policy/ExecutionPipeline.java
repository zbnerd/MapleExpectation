 ...
 * <ul>
 *   <li><b>elapsedNanos는 task.get() 구간만 측정</b> (정책 시간 제외)</li>
 *   <li><b>BEFORE는 등록 순서, AFTER는 역순(LIFO)</b></li>
 *   <li><b>before() 성공한 정책만 after() 호출</b> (entered pairing)</li>
 *   <li><b>BEFORE(PROPAGATE) 실패 시 onFailure 미호출</b>:
 *       task가 시작되지 않았으므로 onFailure를 호출하지 않음 (entered 정책만 after로 정리)</li>
 *   <li><b>onSuccess/onFailure는 관측 훅</b>:
 *       non-Error 예외는 결과(성공/실패/전파 타입)를 변경하지 않음.
 *       단, onFailure 훅 자체 실패(<b>non-Error</b>)는 원인(cause)에 suppressed로 보존함</li>
 *   <li><b>task Error에도 onFailure best-effort 호출</b>:
 *       task가 Error로 실패해도 onFailure는 best-effort로 실행함.
 *       단, onFailure 내부에서 Error 발생 시 즉시 중단함</li>
 *   <li><b>Error는 최우선 전파 대상</b>:
 *       훅 단계에서 Error가 발생하면 우선 전파 대상으로 승격하며,
 *       AFTER unwind를 끝까지 시도한 뒤 최종 throw함</li>
 *   <li><b>실패 경로에서 후속 예외는 suppressed로 보존</b>:
 *       primary가 존재하면 이후 예외는 suppressed로만 누적함</li>
 *   <li><b>성공 경로에서 AFTER 실패는 primary가 될 수 있음</b>:
 *       primary가 없으면 최초 AFTER 예외를 최종 throw 대상으로 설정함</li>
 *   <li><b>outcome은 정책 훅 실패와 무관하게 task 실행 결과만을 의미함</b>:
 *       after(outcome, ...)에 전달되는 outcome은 훅/정책 실패를 반영하지 않고,
 *       오직 task.get()의 성공/실패만을 나타냄</li>
 *   <li><b>ThreadLocal depth로 재진입 폭주 fail-fast</b> (MAX_NESTING_DEPTH)</li>
 * </ul>
 *
 * @since 2.4.0
 */
...
public class ExecutionPipeline {
    ...
    public ExecutionPipeline(List<ExecutionPolicy> policies) {
        ...
        for (int i = 0; i < policies.size(); i++) {
            ...
            FailureMode mode = Objects.requireNonNull(p.failureMode(), "policies[" + i + "].failureMode() is null");
    ...
    public <T> T executeRaw(ThrowingSupplier<T> task, TaskContext context) throws Throwable {
        ...
        if (depth > MAX_NESTING_DEPTH) {
            ...
            log.error("[Pipeline:REENTRANCY] nesting depth exceeded. depth={}, limit={}, contextType={}, contextHash={}",
                    depth, MAX_NESTING_DEPTH, context.getClass().getSimpleName(), System.identityHashCode(context));

            throw new IllegalStateException(
                    "ExecutionPipeline nesting depth exceeded (" + MAX_NESTING_DEPTH + "). Possible recursion loop."
            );
        }
        ...
        try {
                ...
                try {
                    ...
                    for (Slot slot : entered) {
                        try {
                            ...
                            log.error("[Pipeline:CRITICAL] Error in onFailure hook. policy={}, taskName={}",
                                    policyName(slot.policy()), taskName, err);
            ...
            if (primary == null && taskOutcome == ExecutionOutcome.SUCCESS) {
                ...
                for (Slot slot : entered) {
                    try {
                        ...
                        log.error("[Pipeline:CRITICAL] Error in onSuccess hook. policy={}, taskName={}",
                                policyName(slot.policy()), taskName, err);
                        primary = promoteError(primary, err);
            ...
            for (int i = entered.size() - 1; i >= 0; i--) {
                ...
                try {
                    ...
                    log.error("[Pipeline:CRITICAL] Error in after hook. policy={}, taskName={}",
                            policyName(slot.policy()), taskName, err);
                    primary = promoteError(primary, err);
     ...
     *
     * <p>executeWithFinally() 등에서 동적으로 정책을 추가할 때 사용합니다.</p>
     * <p>기존 Pipeline의 정책 + 추가 정책 순서로 병합됩니다 (BEFORE는 순서대로, AFTER는 역순).</p>
     *
    ...
    private <T> void invokeOnSuccess(Slot slot, T result, long elapsedNanos, TaskContext context, String taskName) {
        try {
    ...
    private void invokeOnFailure(Slot slot, Throwable cause, long elapsedNanos, TaskContext context, String taskName) {
    ...
    private void invokeAfter(Slot slot, ExecutionOutcome outcome, long elapsedNanos, TaskContext context, String taskName) throws Throwable {
        try {
    ...
    private void addSuppressedSafely(Throwable primary, Throwable suppressed) {
        ...
        try {
            ...
            log.debug("addSuppressed failed. primary={}, suppressed={}",
                    primary.getClass().getName(), suppressed.getClass().getName(), e);
        }
    }
