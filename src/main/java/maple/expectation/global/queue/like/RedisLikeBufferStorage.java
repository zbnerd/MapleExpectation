 ...
 * <h3>역할</h3>
 *
 * <p>Redis HASH를 사용하여 좋아요 증분(delta)을 버퍼링합니다. Scale-out 환경에서도 단일 진실 소스(Single Source of Truth)를
 * 보장합니다.
 *
 * <h3>기존 LikeBufferStorage 대비 개선</h3>
 *
 * <ul>
 *   <li>기존: Caffeine Cache (In-Memory) → 인스턴스별 분산
 *   <li>개선: Redis HASH (Distributed) → 전역 일관성
 * </ul>
 *
 * <h3>Redis 구조</h3>
 *
 ...
 *
 * <ul>
 *   <li>HINCRBY: 원자적 증분 (동시성 안전)
 *   <li>HSCAN + HDEL: Lua Script로 원자적 fetch + clear
 * </ul>
 *
 ...
 *   <li>Green (Performance): HINCRBY O(1) 복잡도
 *   <li>Red (SRE): 인스턴스 장애 시에도 데이터 보존
...
public class RedisLikeBufferStorage implements LikeBufferStrategy {
   ...
   * 좋아요 증분 (원자적)
   *
   * <p>Redis HINCRBY를 사용하여 동시 요청에도 안전하게 증분합니다.
   *
   ...
   *
   * <p>Lua Script로 HSCAN + HDEL을 원자적으로 수행합니다. 이는 데이터 유실 없이 버퍼를 비우는 것을 보장합니다.
   *
  ...
  public Long remove(String userIgn) {
    return executor.executeOrDefault(
        () -> getBuffer().remove(userIgn), null, TaskContext.of("LikeBuffer", "Remove", userIgn));
  }
  ...
  public int getBufferSize() {
    return executor.executeOrDefault(
        () -> getBuffer().size(), 0, TaskContext.of("LikeBuffer", "Size"));
  }

  /** 총 delta 합계 조회 (메트릭용) */
  private long getTotalDelta() {
    return executor.executeOrDefault(
        () -> getBuffer().readAllValues().stream().mapToLong(Long::longValue).sum(),
        0L,
