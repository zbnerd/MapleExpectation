 ...
 *
 * <h3>역할</h3>
 * <p>분산 환경에서 Like 버퍼를 안전하게 DB로 플러시합니다.
 * 분산 락으로 동일 파티션의 중복 처리를 방지합니다.</p>
 *
 * <h3>분산 락 전략</h3>
 * <ul>
 *   <li>파티션별 독립 락: 한 파티션 실패가 다른 파티션에 영향 없음</li>
 *   <li>tryLock 사용: 이미 처리 중인 파티션은 스킵</li>
 *   <li>Hash-based Partitioning: userIgn.hashCode() % partitionCount</li>
 * </ul>
 *
 * <h3>파티션 구조</h3>
 * <pre>
 ...
 * <ul>
 *   <li>Blue (Architect): 전략 패턴으로 Flush 로직 교체 가능</li>
 *   <li>Green (Performance): 파티션별 병렬 처리로 처리량 향상</li>
 *   <li>Red (SRE): 분산 락으로 중복 처리 방지</li>
 *   <li>Purple (Auditor): 락 획득 실패 시 Skip으로 데이터 유실 없음</li>
 * </ul>
 *
...
public class PartitionedFlushStrategy {
    ...
    public PartitionedFlushStrategy(
        ...
        this(redissonClient, bufferStorage, executor, meterRegistry, syncExecutor, 4, 100L, 30000L, 1000);
    }
     ...
     *
     * <h4>용도</h4>
     * <p>LikeSyncScheduler에서 호출하여 Redis 버퍼를 DB로 동기화합니다.
     * 내부적으로 {@link #flushWithPartitions(BiConsumer)}를 호출하면서
     * DB 저장 로직({@link LikeSyncExecutor#executeIncrement})을 processor로 전달합니다.</p>
     *
     * <h4>5-Agent Council 합의</h4>
     * <ul>
     *   <li>Blue (Architect): Scheduler는 호출만, DB 저장 로직은 캡슐화</li>
     *   <li>Green (Performance): 파티션별 병렬 처리로 throughput 향상</li>
     *   <li>Red (SRE): 분산 락으로 중복 Flush 방지</li>
     * </ul>
     *
     ...
     * 파티션별 분산 Flush 실행
     *
     * <p>각 파티션에 대해 분산 락을 시도하고, 획득한 파티션만 처리합니다.
     * 이미 다른 인스턴스가 처리 중인 파티션은 스킵됩니다.</p>
     *
    ...
    public FlushResult flushWithPartitions(BiConsumer<String, Long> processor) {
        ...
        log.info("[Flush] Completed: partitions={}/{}, entries={}, delta={}",
                acquiredPartitions, partitioned.size(), processedEntries, totalDelta);
    ...
    private PartitionResult processPartition(
            ...
            BiConsumer<String, Long> processor) {
    ...
    private PartitionResult doProcessPartition(
        ...
        if (!failedEntries.isEmpty()) {
            ...
            log.warn("[Flush] Partition {} had {} failed entries, restored to buffer",
                    partitionId, failedEntries.size());
        }
    ...
    private Map<Integer, Map<String, Long>> partitionEntries(Map<String, Long> entries) {
        ...
        for (Map.Entry<String, Long> entry : entries.entrySet()) {
            ...
            partitioned.computeIfAbsent(partitionId, k -> new HashMap<>())
