 ...
 *
 * <p>Redis Cluster에서 원자적 큐 연산을 제공하는 Reliable Queue 패턴 구현
 *
 ...
 *   <li>Yellow (QA): Testcontainers 통합 테스트 가능
 *   <li>Red (SRE): DLQ + Circuit Breaker로 장애 격리
 * </ul>
 *
 * @param <T> 메시지 페이로드 타입
...
public class RedisBufferStrategy<T> implements MessageQueueStrategy<T> {
  ...
  private Long executeNackToRetryScript(
      String sha, String msgId, long nextAttemptAt, int retryCount) {
    return executor.executeOrDefault(
        () -> {
          ...
          PayloadWrapper<T> currentWrapper =
              objectMapper.readValue(
                  currentPayloadJson,
                  objectMapper
                      .getTypeFactory()
          ...
          PayloadWrapper<T> updatedWrapper =
              new PayloadWrapper<>(
                  currentWrapper.payload(), retryCount + 1, currentWrapper.createdAtMs());
  ...
  public List<String> processRetryQueue(int limit) {
    ...
    if (!processed.isEmpty()) {
      ...
      meterRegistry
          .counter("queue.retry.processed", "strategy", getType().name())
