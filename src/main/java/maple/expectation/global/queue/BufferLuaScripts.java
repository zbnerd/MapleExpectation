 ...
 * 모든 스크립트는 Hash Tag를 사용하여 Redis Cluster CROSSSLOT 에러를 방지합니다.</p>
 *
 ...
 *   <li>Yellow (QA): ACK/Redrive 레이스 테스트 가능한 구조</li>
 *   <li>Red (SRE): DLQ로 실패 메시지 격리</li>
 * </ul>
 */
public final class BufferLuaScripts {
     ...
     *
     * <p>msgId를 Main Queue에 추가하고 Payload를 HASH에 저장합니다.</p>
     *
     * <pre>
     ...
     *
     * <p>Main Queue에서 INFLIGHT로 원자적 이동 + 타임스탬프 기록 + Payload 조회</p>
     *
     * <pre>
     * KEYS[1] = mainQueue ({expectation}:buffer)
     ...
     *
     * <h4>원자성</h4>
     * <p>RPOPLPUSH + ZADD + HGET가 단일 트랜잭션으로 실행되어
     * 소비 중 크래시 시 메시지 유실 방지 (INFLIGHT에 있으므로 Re-drive 가능)</p>
     */
     ...
     * Script 3: ACK (처리 완료)
     *
     * <p>INFLIGHT에서 메시지 제거 + 타임스탬프 ZSET 제거 + Payload 삭제</p>
     *
     * <pre>
     ...
     *
     * <h4>멱등성</h4>
     * <p>LREM이 0을 반환하면 이미 ACK된 것이므로 ZREM/HDEL 스킵</p>
     */
     ...
     *
     * <p>INFLIGHT에서 제거 후 Retry Queue에 등록 (Delayed Retry)</p>
     *
     * <pre>
     * KEYS[1] = inflight ({expectation}:buffer:inflight)
     * KEYS[2] = inflightTs ({expectation}:buffer:inflight:ts)
     ...
     * <p>score = nextAttemptAtMs로 지연 재시도 구현</p>
     */
     ...
     *
     * <p>타임아웃된 메시지를 Main Queue로 복귀시킵니다.</p>
     *
     * <pre>
     * KEYS[1] = inflight ({expectation}:buffer:inflight)
     * KEYS[2] = inflightTs ({expectation}:buffer:inflight:ts)
     ...
     * <p>LPOS로 존재 확인 후 이동하여 ACK와 충돌 방지</p>
     */
     ...
     *
     * <p>만료된 Retry 메시지를 Main Queue로 복귀시킵니다.</p>
     *
     * <pre>
     ...
     *
     * <p>만료된 INFLIGHT 메시지 ID 목록을 조회합니다.</p>
     *
     * <pre>
