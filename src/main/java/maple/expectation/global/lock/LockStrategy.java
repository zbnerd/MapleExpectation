 ...
 * <p>다중 락 시나리오에서 Coffman Condition #4 (Circular Wait)를 방지하기 위해
 * {@link #executeWithOrderedLocks} 메서드를 추가했습니다.</p>
 *
 * <h3>CLAUDE.md 준수사항</h3>
 * <ul>
 *   <li>Section 4: OCP 원칙 - default 메서드로 기존 구현체 호환성 유지</li>
 *   <li>P1-BLUE-02: 복합키 방식 기본 구현 (구현체에서 Override 권장)</li>
 * </ul>
 */
public interface LockStrategy {
     ...
     *
     * <p><b>Coffman Condition #4 (Circular Wait) 방지</b>:
     * 락 키들을 알파벳순으로 정렬하여 모든 스레드가 동일한 순서로 락을 획득하도록 강제합니다.</p>
     *
     * <h4>기본 구현: 복합키 방식 (P1-BLUE-02)</h4>
     * <p>키들을 정렬 후 단일 복합키로 결합하여 락을 획득합니다.
     * 순차적 개별 락 획득이 필요한 경우 구현체에서 Override하세요.</p>
     *
     * <h4>사용 예시</h4>
     * <pre>{@code
     * // 계좌 이체: from → to 순서 보장
