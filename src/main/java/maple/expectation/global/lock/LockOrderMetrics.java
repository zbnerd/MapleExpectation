 ...
 *
 * <h3>목적</h3>
 * <p>락 획득 순서 위반을 감지하고 메트릭으로 기록하여 잠재적 Deadlock 위험을 모니터링합니다.</p>
 *
 * <h3>CLAUDE.md 준수사항</h3>
 * <ul>
 *   <li>Section 12: Zero Try-Catch Policy - 모든 예외는 LogicExecutor로 처리</li>
 *   <li>Section 17: 소문자 점 표기법 사용</li>
...
public class LockOrderMetrics {
     ...
     * 메트릭 초기화 (1회만 실행)
     *
     * <p>Green Agent 요구사항: gauge 중복 등록 방지</p>
     */
    @PostConstruct
    public void init() {
        // Counters 초기화
        ...
        log.info("[LockOrderMetrics] Initialized - violation/acquisition counters and held gauge registered");
    }

    /**
     * 락 순서 위반 기록
     *
     * <p>현재 락보다 알파벳순으로 앞선 락을 획득하려 할 때 호출됩니다.
     * 이는 Coffman Condition #4 (Circular Wait) 위반 가능성을 나타냅니다.</p>
     *
    ...
    public void recordViolation(String currentLock, String previousLock) {
        ...
        // 태그 기반 상세 메트릭 (선택적)
        registry.counter("lock.order.violation.detail",
                Tags.of("current", sanitizeKey(currentLock),
                        "previous", sanitizeKey(previousLock))
        ).increment();

        log.warn("[LockOrder] Violation recorded: '{}' requested after '{}' - potential deadlock risk",
     ...
     * 메트릭 태그용 키 정규화
     *
     * <p>Prometheus 태그에 사용할 수 없는 문자 제거</p>
     */
