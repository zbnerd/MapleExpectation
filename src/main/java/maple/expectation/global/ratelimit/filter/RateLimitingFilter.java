package maple.expectation.global.ratelimit.filter;
...
import maple.expectation.global.ratelimit.RateLimitContext;
 ...
 *
 * <p>Filter Chain 위치: MDCFilter → RateLimitingFilter → JwtAuthenticationFilter</p>
 *
 * <h4>처리 흐름</h4>
 * <ol>
 *   <li>클라이언트 IP 추출 (X-Forwarded-For 우선)</li>
 *   <li>SecurityContext에서 인증 정보 확인 (있으면 User 전략, 없으면 IP 전략)</li>
 *   <li>Rate Limit 확인</li>
 *   <li>초과 시 429 응답 + Retry-After 헤더</li>
 * </ol>
 *
 * <p>CRITICAL (Spring Security 6.x Best Practice - Context7):
 * <ul>
 *   <li>@Component 사용 금지 (CGLIB 프록시 → logger NPE)</li>
...
public class RateLimitingFilter extends OncePerRequestFilter {
     ...
     *
     * <p>Purple Agent P1-1 FIX: trustedHeaders 순서대로 확인, 없으면 remoteAddr 사용</p>
     *
     * @param request HTTP 요청
     * @return 클라이언트 IP 주소
     */
     ...
     *
     * <p>JwtAuthenticationFilter가 이 필터보다 나중에 실행되므로,
     * 이 필터에서는 SecurityContext가 비어있을 수 있음 (IP 전략 사용)</p>
     *
     ...
     *
     * <p>예외를 던지지 않고 직접 응답하여 불필요한 처리 방지</p>
     *
     * @param response HTTP 응답
     * @param result ConsumeResult (거부 결과)
     */
    private void handleRateLimitExceeded(HttpServletResponse response, ConsumeResult result) throws IOException {
     ...
     * IP 마스킹 (로깅용)
     *
     * <p>CLAUDE.md 섹션 19 준수: 민감 데이터 마스킹</p>
     *
     * @param ip IP 주소
