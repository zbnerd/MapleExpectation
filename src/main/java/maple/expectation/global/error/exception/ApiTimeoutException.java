 ...
 *
 * <ul>
 *   <li><b>ApiTimeoutException</b>: API 레벨 timeout → 서킷브레이커 기록 (CircuitBreakerRecordMarker)
 *   <li><b>ExpectationCalculationUnavailableException</b>: Follower timeout → 서킷브레이커 무시
 * </ul>
 *
 * <h4>사용 시나리오</h4>
 *
 * <ul>
 *   <li>Nexon API 호출 타임아웃 (EquipmentDataResolver)
 *   <li>외부 서비스 응답 지연
 * </ul>
 *
 * <h4>서킷브레이커 동작</h4>
 *
 * <p>CircuitBreakerRecordMarker를 구현하여 Resilience4j 서킷브레이커가 이 예외를 기록합니다. 반복적인 API 타임아웃 발생 시 서킷브레이커가
 * OPEN 상태로 전환되어 불필요한 API 호출을 차단합니다.
 *
 * @see CircuitBreakerRecordMarker
 * @see ExpectationCalculationUnavailableException
 * @since 2.6.0
 */
public class ApiTimeoutException extends ServerBaseException implements CircuitBreakerRecordMarker {
