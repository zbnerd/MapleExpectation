 ...
 * Retry Budget 관리자
 *
 * <p>장기간 장애 시 재시도 폭주(Retry Storm)를 방지하기 위해 시간 기반 예산 관리</p>
 *
 * <h3>핵심 기능</h3>
 * <ul>
 *   <li><b>시간 윈도우 기반 예산 관리</b>: Sliding Window로 최근 1분간 재시도 횟수 추적</li>
 *   <li><b>예산 소진 시 Fail Fast</b>: 예산 고갈 시 즉시 실패 반환으로 시스템 보호</li>
 *   <li><b>메트릭 게시</b>: Micrometer를 통해 예산 소비율 모니터링</li>
 *   <li><b>Thread-Safe</b>: AtomicLong, LongAdder로 동시성 안전성 확보</li>
 * </ul>
 *
 * <h3>동작 방식</h3>
 * <pre>
 ...
 * <ul>
 *   <li>{@code retry_budget_attempts_total}: 전체 예산 요청 횟수</li>
 *   <li>{@code retry_budget_allowed_total}: 예산 허용 횟수</li>
 *   <li>{@code retry_budget_rejected_total}: 예산 거부 횟수</li>
 *   <li>{@code retry_budget_window_seconds}: 현재 윈도우 경과 시간</li>
 * </ul>
 *
 * @see RetryBudgetProperties
 * @see <a href="https://sre.google/sre-book/addressing-cascading-failures/">Google SRE - Cascading Failures</a>
 */
@Slf4j
@Component
...
public class RetryBudgetManager {
     ...
     *
     * <p>현재 시간 윈도우 내 예산 여부를 확인하고 필요 시 카운터 증가</p>
     *
     * <h4>동작 순서</h4>
     * <ol>
     *   <li>현재 윈도우 확인 및 경과 시 리셋</li>
     *   <li>예산 한도 초과 여부 확인</li>
     *   <li>예산 있으면 카운터 증가 후 true 반환</li>
     *   <li>예산 소진 시 false 반환 (Fail Fast)</li>
     * </ol>
     *
    ...
    public boolean tryAcquire(String serviceName) {
        ...
        if (currentCount >= maxRetries) {
            ...
            log.warn("[RetryBudget] 예산 소진으로 재시도 차단. serviceName={}, current={}, limit={}, window={}s",
                    serviceName, currentCount, maxRetries, getWindowElapsedSeconds());
            return false;
        }
        ...
        log.debug("[RetryBudget] 예산 허용. serviceName={}, count={}/{}", serviceName, currentCount + 1, maxRetries);
        return true;
    }
    ...
    /**
     * 윈도우 경과 시 리셋 수행
     * <p>Thread-safe: CAS(Compare-And-Swap) 없이 epoch 기반 단순 비교</p>
     */
    private void resetWindowIfNeeded() {
        ...
        if (currentEpoch - startEpoch >= windowSize) {
            ...
            retryCounter.reset();
            log.debug("[RetryBudget] 윈도우 경과로 리셋. elapsed={}s, limit={}s",
                    currentEpoch - startEpoch, windowSize);
        }
    }
