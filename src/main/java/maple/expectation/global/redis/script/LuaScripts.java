 ...
 *
 * <p>Redis 싱글 스레드 특성을 활용하여 원자적 연산을 보장합니다. 모든 스크립트는 Hash Tag {buffer:likes}를 사용하여 Redis Cluster
 * CROSSSLOT 에러를 방지합니다.
 *
 ...
 * </pre>
 *
 * <p>Hash Tag는 {buffer:likes} 전체를 감싸야 같은 슬롯 보장됨
 */
public final class LuaScripts {
   ...
   *
   * <p>CRITICAL: Hash Tag 패턴 - {buffer:likes}가 해시 대상
   */
   ...
   * </pre>
   *
   * <p>원자성: HINCRBY + INCRBY가 단일 트랜잭션으로 실행되어 중간 크래시 시 데이터 불일치 방지
   */
   ...
   *
   * <p>동기화 완료 후 임시 키에서 엔트리 삭제 및 전역 카운터 차감
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>멱등성: 중복 실행 시 HDEL이 0을 반환하면 DECRBY를 스킵하여 카운터 중복 차감 방지
   */
   ...
   * </pre>
   *
   * <p>원자성: HINCRBY(복구) + HDEL(임시키 정리)가 단일 트랜잭션으로 실행되어 복구 중 크래시 시 데이터 중복/유실 방지
   */
