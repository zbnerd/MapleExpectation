 ...
 *
 * <p>ISP(Interface Segregation Principle)를 준수하여 좋아요 동기화에 필요한
 * 원자적 연산만 정의합니다.</p>
 *
 * <h2>구현체</h2>
 * <ul>
 *   <li>{@link RedissonLikeAtomicOperations} - Redisson 기반 Lua Script 구현</li>
 * </ul>
 *
 * <h2>원자성 보장</h2>
 * <p>모든 메서드는 Redis Lua Script로 구현되어 원자성이 보장됩니다.
 * 중간 크래시 시에도 데이터 일관성이 유지됩니다.</p>
 */
public interface LikeAtomicOperations {
     ...
     * L1 → L2 원자적 전송
     *
     * <p>로컬 버퍼의 카운트를 Redis Hash와 전역 카운터에 원자적으로 반영합니다.</p>
     *
     * <pre>
     * 수행 연산:
     ...
     *
     * <p>동기화 완료된 엔트리를 임시 키에서 삭제하고 전역 카운터에서 차감합니다.
     * 중복 실행 시 HDEL이 0을 반환하면 DECRBY를 스킵하여 멱등성을 보장합니다.</p>
     *
     * <pre>
     * 수행 연산:
     ...
     *
     * <p>DB 반영 실패 시 임시 키의 데이터를 원본 버퍼로 복구합니다.</p>
     *
     * <pre>
     * 수행 연산:
     * 1. HINCRBY buffer:{likes}:hash userIgn count
