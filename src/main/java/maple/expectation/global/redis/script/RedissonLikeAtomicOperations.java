 ...
 *
 * <p>Lua Script를 사용하여 Redis의 싱글 스레드 특성을 활용한 원자적 연산을 제공합니다.
 *
 * <h2>특징</h2>
 *
 * <ul>
 *   <li>SHA 캐싱: scriptLoad() + evalSha()로 네트워크 오버헤드 최소화
 *   <li>NOSCRIPT 자동 복구: Redis 재시작 시 스크립트 자동 재로드
 *   <li>Fail-Fast 검증: 모든 public 메서드 첫 줄에서 입력값 검증
 *   <li>Graceful Degradation: Redis 장애 시 false 반환 (서비스 가용성 유지)
 *   <li>관측성: 실행 시간, 실패 횟수 메트릭 수집
 * </ul>
 *
 * <h2>Hash Tag (CRITICAL FIX - PR #175, #164)</h2>
 *
 * <p>모든 키는 {buffer:likes} Hash Tag를 사용하여 Redis Cluster CROSSSLOT 에러를 방지합니다.
 */
@Slf4j
@Component
...
public class RedissonLikeAtomicOperations implements LikeAtomicOperations {
  ...
  private long executeDeleteAndDecrementScript(
      String sha, String tempKey, String userIgn, long count) {
    return executor.executeOrDefault(
        () -> {
          RScript script = redissonClient.getScript(StringCodec.INSTANCE);
          Long result =
  ...
  private boolean executeCompensationScript(
      String sha, String tempKey, String userIgn, long count) {
    return executor.executeOrDefault(
        () -> {
          RScript script = redissonClient.getScript(StringCodec.INSTANCE);
