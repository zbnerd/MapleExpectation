 ...
 *
 * <p>Redis Lua Script 실행 시 성능 최적화를 위해 SHA 해시를 캐싱하고,
 * Redis 서버 재시작 등으로 인한 NOSCRIPT 에러 발생 시 자동 재로드합니다.</p>
 *
 * <h2>Thread Safety</h2>
...
public class LuaScriptProvider {
    ...
    public void loadScripts() {
        boolean loaded = executor.executeOrDefault(() -> {
            ...
            String deleteAndDecrementSha = script.scriptLoad(LuaScripts.ATOMIC_DELETE_AND_DECREMENT);
            ...
            log.info("✅ [LuaScriptProvider] SHA 캐싱 완료 - Transfer: {}, DeleteDecr: {}, Compensation: {}",
                    transferSha, deleteAndDecrementSha, compensationSha);
            return true;
        }, false, TaskContext.of("LuaScript", "LoadAll"));
    ...
    public String getTransferSha() {
        return transferShaRef.updateAndGet(current ->
    ...
    public String getDeleteAndDecrementSha() {
        return deleteAndDecrementShaRef.updateAndGet(current ->
                current != null ? current : reloadScript(LuaScripts.ATOMIC_DELETE_AND_DECREMENT, "DeleteAndDecrement"));
    }
    ...
    public String getCompensationSha() {
        return compensationShaRef.updateAndGet(current ->
                current != null ? current : reloadScript(LuaScripts.ATOMIC_COMPENSATION, "Compensation"));
    }
