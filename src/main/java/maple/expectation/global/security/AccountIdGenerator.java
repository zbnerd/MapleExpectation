 ...
 * 넥슨 계정 식별자 생성기
 *
 * <p>동일 넥슨 계정은 API Key나 로그인 캐릭터가 달라도
 * 항상 동일한 myOcids(소유 캐릭터 목록)를 반환합니다.
 * 이를 정렬 후 SHA-256 해싱하여 안정적인 계정 식별자를 생성합니다.</p>
 *
 * <p>용도: 좋아요 중복 방지 키 (동일 계정 = 동일 accountId)</p>
 */
@Component
...
public class AccountIdGenerator {
    ...
    public String generate(Set<String> myOcids) {
        String canonical = myOcids.stream()
                .sorted()
