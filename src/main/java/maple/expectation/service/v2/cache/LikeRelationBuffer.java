 ...
 * <ul>
 *   <li>L1 (Caffeine): 로컬 버퍼 - 빠른 중복 체크</li>
 *   <li>L2 (Redis): 글로벌 버퍼 - 분산 환경 중복 체크</li>
 *   <li>L3 (DB): 영구 저장 - 배치 동기화</li>
 * </ul>
 * </p>
 *
 * <p>흐름:
 * <ol>
 *   <li>요청 → L1 체크 → L2 체크 + 추가 (원자적)</li>
 *   <li>스케줄러 → L1 → L2 동기화</li>
 *   <li>스케줄러 → L2 → DB 배치 동기화</li>
 * </ol>
 * </p>
 *
...
public class LikeRelationBuffer implements LikeRelationBufferStrategy {
     ...
     */
    @Getter
    private final ConcurrentHashMap<String, Boolean> localPendingSet;

    private final RedissonClient redissonClient;
    private final LogicExecutor executor;

    public LikeRelationBuffer(RedissonClient redissonClient, LogicExecutor executor, MeterRegistry registry) {
     ...
     * <p>흐름:
     * <ol>
     *   <li>L1 (Caffeine) 빠른 체크 - 있으면 즉시 false</li>
     *   <li>L2 (Redis) RSet.add() - 원자적 중복 검사</li>
     *   <li>L1에 추가 + Pending Set에 등록</li>
     * </ol>
     * </p>
     *
    ...
    public Boolean addRelation(String fingerprint, String targetOcid) {
        ...
        // 2. L2 원자적 중복 검사 + 추가
        Boolean isNew = executor.executeOrDefault(
    ...
    /**
     * L1 Pending → L2 동기화 (스케줄러 호출용)
     * 기존 LikeSyncService.flushLocalToRedis()와 동일 패턴
     */
    public void flushLocalToRedis() {
        ...
        localPendingSet.forEach((relationKey, value) -> {
