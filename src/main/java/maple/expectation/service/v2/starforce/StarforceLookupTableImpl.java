 ...
 * <ul>
 *   <li>실패 시 하락 없음 (유지 또는 파괴)</li>
 *   <li>파괴 시 12성으로 리셋 (전승)</li>
 *   <li>마르코프 체인 기대값 계산</li>
 * </ul>
 *
 * <h3>기대값 계산식</h3>
 * <pre>
 ...
 *   <li>썬데이메이플: 파괴율 30% 감소 (15-21성만)</li>
 *   <li>30% 할인: 강화비용 30% 할인</li>
 *   <li>파괴방지 (15-17성): 비용 3배, 파괴율 0%</li>
 * </ul>
 */
@Slf4j
@Component
...
public class StarforceLookupTableImpl implements StarforceLookupTable {
    ...
    public void initialize() {
        if (initialized.compareAndSet(false, true)) {
            ...
            log.info("[Starforce] Lookup table initialized (Markov Chain). Cache size: {}", expectedCostCache.size());
        }
    }
    ...
    public BigDecimal getExpectedCost(int currentStar, int targetStar, int itemLevel,
        ...
        // 캐시 키
        String key = cacheKey(currentStar, targetStar, itemLevel, useStarCatch, useSundayMaple, useDiscount, useDestroyPrevention);
        ...
        BigDecimal result = computeMarkovExpectedCost(
                currentStar, targetStar, itemLevel,
                useStarCatch, useSundayMaple, useDiscount, useDestroyPrevention);
     ...
     *
     * E[s] = a[s]*E[12] + b[s] 형태로 표현 후,
     * E[12] = b[12] / (1 - a[12])로 닫아서 해결
     */
    private BigDecimal computeMarkovExpectedCost(int currentStar, int targetStar, int itemLevel,
                                                   boolean useStarCatch, boolean useSundayMaple,
                                                   boolean useDiscount, boolean useDestroyPrevention) {
        int T = targetStar;
        ...
        // T-1부터 0까지 역순으로 계산
        for (int s = T - 1; s >= 0; s--) {
            double[] params = getStageParams(s, itemLevel, useStarCatch, useSundayMaple, useDiscount, useDestroyPrevention);
     ...
     */
    private double[] getStageParams(int star, int itemLevel,
                                     boolean useStarCatch, boolean useSundayMaple,
                                     boolean useDiscount, boolean useDestroyPrevention) {
    ...
    public BigDecimal getExpectedDestroyCount(int currentStar, int targetStar,
        ...
        for (int s = T - 1; s >= 0; s--) {
            double[] params = getStageParams(s, 200, useStarCatch, useSundayMaple, false, useDestroyPrevention);
            double p = params[0];
            double d = params[2];
    ...
    private void computeAndCache(int currentStar, int targetStar, int level,
                                  boolean starCatch, boolean sunday, boolean discount, boolean destroyPrev) {
        String key = cacheKey(currentStar, targetStar, level, starCatch, sunday, discount, destroyPrev);
        BigDecimal cost = computeMarkovExpectedCost(currentStar, targetStar, level, starCatch, sunday, discount, destroyPrev);
