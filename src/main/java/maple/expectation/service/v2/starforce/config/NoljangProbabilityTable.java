 ...
 * <h3>놀장 특성</h3>
 * <ul>
 *   <li>최대 15성까지만 강화 가능</li>
 *   <li>파괴 없음 - 실패 시 현금 비용만 발생</li>
 *   <li>실패 비용: 9,400원 (캐시)</li>
 *   <li>보호권 12성부터 사용 불가</li>
 * </ul>
 *
 ...
 * <h3>비용 구조</h3>
 * <ul>
 *   <li>강화 비용: 일반 스타포스와 동일 (메소)</li>
 *   <li>실패 비용: 9,400원 (캐시, 게임 내 약 150,000,000 메소 환산)</li>
 * </ul>
 */
public final class NoljangProbabilityTable {
     ...
     * <p>9,400원 × 16,000 = 150,400,000 메소</p>
     */
     ...
     * 놀장 실패 비용 (메소 환산)
     */
    public static final BigDecimal FAIL_COST_MESO = BigDecimal.valueOf(FAIL_CASH_COST_KRW * CASH_TO_MESO_RATE);

    /**
     * 놀장 성공 확률 테이블 (star 0~14, 스타캐치 미적용)
     *
     * <p>인덱스 n = n성에서 n+1성으로 강화할 때의 성공 확률</p>
     */
     ...
     *
     * <h3>놀장 기대값 공식</h3>
     * <p>E[비용] = Σ (강화비용 + 실패비용 × (1-p)/p) / p</p>
     * <p>여기서 p = 성공 확률, 파괴 확률 = 0</p>
     *
     ...
     * @return 기대 비용 (메소)
     */
    public static BigDecimal getExpectedCost(int targetStar, int itemLevel,
                                              boolean useStarCatch, boolean useDiscount) {
     ...
     * @return 기대 비용 (메소)
     */
    public static BigDecimal getExpectedCostFromStar(int currentStar, int targetStar, int itemLevel,
                                                      boolean useStarCatch, boolean useDiscount) {
        // 범위 검증
        ...
        for (int star = currentStar; star < targetStar; star++) {
            BigDecimal singleCost = computeExpectedCostForSingleStar(star, itemLevel, useStarCatch, useDiscount);
     ...
     * 단일 스타 강화 기대값 계산
     */
    private static BigDecimal computeExpectedCostForSingleStar(int star, int itemLevel,
                                                                boolean useStarCatch, boolean useDiscount) {
        // 성공 확률
        ...
        BigDecimal expectedFails = BigDecimal.valueOf(failRate).divide(BigDecimal.valueOf(successRate), MC);
