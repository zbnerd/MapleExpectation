 ...
 * <p>금융수준 안전 설계:
 * <ul>
 *   <li><b>Thread-Safe</b>: AtomicReference/AtomicBoolean으로 동시성 보장</li>
 *   <li><b>멱등성</b>: compensate()는 한 번만 실행됨</li>
 *   <li><b>Graceful Degradation</b>: 복구 실패 시 로그만 남기고 진행</li>
 * </ul>
 * </p>
 *
 * @since 2.0.0
...
public class RedisCompensationCommand implements CompensationCommand {
     ...
     */
    public RedisCompensationCommand(String sourceKey,
    ...
    public void save(FetchResult result) {
        ...
        log.debug("Compensation state saved: tempKey={}, entries={}",
                result.tempKey(), result.size());
    }
    ...
    public void compensate() {
        ...
        executor.executeOrCatch(
                () -> {
                    ...
                    log.warn("Compensation triggered: tempKey={} → sourceKey={}, entries={}",
                            result.tempKey(), sourceKey, result.size());
                    return null;
                },
                e -> {
                    // P0 FIX: 복구 실패 시 DLQ 이벤트 발행 (데이터 영구 손실 방지)
                    log.error("Compensation FAILED - Publishing DLQ event: tempKey={}, sourceKey={}, reason={}",
                            result.tempKey(), sourceKey, e.getMessage());
     ...
     * <p>금융수준 안전 설계:
     * <ul>
     *   <li>이벤트 발행 실패해도 로그에 데이터 기록</li>
     *   <li>Listener에서 파일 백업 + Discord 알림</li>
     * </ul>
     * </p>
     */
    private void publishDlqEvent(FetchResult result, Throwable cause) {
        executor.executeOrCatch(
                ...
                e -> {
                    ...
                    log.error("🚨 [CRITICAL] DLQ event publish failed! Data logged for manual recovery: {}",
                            result.data(), e);
                    return null;
                },
    ...
    public void commit() {
        ...
        executor.executeOrCatch(
                ...
                e -> {
                    ...
                    log.warn("TempKey delete failed (will expire by TTL): tempKey={}",
                            result.tempKey());
                    return null;
                },
