 ...
 * <p>Redisson RReliableTopic을 사용하여 인스턴스 간 이벤트 Fanout</p>
 * <p>RTopic(at-most-once)과 달리 인스턴스 재시작 시에도 메시지 유실 방지</p>
 *
 * <h3>5-Agent Council 합의</h3>
 * <ul>
 *   <li>Purple (Data): L1 eviction은 idempotent → 중복 수신 무해</li>
 *   <li>Purple (Data): Hash Tag {likes} 호환성 문제 없음 (RReliableTopic 내부 키도 동일 슬롯)</li>
 *   <li>Red (SRE): 메트릭으로 발행 성공/실패 + transport 태그로 구분</li>
 *   <li>Green (Performance): countSubscribers() Gauge로 구독자 수 모니터링</li>
 * </ul>
 *
 * <h3>RTopic과의 차이</h3>
 * <ul>
 *   <li>RTopic: Redis Pub/Sub → at-most-once, 수신자 없으면 유실</li>
 *   <li>RReliableTopic: Redis Stream 기반 → at-least-once, watchdog 지원</li>
 * </ul>
 *
...
public class ReliableRedisLikeEventPublisher implements LikeEventPublisher {
     ...
     *
     * <p>countSubscribers() 호출로 현재 RReliableTopic 구독자 수를 모니터링</p>
     */
    @PostConstruct
     ...
     *
     * <p>Redis Stream 장애 시에도 좋아요 기능은 정상 동작 (Graceful Degradation)</p>
     */
    @Override
    public void publish(LikeEvent event) {
        ...
        if (clientsReceived > 0) {
            ...
            log.debug("[ReliableLikeEventPublisher] Event published: userIgn={}, delta={}, clients={}",
                    event.userIgn(), event.newDelta(), clientsReceived);
        } else {
            recordPublishFailure();
            log.warn("[ReliableLikeEventPublisher] Event publish failed or no subscribers: userIgn={}",
                    event.userIgn());
        }
    }
