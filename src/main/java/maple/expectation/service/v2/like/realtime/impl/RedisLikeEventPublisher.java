 ...
 *
 * <p>Redisson RTopic을 사용하여 인스턴스 간 이벤트 Fanout
 *
 * <h3>5-Agent Council 합의</h3>
 *
 * <ul>
 *   <li>Green (Performance): RTopic은 Redis Pub/Sub 네이티브 래퍼 → 오버헤드 최소
 *   <li>Red (SRE): 메트릭으로 발행 성공/실패 모니터링
 *   <li>Purple (Data): Hash Tag {likes}로 클러스터 슬롯 보장
 * </ul>
 *
...
public class RedisLikeEventPublisher implements LikeEventPublisher {
  ...
  public void publish(LikeEvent event) {
    ...
    if (clientsReceived > 0) {
      ...
      log.debug(
          "[LikeEventPublisher] Event published: userIgn={}, delta={}, clients={}",
          event.userIgn(),
          event.newDelta(),
          clientsReceived);
    } else {
      ...
      log.warn(
          "[LikeEventPublisher] Event publish failed or no subscribers: userIgn={}",
          event.userIgn());
    }
  }
