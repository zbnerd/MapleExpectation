 ...
 * <p>RReliableTopic: Redis Stream 기반으로 인스턴스 재시작 시에도 메시지 유실 방지</p>
 *
 * <h3>캐시 무효화 전략 (5-Agent Council 합의)</h3>
 * <ul>
 *   <li>Pub/Sub 수신 → L1 즉시 evict</li>
 *   <li>TTL(5분)은 Fallback용 (Pub/Sub 유실 시)</li>
 *   <li>자기 자신이 발행한 이벤트는 무시 (Self-skip)</li>
 *   <li>RReliableTopic at-least-once → L1 eviction idempotent → 중복 수신 무해 (Purple 확인)</li>
 * </ul>
 *
 * <h3>RTopic과의 차이</h3>
 * <ul>
 *   <li>listenerId 타입: RTopic은 int, RReliableTopic은 String</li>
 *   <li>removeListener: RReliableTopic은 listenerId(String)로 해제</li>
 * </ul>
 *
 * <h3>Phase D: 모니터링</h3>
 * <ul>
 *   <li>Gauge: like.reliable.subscriber.active (1.0 = subscribed, 0.0 = not)</li>
 * </ul>
 *
 * <h3>CLAUDE.md Section 12: LogicExecutor 패턴</h3>
 * <p>모든 캐시/Redis 작업은 executeVoid로 예외 처리</p>
 *
...
public class ReliableRedisLikeEventSubscriber implements LikeEventSubscriber {
    ...
    public void subscribe() {
        ...
        executor.executeVoid(() -> {
            topic = redissonClient.getReliableTopic(
            ...
            log.info("[ReliableLikeEventSubscriber] Subscribed to reliable topic: {}, instanceId={}",
                    RedisKey.LIKE_EVENTS_RELIABLE_TOPIC.getKey(), instanceId);
        }, context);
    }
     ...
     * 이벤트 수신 및 처리
     *
     * <p>Self-skip: 자기가 발행한 이벤트는 무시</p>
     * <p>at-least-once: 중복 수신 가능하지만 L1 eviction은 idempotent</p>
     */
    @Override
    public void onEvent(LikeEvent event) {
        ...
        executor.executeVoid(() -> {
            ...
            recordEventReceived();
            log.debug("[ReliableLikeEventSubscriber] L1 cache evicted: userIgn={}, source={}, delta={}",
                    event.userIgn(), event.sourceInstanceId(), event.newDelta());
        }, context);
    }
     ...
     *
     * <p>TieredCacheManager.getL1CacheDirect()로 L1만 직접 evict</p>
     * <p>L2(Redis)는 모든 인스턴스가 공유하므로 evict 불필요</p>
     */
     ...
     * <p>@PreDestroy에서 명시적 해제 + 상태 플래그 업데이트</p>
     */
    @Override
    @PreDestroy
    public void unsubscribe() {
        ...
        executor.executeVoid(() -> {
            if (topic != null && listenerId != null) {
                ...
                log.info("[ReliableLikeEventSubscriber] Unsubscribed from reliable topic: instanceId={}",
                        instanceId);
            }
        }, context);
    }
    ...
    private void registerSubscriberGauge() {
        meterRegistry.gauge("like.reliable.subscriber.active", subscribed,
                b -> b.get() ? 1.0 : 0.0);
    }

    private void recordEventReceived() {
        meterRegistry.counter("like.event.received",
