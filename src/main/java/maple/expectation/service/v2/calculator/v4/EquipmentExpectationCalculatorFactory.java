 ...
 *
 * <h3>역할</h3>
 * <p>장비 입력을 기반으로 적절한 Decorator 체인을 구성합니다.</p>
 *
...
public class EquipmentExpectationCalculatorFactory {
    ...
    public EquipmentExpectationCalculator createFullCalculator(EquipmentCalculationInput input) {
        ...
        if (input.hasPotential()) {
            ...
            calculator = new BlackCubeDecoratorV4(
                    calculator, trialsProvider, costPolicy, potentialInput
            );
        }
        ...
        if (input.hasAdditionalPotential()) {
            ...
            calculator = new AdditionalCubeDecoratorV4(
                    calculator, trialsProvider, costPolicy, additionalInput
            );
        }

        // 4. 스타포스 - 스타포스 정보가 있는 경우에만
        if (input.hasStarforce()) {
            calculator = new StarforceDecoratorV4(
                    calculator,
