 ...
 * <p>분산 환경에서의 중복 처리 방지와 격리성 보장을 위해 2단계 트랜잭션을 사용합니다.</p>
 *
 * <h4>Phase 1: fetchAndLock() (단일 트랜잭션)</h4>
 * <ul>
 *   <li>SKIP LOCKED로 PENDING/FAILED 항목 조회</li>
 *   <li>즉시 PROCESSING 상태로 변경하여 중복 폴링 방지</li>
 *   <li>트랜잭션 종료 시 락 해제되지만 상태가 유지되어 안전함</li>
 * </ul>
 *
 * <h4>Phase 2: processBatch() (항목별 독립 트랜잭션)</h4>
 * <ul>
 *   <li>각 항목을 TransactionTemplate으로 독립 트랜잭션에서 처리</li>
 *   <li>개별 실패가 전체 배치에 영향을 주지 않음 (P0-2 Fix)</li>
 *   <li>실패 시 handleFailure()로 retryCount 증가 -> DLQ 이동 (P0-1 Fix)</li>
 * </ul>
 *
 * <h3>Financial-Grade 특성</h3>
 * <ul>
 *   <li>SKIP LOCKED: 분산 환경 중복 처리 방지</li>
 *   <li>Exponential Backoff: 재시도 간격 증가 (최대 1시간)</li>
 *   <li>Stalled Recovery: JVM 크래시로 Zombie 상태 복구</li>
 *   <li>Integrity Verification: Content Hash 기반 무결성 검증</li>
 * </ul>
 *
 * <h3>Nexon API 특화</h3>
 * <ul>
 *   <li>외부 API 장애 시 Outbox 적재 -> 재시도</li>
 *   <li>6시간 장애 대응 (N19 시나리오)</li>
 *   <li>NexonApiRetryClient로 실제 API 호출 위임</li>
 * </ul>
 *
...
public class NexonApiOutboxProcessor {
    ...
    public void pollAndProcess() {
        TaskContext context = TaskContext.of("NexonApiOutbox", "PollAndProcess", properties.getInstanceId());
     ...
     *
     * <h4>인덱스 활용</h4>
     * <p>idx_pending_poll (status, next_retry_at, id) 복합 인덱스 사용</p>
     */
     ...
     *
     * <p>개별 항목 실패가 전체 배치에 영향을 주지 않음</p>
     */
     ...
     * <h4>처리 흐름</h4>
     * <ol>
     *   <li>무결성 검증 (Content Hash)</li>
     *   <li>NexonApiRetryClient로 실제 API 호출</li>
     *   <li>성공 시 markCompleted()</li>
     *   <li>실패 시 예외 발생 -> executeOrCatch가 handleFailure() 호출</li>
     * </ol>
     */
     ...
     * 무결성 검증
     *
     * <p>Content Hash를 재계산하여 데이터 위변조 검증</p>
     */
    ...
    private void recoverFailedEntry(Long entryId, String errorMessage) {
        TaskContext context = TaskContext.of("NexonApiOutbox", "RecoverFailed", String.valueOf(entryId));
     ...
     * 처리 실패 핸들링
     *
     * <p>retryCount 증가 -> maxRetries 도달 시 DLQ 이동</p>
     */
    public void handleFailure(NexonApiOutbox entry, String error) {
        ...
        if (entry.shouldMoveToDlq()) {
            log.warn("[NexonApiOutbox] DLQ 이동: requestId={}, retryCount={}", 
                    entry.getRequestId(), entry.getRetryCount());
     ...
     *
     * <p>복구 전 Content Hash 기반 무결성 검증 수행</p>
     *
     * <h4>인덱스 활용</h4>
     * <p>idx_locked (locked_by, locked_at) 인덱스 사용</p>
     */
    ...
    public void recoverStalled() {
        ...
        List<NexonApiOutbox> stalledEntries = outboxRepository.findStalledProcessing(
                staleTime,
                PageRequest.of(0, properties.getBatchSize())
        );
        ...
        for (NexonApiOutbox entry : stalledEntries) {
            if (!verifyIntegrity(entry)) {
                log.error("[NexonApiOutbox] 무결성 검증 실패 - Zombie 복구 중단, DLQ 이동: requestId={}",
                        entry.getRequestId());
                handleIntegrityFailure(entry);
                integrityFailed++;
                continue;
