 ...
 * 직업명 → 주스탯/부스탯 매핑 유틸리티
 *
 * <p>Nexon API의 {@code character_class} 문자열로부터 직업 가중치(JobWeights)를 결정합니다.</p>
 */
public final class JobStatMapping {
    ...
    /** STR 주스탯 + DEX 부스탯 */
    private static final Set<String> STR_DEX_JOBS = Set.of(
            "히어로", "팔라딘", "다크나이트",
            "소울마스터", "미하일", "블래스터",
            "데몬슬레이어", "아란", "아델", "카이저", "제로",
            "바이퍼", "캐논슈터", "스트라이커", "아크", "은월", "제트"
    );

    /** DEX 주스탯 + STR 부스탯 */
    private static final Set<String> DEX_STR_JOBS = Set.of(
            "보우마스터", "신궁", "패스파인더",
            "윈드브레이커", "와일드헌터", "카인", "메르세데스",
            "캡틴", "메카닉", "엔젤릭버스터"
    );

    /** INT 주스탯 + LUK 부스탯 */
    private static final Set<String> INT_LUK_JOBS = Set.of(
    ...
    /** LUK 주스탯 + DEX 부스탯 */
    private static final Set<String> LUK_DEX_JOBS = Set.of(
            "나이트로드", "나이트워커", "팬텀", "칼리", "호영"
    );

    /** LUK 주스탯 + STR,DEX 부스탯 2개 */
    private static final Set<String> LUK_STR_DEX_JOBS = Set.of(
            "섀도어", "듀얼블레이드", "카데나"
    );
