 ...
 *
 * <p>장비에 현재 붙어 있는 추옵 스탯을 직업 가중치로 환산하여
 * 불꽃 DP 계산의 {@code target} 파라미터로 사용합니다.</p>
 *
 * <p>환산치 = str*wStr + dex*wDex + int*wInt + luk*wLuk
 *   + hp*wHp + allstat%*wAllstatPct + att*wAtt + mag*wMag
 *   + dmg%*wDmgPct + bossDmg%*wBossDmgPct</p>
 */
@Component
public class FlameScoreResolver {
     ...
     */
    public int calculate(int addStr, int addDex, int addInt, int addLuk,
                         int addHp, int addAllStat,
                         int addAtt, int addMag,
                         int addDmg, int addBossDmg,
                         JobWeights weights) {
