 ...
 * 환생의 불꽃 입력 해석기
 *
 * <p>장비 데이터(CubeCalculationInput)와 직업명으로부터
 * 불꽃 기대값 계산에 필요한 5가지 입력을 추출합니다:</p>
 * <ol>
 *   <li>보스 장비 여부</li>
 *   <li>무기 여부</li>
 *   <li>추옵 환산치 (목표값)</li>
 *   <li>무기 기본 공/마</li>
 *   <li>직업 가중치 (JobWeights)</li>
 * </ol>
 */
@Component
...
public class FlameInputResolver {
    ...
    public FlameInput resolve(CubeCalculationInput input, String characterClass) {
        ...
        int target = scoreResolver.calculate(
                input.getAddOptionStr(), input.getAddOptionDex(),
                input.getAddOptionInt(), input.getAddOptionLuk(),
                input.getAddOptionMaxHp(), input.getAddOptionAllStat(),
                input.getAddOptionAtt(), input.getAddOptionMag(),
                input.getAddOptionDmg(), input.getAddOptionBossDmg(),
                weights
        );
