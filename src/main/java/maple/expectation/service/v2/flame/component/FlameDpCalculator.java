 ...
 *
 * <h3>핵심 알고리즘</h3>
 * <p>"옵션 종류 중복 없이 k개 선택"을 조합-평균 DP로 처리한다.</p>
 * <p>dp[r][t] = 앞에서 처리한 옵션들 중 r개를 선택해서 캡 점수 t가 되는 확률질량의 합</p>
 *
 * <h3>보스 장비: k=4 고정</h3>
 * <p>P = dp[4][T] / C(N,4)</p>
 *
 * <h3>그외 장비: k=1~4 균등</h3>
 * <p>P = (1/4) * sum(dp[k][T] / C(N,k)) for k=1..4</p>
 *
...
public class FlameDpCalculator {
    ...
    @Cacheable(
        value = "flameTrials",
        key = "#category.name() + ':' + #flameType.name() + ':' + #level + ':' + "
    ...
    public Double calculateExpectedTrials(
        ...
        List<Map<Integer, Double>> optionPmfs = scoreCalculator.buildOptionPmfs(
        ...
        double successProb = category.isBossDrop()
     ...
     */
    private double calculateProbForUniformK(List<Map<Integer, Double>> optionPmfs, int target, int n) {
     ...
     * 핵심 DP 실행
     *
     * <p>dp[r][t] = r개 옵션 선택했을 때 캡 점수 t의 확률질량 합</p>
     * <p>캡핑: t = min(T, t + val)</p>
     */
