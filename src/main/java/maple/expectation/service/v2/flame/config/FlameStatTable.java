 ...
 * 환생의 불꽃 추가옵션 수치 테이블
 *
 * <p>장비 레벨과 단계(stage 1-7)에 따른 추가옵션 수치를 정의한다.
 * 방어구/악세서리와 무기는 서로 다른 테이블을 사용하며,
 * 무기의 ATT/MAG는 기본 공격력 기반 공식으로 계산한다.</p>
 *
 * <p>레벨 버킷 매핑: 130/135 -> 130, 140/145 -> 140, 나머지는 동일</p>
 */
public final class FlameStatTable {
    ...
    /**
     * armor[level][optionType] = Integer[7] (stage 1-7, 0-indexed)
     * null element = 해당 단계 미존재
     */
    private static final NavigableMap<Integer, Map<FlameOptionType, Integer[]>> ARMOR_TABLE = new TreeMap<>();
     ...
     * 무기 공격력/마력 보너스 계산
     *
     * <p>공식: floor((level / 40 + 1) * stage * (1 + 0.1 * (stage - 3)) * baseAtt)</p>
     *
     * @param level   장비 레벨
     * @param stage   단계 (1-7)
     ...
     *
     * <p>무기의 ATT/MAG는 {@link #weaponAttBonus}를 사용하고,
     * BOSS_DMG_PCT는 {@link #weaponBossDmgPct}를 사용한다.
     * 나머지 옵션은 방어구 테이블과 동일하다.</p>
     *
     * @param option 옵션 타입
     * @param level  장비 레벨
     * @param stage  단계 (1-7)
    ...
    private static Integer[] stages(Integer... values) {
        if (values.length != STAGES) {
            throw new IllegalArgumentException("Expected " + STAGES + " stage values, got " + values.length);
        }
        return values;
    }
