 ...
 * <h3>핵심 책임</h3>
 * <ul>
 *   <li>확률 테이블에서 슬롯별 분포 추출</li>
 *   <li>테이블 버전 고정 (TOCTOU 방지)</li>
 *   <li>질량 검증 및 정규화</li>
 * </ul>
 *
 * <h3>불변식 (사후조건)</h3>
 * <ul>
 *   <li>Σ probs = 1 ± MASS_TOLERANCE</li>
 *   <li>모든 probs ∈ [0, 1]</li>
 *   <li>values는 0 포함 (타깃 스탯 아닌 옵션)</li>
 * </ul>
 *
 * <h3>안 A 채택</h3>
 * <p>루프에서 contribution=0 포함 전체 merge → 추가 0버킷 보정 불필요</p>
 */
@Slf4j
@Component
...
public class SlotDistributionBuilder {
     ...
     *
     * <p>테이블 버전을 고정하여 계산 중 테이블 변경으로 인한
     * 불일치를 방지합니다.</p>
     *
    ...
    public SparsePmf buildDistributionByVersion(
            CubeType cubeType, int level, String part,
            String grade, int slot, StatType targetStat,
            String tableVersion
    ) {
    ...
    private SparsePmf doBuildAndValidate(
        ...
        List<CubeProbability> probs = repository.findProbabilitiesByVersion(
    ...
    private Map<Integer, Double> buildDistributionMap(
        ...
        for (CubeProbability p : probs) {
            int contribution = extractor.extractContributionFor(
                    p.getOptionName(), targetStat
            );
     ...
     * PMF 불변식 검증
     *
     * <p>DoD 1e-12 기준 충족을 위해 Kahan summation 사용</p>
     *
     * @param pmf 검증 대상
     * @throws ProbabilityInvariantException 불변식 위반 시
     */
