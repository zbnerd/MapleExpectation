 ...
 *
 * <h3>핵심 역할</h3>
 * <p>@Cacheable self-invocation 방지를 위한 별도 Bean으로 분리하여 Spring 프록시 적용</p>
 *
 * <h3>캐시 키 구성</h3>
 * <ul>
 *   <li>type + level + part + grade + targetStatType + minTotal + enableTailClamp + tableVersion</li>
 *   <li>tableVersion 포함으로 테이블 갱신 시 과거 확률 캐시 방지</li>
 * </ul>
 *
...
public class CubeDpCalculator {
     ...
     *
     * <p>캐시 키에 tableVersion이 포함되어 테이블 갱신 시 과거 확률 캐시를 방지합니다.</p>
     *
    ...
    private List<SparsePmf> buildSlotDistributions(
        ...
        return IntStream.rangeClosed(1, slotCount)
                .mapToObj(slot -> distributionBuilder.buildDistributionByVersion(
                        type, input.getLevel(), input.getPart(),
                        input.getGrade(), slot, input.getTargetStatType(),
                        tableVersion
                ))
                .toList();
    }
