 ...
 * <h3>타입 분리 설계</h3>
 * <ul>
 *   <li>입력: List&lt;SparsePmf&gt; (희소, K가 작음)</li>
 *   <li>출력: DensePmf (밀집, 인덱스=값)</li>
 * </ul>
 *
 * <h3>Tail Clamp 전략</h3>
 * <ul>
 *   <li>인덱스는 0..target</li>
 *   <li>합이 target 초과 시 모두 target 버킷에 누적</li>
 *   <li>결과적으로 O(slots × target × K) 보장</li>
 * </ul>
 */
@Component
...
public class ProbabilityConvolver {
     ...
     * <p>사후조건:</p>
     * <ul>
     *   <li>질량 보존: Σ=1 ± MASS_TOLERANCE</li>
     *   <li>NaN/Inf 없음</li>
     *   <li>enableTailClamp=true면 상태 크기 = target+1</li>
     * </ul>
     *
    ...
    private DensePmf doConvolveWithClamp(List<SparsePmf> slotPmfs, int target, boolean enableTailClamp) {
    ...
    private int calculateMaxSum(List<SparsePmf> slotPmfs) {
        return slotPmfs.stream()
                .mapToInt(SparsePmf::maxValue)
                .sum();
    }
