 ...
 * <h3>정책 B 적용</h3>
 * <ul>
 *   <li>직접 매칭: STR +12% → targetStatType=STR_PERCENT면 기여 12</li>
 *   <li>ALLSTAT 기여: 올스탯 +9% → targetStatType=STR_PERCENT면 기여 9</li>
 *   <li>불일치: DEX +12% → targetStatType=STR_PERCENT면 기여 0</li>
 * </ul>
 */
@Component
...
public class StatValueExtractor {
     ...
     * <p>예시:</p>
     * <ul>
     *   <li>"STR/DEX +6%" → [StatContribution(STR_PERCENT, 6), StatContribution(DEX_PERCENT, 6)]</li>
     *   <li>"올스탯 +9%" → [StatContribution(ALLSTAT_PERCENT, 9)]</li>
     * </ul>
     *
     ...
     *
     * <p>P0-1: findAllTypesOrEmpty() 사용 → 프록 옵션은 빈 리스트</p>
     * <p>P0-2: Primary stat처럼 보이는데 매칭 실패 → Fail-Fast</p>
     */
    private List<StatContribution> doExtractAll(String optionName) {
        ...
        return types.stream()
                .map(type -> new StatContribution(type, value))
                .toList();
    }
     ...
     * <h3>기여 규칙</h3>
     * <ul>
     *   <li>정확히 일치하는 타입 → 해당 값</li>
     *   <li>ALLSTAT_PERCENT → 개별 스탯(STR_PERCENT 등)에 동일 값 기여</li>
     *   <li>불일치 → 0</li>
     * </ul>
     *
     * @param optionName 옵션 문자열
