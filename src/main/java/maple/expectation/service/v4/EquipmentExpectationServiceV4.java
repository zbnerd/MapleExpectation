...
public class EquipmentExpectationServiceV4 {
    ...
    public CompletableFuture<EquipmentExpectationResponseV4> calculateExpectationAsync(String userIgn, boolean force) {
        return CompletableFuture.supplyAsync(
                        () -> selfProvider.getObject().calculateExpectation(userIgn, force),
                        equipmentExecutor
                )
    ...
    public byte[] getGzipExpectation(String userIgn, boolean force) {
        validateInitialized();
        return cacheCoordinator.getGzipOrCalculate(userIgn, force, () -> doCalculateExpectation(userIgn));
    }
     ...
     * <p>loadEquipmentDataAsync로 API 대기 시간 분리.
     * TieredCache Callable 내에서는 .join() 사용 (분산 락 내부이므로 스레드 점유 제한적)</p>
     */
     ...
     *
     * <p>DB에 캐시된 데이터가 있으면 즉시 반환, 없으면 API 비동기 호출</p>
     */
