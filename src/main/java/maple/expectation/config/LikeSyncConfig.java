 ...
 * <p>금융수준 안전 설계:
 * <ul>
 *   <li><b>lua (기본)</b>: Lua Script 기반 원자적 연산 (권장)</li>
 *   <li><b>rename (폴백)</b>: RENAME 기반 (Lua 미지원 환경)</li>
 * </ul>
 * </p>
 *
 * <p>설정:
...
public class LikeSyncConfig {
     ...
     *
     * <p>설정에 따라 Lua Script 또는 Rename 전략 선택</p>
     */
    @Bean
    public AtomicFetchStrategy atomicFetchStrategy(
        ...
        return switch (strategyType.toLowerCase()) {
            case STRATEGY_RENAME -> {
                ...
                yield new RenameAtomicFetchStrategy(redisTemplate, executor, meterRegistry, tempKeyTtlSeconds);
            }
            default -> {
                log.info("AtomicFetchStrategy initialized: LUA_SCRIPT (primary), TTL={}s", tempKeyTtlSeconds);
                yield new LuaScriptAtomicFetchStrategy(redissonClient, executor, meterRegistry, tempKeyTtlSeconds);
            }
        };
    }
}
