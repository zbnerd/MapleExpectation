 ...
 * <h4>설계 의도</h4>
 * <ul>
 *   <li>P1-1: backupDirectory, archiveDirectory @Value 주입 -> 생성자 주입 변경</li>
 *   <li>P1-2: Equipment 대기 타임아웃, 분산 락 설정 하드코딩 제거</li>
 *   <li>P1-3: Shutdown batch size, retry count 외부화</li>
 *   <li>P1-5: instanceId 외부 설정으로 DNS 블로킹 제거</li>
 * </ul>
 *
 * <h4>OutboxProperties 패턴 참조</h4>
 * <p>{@code @ConfigurationProperties} + {@code @Validated}로 타입 안전 바인딩</p>
 *
 * @see maple.expectation.global.shutdown.GracefulShutdownCoordinator
...
public class ShutdownProperties {
     ...
     * <p>기본값: 20초. Shutdown 시 Equipment 저장 완료 대기 최대 시간</p>
     */
    @NotNull
    private Duration equipmentAwaitTimeout = Duration.ofSeconds(20);

    /**
     * DB 동기화 분산 락 대기 시간 (초)
     * <p>Scale-out 시 다른 인스턴스와 락 경합 대기 시간</p>
     */
    @Min(1)
    @Max(30)
     ...
     */
    @NotBlank
    private String backupDirectory = "/tmp/maple-shutdown";

    /**
     * 아카이브 디렉토리 (처리 완료 파일 이동)
     */
    @NotBlank
    private String archiveDirectory = "/tmp/maple-shutdown/processed";

    /**
     * 인스턴스 식별자 (P1-5 Fix: DNS 블로킹 제거)
     * <p>Scale-out 환경에서 어떤 인스턴스의 백업인지 식별</p>
     */
    @NotBlank
