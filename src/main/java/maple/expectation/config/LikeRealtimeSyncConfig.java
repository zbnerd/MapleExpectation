 ...
 * <p>like.realtime.transport로 RTopic / RReliableTopic 전환</p>
 *
 * <h3>5-Agent Council 합의</h3>
 * <ul>
 *   <li>Blue (Architect): Strategy 패턴으로 구현체 교체 가능</li>
 *   <li>Red (SRE): ConditionalOnProperty로 런타임 비활성화 지원</li>
 *   <li>Green (Performance): 단일 토픽 인스턴스 재사용</li>
 *   <li>Yellow (QA): RTopic/RReliableTopic 교차 통신 불가 → Blue-Green 배포 필수</li>
 *   <li>Purple (Data): RReliableTopic at-least-once + L1 eviction idempotent → 중복 수신 무해</li>
 * </ul>
 *
...
public class LikeRealtimeSyncConfig {
     ...
     *
     * <p>like.realtime.transport=rtopic 또는 미설정 시 활성화</p>
     */
    @Configuration
    @RequiredArgsConstructor
    @ConditionalOnProperty(name = "like.realtime.transport", havingValue = "rtopic", matchIfMissing = true)
     ...
     *
     * <p>like.realtime.transport=reliable-topic 시 활성화</p>
     *
     * <h4>주의: Blue-Green 배포 필수</h4>
     * <p>RTopic과 RReliableTopic은 Redis 구조가 다르므로
     * 롤링 배포 시 교차 통신 불가. 전체 동시 전환 필요.</p>
     */
    @Configuration
    @RequiredArgsConstructor
    ...
    static class ReliableTopicConfig {
        ...
        public LikeEventSubscriber likeEventSubscriber() {
            ...
            return new ReliableRedisLikeEventSubscriber(redissonClient, cacheManager, executor, meterRegistry);
        }
    }
}
