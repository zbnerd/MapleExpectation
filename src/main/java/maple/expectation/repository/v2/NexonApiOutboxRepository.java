 ...
 *   <li>@Version 낙관적 락 병행하여 강한 일관성 보장</li>
 * </ul>
 *
 * @see maple.expectation.domain.v2.NexonApiOutbox
 * @see <a href="../../../../docs/01_Chaos_Engineering/06_Nightmare/Scenarios/N19-outbox-replay.md">N19 Scenario</a>
 */
public interface NexonApiOutboxRepository extends JpaRepository<NexonApiOutbox, Long> {
     ...
     *
     * <h4>인덱스 활용</h4>
     * <p>idx_pending_poll (status, next_retry_at, id) 복합 인덱스 활용.</p>
     *
     ...
     * <p>복수 인스턴스가 동시 실행 가능하므로 {@link #findStalledProcessing(LocalDateTime, Pageable)}
     *와 함께 사용하여 SKIP LOCKED로 중복 방지.</p>
     *
     * @param staleTime Stale 판정 기준 시간
     * @return 업데이트된 행 수
     */
    @Modifying
    @Query("UPDATE NexonApiOutbox o SET o.status = 'PENDING', o.lockedBy = NULL, o.lockedAt = NULL " +
     ...
     *
     * <h4>인덱스 활용</h4>
     * <p>idx_locked (locked_by, locked_at) 인덱스 활용.</p>
     *
     * @param staleTime Stale 판정 기준 시간
    ...
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @QueryHints(@QueryHint(name = "jakarta.persistence.lock.timeout", value = "-2"))
    @Query("SELECT o FROM NexonApiOutbox o WHERE o.status = 'PROCESSING' AND o.lockedAt < :staleTime ORDER BY o.id")
    List<NexonApiOutbox> findStalledProcessing(@Param("staleTime") LocalDateTime staleTime, Pageable pageable);
