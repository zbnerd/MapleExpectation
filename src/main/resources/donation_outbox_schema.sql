-- ============================================
-- Donation Outbox Schema (Issue #80)
-- Financial-Grade Transactional Outbox Pattern
-- ============================================

-- Outbox Table
CREATE TABLE IF NOT EXISTS donation_outbox (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    version BIGINT DEFAULT 0,
    request_id VARCHAR(50) NOT NULL UNIQUE,
    event_type VARCHAR(50) NOT NULL,
    payload TEXT NOT NULL,
    content_hash VARCHAR(64) NOT NULL,
    status ENUM('PENDING', 'PROCESSING', 'COMPLETED', 'FAILED', 'DEAD_LETTER')
           NOT NULL DEFAULT 'PENDING',
    locked_by VARCHAR(100) NULL,
    locked_at DATETIME NULL,
    retry_count INT DEFAULT 0,
    max_retries INT DEFAULT 3,
    last_error VARCHAR(500),
    next_retry_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    created_at DATETIME(6) DEFAULT CURRENT_TIMESTAMP(6),
    updated_at DATETIME(6) DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6),

    -- SKIP LOCKED optimization index
    INDEX idx_pending_poll (status, next_retry_at, id),
    INDEX idx_locked (locked_by, locked_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- DLQ Table (Triple Safety Net 1st layer)
CREATE TABLE IF NOT EXISTS donation_dlq (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    original_outbox_id BIGINT NOT NULL,
    request_id VARCHAR(50) NOT NULL,
    event_type VARCHAR(50) NOT NULL,
    payload TEXT NOT NULL,
    failure_reason VARCHAR(500),
    moved_at DATETIME(6) DEFAULT CURRENT_TIMESTAMP(6),

    INDEX idx_dlq_moved_at (moved_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
