 ...
 *
 * <ul>
 *   <li>🔴 Red (SRE): 장애 주입 - JVM 크래시 시뮬레이션으로 PROCESSING 상태 고착
 *   <li>🔵 Blue (Architect): 흐름 검증 - Outbox 상태 전이 및 복구 로직
 *   <li>🟢 Green (Performance): 메트릭 검증 - Stalled 메시지 수, 복구 시간
 *   <li>🟣 Purple (Auditor): 데이터 무결성 - 메시지 손실 없음 확인
 *   <li>🟡 Yellow (QA Master): 테스트 전략 - Zombie 메시지 발견 시 P0 Issue 생성
 * </ul>
 *
 * <h4>실제 서비스 사용</h4>
 *
 * <p>이 테스트는 실제 DonationService.sendCoffee()를 통해 Outbox를 생성합니다. 임의로 SQL INSERT를 사용하지 않고, 실제 운영 환경과
 * 동일한 흐름을 검증합니다.
 *
 * <h4>예상 결과: CONDITIONAL PASS</h4>
 *
 * <p>PROCESSING 상태에서 JVM 크래시 시 메시지가 Zombie화. recoverStalled() 메서드가 정상 작동하면 복구됨.
 *
 * <h4>관련 CS 원리</h4>
 *
 * <ul>
 *   <li>Outbox Pattern: 분산 트랜잭션 대안
 *   <li>Idempotency: 중복 처리 방지
 *   <li>At-Least-Once Delivery: 최소 1회 전달 보장
 *   <li>Stale Processing: 오래된 작업 상태 감지
 *   <li>Heartbeat: 작업 생존 신호
 * </ul>
 *
...
class ZombieOutboxNightmareTest extends IntegrationTestSupport {
   ...
   *
   * <ol>
   *   <li>DonationService.sendCoffee()로 실제 Outbox 생성
   *   <li>Outbox 상태를 PROCESSING으로 변경하고 processed_at을 10분 전으로 설정 (JVM 크래시 시뮬레이션)
   *   <li>recoverStalled() 호출
   *   <li>상태가 PENDING 또는 FAILED로 복구되는지 확인
   * </ol>
   *
   * <p><b>성공 기준</b>: PROCESSING 상태가 복구됨
   *
   * <p><b>실패 조건</b>: PROCESSING 상태 유지 → Zombie 메시지
   */
  @Test
  ...
  void shouldRecoverZombieOutbox_createdByActualDonationService() throws Exception {
    ...
    if (recovered != null) {
      ...
      log.info(
          "│ Final Status: {}                                           │", recovered.getStatus());
      log.info(
          "│ Retry Count: {}                                            │",
      ...
      if (isRecovered) {
        ...
        log.info(
            "│ ❌ ZOMBIE STILL ALIVE! Status: {}                          │", recovered.getStatus());
      }
    } else {
  ...
  void shouldRecoverMultipleZombies_createdByRealDonations() throws Exception {
    ...
    log.info(
        "│ Still Zombies: {}                                          │",
        zombieCount - recoveredCount);
    log.info("└────────────────────────────────────────────────────────────┘");

    assertThat(recoveredCount).as("All stalled entries should be recovered").isEqualTo(zombieCount);
  }

  /**
   * 🟢 Green's Test 3: Zombie 감지 임계값 검증 (5분 기준)
   *
   * <p>STALE_THRESHOLD(5분)이 적절한지 검증
   */
  @Test
  ...
  void shouldMaintainDataIntegrity_afterZombieRecovery() throws Exception {
    ...
    if (recovered != null) {
      ...
      log.info(
          "│ Payload Intact: {}                                         │",
          payloadIntact ? "✅ YES" : "❌ NO");
      log.info(
          "│ Event Type Intact: {}                                      │",
          eventTypeIntact ? "✅ YES" : "❌ NO");
      log.info(
          "│ Content Hash Valid: {}                                     │",
          hashValid ? "✅ YES" : "❌ NO");
      log.info(
          "│ Recovery Status: {}                                        │", recovered.getStatus());

      assertThat(payloadIntact).as("Payload should remain intact after recovery").isTrue();
      assertThat(eventTypeIntact).as("Event type should remain intact after recovery").isTrue();
    } else {
   ...
   *
   * <p>실제 JVM 크래시 시 OutboxProcessor가 PROCESSING 상태로 변경한 후 완료 처리 전에 크래시되는 상황을 재현합니다.
   */
