...
| Individual Tests | 3 |
| Actual DB Queries | **1** (for 1000 requests) |
| Cache Hit Rate | 98% after recovery |
...
| 용어 | 정의 | 예시 |
|------|------|------|
| **Thundering Herd (우 herd)** | 동시에 발생한 대규모 캐시 미스로 인한 DB 쿼리 폭증 | 1,000명 동시 요청 시 Single DB 쿼리 |
| **Singleflight Pattern** | 여러 요청 중 하나만 실행하고 결과를 공유하는 패턴 | Redisson Lock + Double-check 패턴 |
| **Cache Stampede** | 캐시 무효화 후 동시에 발생한 요청으로 인한 서버 과부하 | Redis FLUSHALL → 1,000명 동시 요청 |
| **Double-check Pattern** | 캐시 조회 후 다시 확인하여 중복 작업 방지 | L1 → L2 → DB 순서로 재확인 |
| **MTTD (Mean Time To Detect)** | 장애 발생부터 감지까지의 평균 시간 | 0.1s (Singleflight lock 획득) |
| **MTTR (Mean Time To Recovery)** | 장애 감지부터 복구 완료까지의 평균 시간 | 2.0s (전체 시스템 복구) |

## Grafana Dashboards

### 모니터링 대시보드
- **Cache Hit Rate**: `http://localhost:3000/d/cache-hit-rate` (Evidence: METRIC M1)
- **DB Query Rate**: `http://localhost:3000/d/db-query-metrics` (Evidence: METRIC M2)
- **HikariCP Connection Pool**: `http://localhost:3000/d/hikaricp-pool`

### 주요 패널
1. **Cache Hit Rate Drop**: 테스트 시 0%까지 급격히 하락 (Evidence: METRIC M1)
2. **DB Query Spike**: 동시 요청 시 DB 쿼리 수 증가 (Evidence: METRIC M2)
3. **Connection Pool Usage**: Connection Pool 상태 모니터링 (Evidence: METRIC M3)
4. **Singleflight Lock Count**: 락 경합 횟수 모니터링

## 생성된 이슈

- **Priority**: P1 (High)
- **Title**: [N01][Nightmare] Thundering Herd 방지 Singleflight 패턴 검증

## Fail If Wrong (문서 무효 조건)

이 문서는 다음 조건에서 **즉시 폐기**해야 합니다:

1. **Singleflight 효과 미검증**: DB 쿼리 수가 10% 이상으로 증가할 때
2. **데이터 일관성 파괴**: 여러 클라이언트가 다른 값을 수신할 때
3. **재현 불가능**: 동일한 테스트 조건에서 결과 재현 실패
4. **MTTD 0.1s 초과**: Cache miss 감지 시간이 0.1s를 넘을 때
5. **메인 스레드 블로킹**: Singleflight로 인해 메인 스레드 블로킹 발생

**현재 상태**: ✅ 모든 조건 충족 (Evidence: LOG L1, L2, Test 3 output)
