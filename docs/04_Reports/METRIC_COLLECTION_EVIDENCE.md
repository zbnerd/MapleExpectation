...
**Source**: `docs/04_Reports/Load_Tests/LOAD_TEST_REPORT_20260125_V4_PARALLEL_WRITEBEHIND.md`

| Metric | Value | Condition | Evidence |
|--------|-------|-----------|----------|
| **RPS (wrk, 200c)** | 719 | 200 connections, 10s | wrk Benchmark (#266) |
| **RPS (wrk, 100c)** | 674 | 100 connections, 30s | wrk Benchmark (#266) |
| **p50 Latency** | 163.89ms | wrk 100 connections | wrk Benchmark |
| **p50 Latency (Warm)** | 27ms | Warm Cache | Prometheus |
| **p95 Latency (Warm)** | 360ms | Warm Cache | Prometheus |
| **Throughput** | 4.56 MB/s | wrk 측정 | wrk Benchmark |
...
| 2. Expansion | 200-300KB (JSON) | 메모리에서 압축 해제 |
| 3. Parsing | 200-300KB | JSON 트리 파싱 + 비즈니스 로직 |
| 4. Serialization | 4.3KB (DTO) | 응답 변환 |

**Equivalent Load Calculation**:
```
1 Request = 300KB / 2KB (일반 요청) = 150 Standard Requests
...
| Baseline Source | LOAD_TEST_REPORT_20260125_V4_PARALLEL_WRITEBEHIND.md |
...
- [V4 Load Test Report](./Load_Tests/LOAD_TEST_REPORT_20260125_V4_PARALLEL_WRITEBEHIND.md) - 최신 부하 테스트 결과
